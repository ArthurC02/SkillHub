...
export interface DefaultApiInterface {
    ...
    /**
     * Usable before a run to withdraw a file and after a run to remove it. The stored object is deleted; snapshots keep the file\'s name and content hash so past runs stay traceable. 
    ...
    /**
     * Soft delete. The draft and its live datasets leave every read now and the stored objects are removed. Snapshots of past runs are retained and keep the prompt, the criteria and each file\'s name and content hash, so those runs stay traceable although they are no longer reproducible. 
     ...
     * @summary Offline dev-provider login
     * @param {DevLoginRequest} [devLoginRequest] 
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
     * @memberof DefaultApiInterface
    ...
    /**
     * Operator only. The single answer to 「現在到底有沒有在派送」, which is the whole reason 03:SEC-012 requires the P1 halt and the [Sandbox 隔離與執行安全](../../docs/adr/README.md#sandbox-隔離與執行安全) X-04\'s drain/suspend to be one switch rather than two.  Both triggers appear here in the same list and are told apart by `source`: `p1_incident` waits for a person, `orphan_threshold` clears itself after two consecutive reconciler rounds below the threshold. 
    ...
    /**
     * Operator only. OPERATOR_USER_IDS and BETA_ALLOWLIST stay deployment config; changing either is still an edit and a restart. This only shows what is in force. 
    ...
    /**
     * The question people ask `/healthz`. That one is a liveness probe and a constant, which is correct; what it must not be is the only endpoint that looks like it answers this one (05 R-36 第二段).  **`ready` is reachable only by measurement.** A capability whose variables are all present and which nothing probed reports `unmeasured`, a distinct value — configuration is not function. A launcher that tests only whether a variable is set, this platform\'s `/healthz` and apps/llm\'s own `/healthz` can all report green over a service that can perform none of its four jobs; that is the state this value names.  Always `200`, whatever the table says: a readiness endpoint that answered 503 because an OPTIONAL capability is off would make \"packaging is not configured\" indistinguishable from \"the process is broken\", which is the collapsing of two facts into one signal that this endpoint exists to undo.  Unauthenticated, because the launcher that has to read it holds no session — and R-36\'s hard condition is that the launcher reads THIS answer rather than keeping a second list of the same preconditions. Outside clean test mode the per-row detail is withheld: a list of what a deployment has not configured is reconnaissance, so `missing`, `detail`, `without` and `fix` are served only in clean test mode, where the reader is the operator on that machine. 
    ...
    /**
     * The read half of 02:GEN-003 「在工作區留下可查的失敗紀錄」. A generation that failed leaves an audit row; until this route existed, that row could only be seen by someone holding a database connection, which is not a record left in the workspace.  Requires a session and an invite, and is **mounted on the same flag as POST /skills/generate** — a failure list is a generation surface, and a route answering 200 with an empty array is still an answer about a feature that must not be discoverable. Where the flag is off this route does not exist and answers 404.  Workspace-scoped from the session; the caller never names a workspace (iron rule 3).  **The task description is not here and will not be added.** It belongs to the skill_sources row, under NFR-002 deletion; these rows are kept 400 days under a different rule, and one copy under each is a retention promise nobody made (the audit/analytics boundary decision draws the same line). 
    ...
    /**
     * Workspace scoped from the session like every other registry read : a skill in somebody else\'s workspace answers with an empty list rather than with their history.  This is the list the pre-run permission screen and the packaging screen pick a version from. Before it existed, both took the version id from the URL only, so the reader had to paste one by hand. 
    ...
    /**
     * FTS leg of hybrid retrieval; vector recall and match reasons arrive with the Explorer milestone. Scoped to the session workspace. 
    ...
    /**
     * Operator only. Sets the redistribution verdict on one skill and records who changed it and why.  This gate and the `restriction` above block the same download, so both need a route, an operator check and an audit event (`05` R-3c). A route without a contract entry is the same gap one layer up: an operator tool generated from this file could set a hold and not release content.  Idempotent, for the same reason as `restriction`: writing the value a skill already has is a second audit event and no change to the row.  The column write and the audit event share one transaction (iron rule 9). This gate decides whether content leaves the platform, so \"released, and no record of who released it\" is the one outcome that must be impossible.  Cross-workspace like `restriction`, and for the same reason: the verdict is about a *source*, so it has to reach the catalogue entry and every fork alike. Nothing here reads workspace-private data.  Operator-only is a ruling rather than a holding position (`05` R-3a). The route was written narrow while the question was open, on the grounds that widening later adds callers where narrowing later takes something away; the ruling kept it there, because that repo-root-license false positive was made by people who audit licences for a living.  Releasing a skill also has to carry evidence now — see `license_expression` and `license_source` (`05` R-3b). 
