...
先看 repo 有沒有把下面整套做成一支指令。本 repo 有：`go -C tools/devctl run . ci-status [ref] [--wait]`（`task ci:status`）。它照 §1～§3.1 做——完整 SHA、所有 workflow、失敗的 job 與 step、沒跑的 job——並用 §0.1 的憑證驗證，所以幾個工作階段同時輪詢也不會撞到未驗證的每小時 60 次上限。結束碼 0 綠、1 紅、3 還在跑、4 還沒有 run。下面的手動做法留給沒有這種指令的 repo，或指令本身壞掉的時候。

## 0. 沒有 `gh` 不等於查不到

**這一節是這份技能最貴的一課**：曾經有一整個工作階段連續十幾次回報「CI 查不到（沒有 `gh`、沒有 token）」，而那句話**從頭到尾是錯的**。沒有人去測，只是看到 `gh` 不在 PATH 上就結案了。
