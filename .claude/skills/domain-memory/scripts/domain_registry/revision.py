...
def registry_digest(root: Path) -> str:
    ...
    for name in ("manifest.json", *sorted(ASSET_KEYS)):
        ...
        if not path.is_file():
            raise ValueError(f"registry asset is missing while calculating revision: {path}")
...
def valid_registry_revision(value: Any) -> bool:
    if not isinstance(value, dict):
        return False
    commit = value.get("observed_commit")
    digest = value.get("registry_digest")
    return (
        (commit is None or (isinstance(commit, str) and len(commit) == 40 and all(character in "0123456789abcdef" for character in commit)))
        and isinstance(digest, str)
        and digest.startswith("sha256:")
        and len(digest) == 71
        and all(character in "0123456789abcdef" for character in digest.removeprefix("sha256:"))
    )


def require_current_registry_revision(proposal: dict[str, Any], root: Path, repo_root: Path) -> None:
    expected = proposal.get("base_registry_revision")
    if not valid_registry_revision(expected):
        raise ValueError("proposal requires a Git commit and Registry digest base revision")
    actual = current_registry_revision(root, repo_root)
    if expected["registry_digest"] != actual["registry_digest"]:
        raise ValueError("proposal base revision is stale; rebase and obtain fresh approval")
