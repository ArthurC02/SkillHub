from .common import ASSET_FORMATS, ASSET_KEYS, REGISTRY_STATUSES, REVIEW_REQUIRED_FIELDS, load_json, registry_dir, template_dir
...
def validate(root: Path, repo_root: Path | None, require_reviewed: bool) -> list[str]:
    ...
    for name, entries in records.items():
        ...
        for entry in entries:
            status = entry.get("review_status", asset_status) if name == "decisions.json" else entry.get("status", asset_status)
            ...
            if status == "reviewed":
                ...
                if missing:
                    errors.append(f"{name}:{entry.get('id')} is reviewed but missing {', '.join(missing)}")
                if not entry.get("evidence"):
                    errors.append(f"{name}:{entry.get('id')} is reviewed but lacks evidence")
    ...
    for entry in records.get("contexts.json", []):
        ...
        if not isinstance(declarations, list):
            errors.append(f"contexts.json:{entry.get('id')} confirmed_absences must be a list")
            continue
        for declaration in declarations:
            if not isinstance(declaration, dict) or declaration.get("asset") not in ABSENCE_ASSETS:
                errors.append(f"contexts.json:{entry.get('id')} confirms an absence of an unknown asset; "
                              f"use one of {', '.join(sorted(ABSENCE_ASSETS))}")
                continue
            if not isinstance(declaration.get("reason"), str) or not declaration["reason"].strip():
                errors.append(f"contexts.json:{entry.get('id')} confirms an absence of {declaration['asset']} "
    ...
    for name in ("aggregates.json", "interactions.json"):
        for entry in records.get(name, []):
            fields = ["context"] if name == "aggregates.json" else ["producer_context", "consumer_context"]
            for field in fields:
                if entry.get(field) not in known_contexts:
                    errors.append(f"{name}:{entry.get('id')} references unknown context")
    contract_ids = {entry.get("id") for entry in records.get("contracts.json", [])}
    for name, field in (("events.json", "owner_context"), ("capabilities.json", "context"), ("value-objects.json", "context")):
        for entry in records.get(name, []):
            if entry.get(field) not in known_contexts:
                errors.append(f"{name}:{entry.get('id')} references unknown context")
    for entry in records.get("dependency-policies.json", []):
        if entry.get("from_context") not in known_contexts or entry.get("to_context") not in known_contexts:
            errors.append(f"dependency-policies.json:{entry.get('id')} references unknown context")
    for entry in records.get("contracts.json", []):
        if entry.get("producer_context") not in known_contexts:
            errors.append(f"contracts.json:{entry.get('id')} references unknown producer context")
        consumers = entry.get("consumer_contexts", [])
        if not isinstance(consumers, list) or set(consumers) - known_contexts:
            errors.append(f"contracts.json:{entry.get('id')} references unknown consumer context")
    for entry in records.get("interactions.json", []):
        contract_id = entry.get("contract_id")
        if contract_id is not None and contract_id not in contract_ids:
            errors.append(f"interactions.json:{entry.get('id')} references unknown contract")
    if repo_root is not None:
        for reference in evidence_values(records):
            result = verify(reference, repo_root)
            if result["status"] in {"invalid", "missing"}:
                errors.append(f"evidence {result['status']}: {result.get('reason', result.get('path'))}")
    return errors


def verify_evidence(root: Path, repo_root: Path) -> dict[str, Any]:
    results = [verify(reference, repo_root) for reference in evidence_values({name: asset_records(root, name) for name in ASSET_KEYS})]
    summary = {status: sum(item["status"] == status for item in results) for status in ("current", "stale", "missing", "invalid", "legacy-unverified")}
    kinds = sorted({item["source_kind"] for item in results if "source_kind" in item})
    summary["by_source_kind"] = {kind: sum(item.get("source_kind") == kind for item in results) for kind in kinds}
        ...
        if isinstance(value, dict):
            ...
            for key, child in value.items():
                if key == "evidence" and isinstance(child, list):
                    replacement = [classified(migrate_legacy(item, repo_root), source_map) for item in child]
                    migrated += sum(before != after for before, after in zip(child, replacement))
...
def contexts_holding(records: dict[str, list[dict[str, Any]]]) -> dict[str, set[Any]]:
    ...
    return {
        "vocabulary": {context for entry in records.get("vocabulary.json", []) for context in entry.get("contexts", [])},
        "aggregates": {entry.get("context") for entry in records.get("aggregates.json", [])},
        "rules": {context for entry in records.get("rules.json", []) for context in entry.get("contexts", [])},
        "contracts": contracts,
    }
...
def coverage(root: Path) -> dict[str, Any]:
    ...
    holding = contexts_holding({
        "vocabulary.json": vocabulary, "aggregates.json": aggregates,
        "rules.json": rules, "contracts.json": contracts,
    })
...
def context_model(root: Path, identifier: str) -> dict[str, Any] | None:
    ...
    return {
        "context": context,
        "vocabulary": [entry for entry in asset_records(root, "vocabulary.json") if identifier in entry.get("contexts", [])],
        "aggregates": [entry for entry in asset_records(root, "aggregates.json") if entry.get("context") == identifier],
        "rules": [entry for entry in asset_records(root, "rules.json") if identifier in entry.get("contexts", [])],
        "contracts": [entry for entry in contracts if identifier == entry.get("producer_context") or identifier in entry.get("consumer_contexts", [])],
        "interactions": [entry for entry in asset_records(root, "interactions.json") if identifier in {entry.get("producer_context"), entry.get("consumer_context")}],
        "events": [entry for entry in asset_records(root, "events.json") if entry.get("owner_context") == identifier],
        "capabilities": [entry for entry in asset_records(root, "capabilities.json") if entry.get("context") == identifier],
        "value_objects": [entry for entry in asset_records(root, "value-objects.json") if entry.get("context") == identifier],
    }
...
def boundary_analysis(root: Path, source: str, target: str) -> dict[str, Any]:
    interactions = [
        ...
        if entry.get("producer_context") == source and entry.get("consumer_context") == target
    ]
    contracts = [
        entry
        for entry in asset_records(root, "contracts.json")
        if entry.get("producer_context") == source and target in entry.get("consumer_contexts", [])
    ]
    dependencies = [entry for entry in asset_records(root, "dependency-policies.json") if entry.get("from_context") == source and entry.get("to_context") == target]
    return {
