...
| Apply an approved change | `apply-approved-updates --package-root <path> --registry-root <path> --repo-root <repo>` | Requires a valid approved current Change Package and, through a recoverable update, promotes new or candidate records and removes records the proposal names — a reviewed record only when the proposal supersedes the one that reviewed it. |
...
For an update, include `registry_updates` in `domain-change-proposal.json`. Each update names an asset and uses `operation: "upsert"` with the complete record, or `operation: "remove"` with the `id` of a record the Registry holds. A removal is refused when no such record exists, so a mistyped id fails loudly instead of doing nothing, and the staged Registry is validated before the update lands, so a record the rest of the Registry still references cannot be removed. Submission records a deterministic digest of all standard Registry files plus the observed Git HEAD when available. Finalize and apply require the Registry digest to remain current; an unrelated Git commit alone does not invalidate review. They do not traverse Git history. `verify-proposal` validates submitted test results as attestations; it does not run an arbitrary command from the package. The apply command validates the Change Package, then rejects an update that would overwrite or remove a reviewed record unless the proposal's `supersedes` names the exact proposal that reviewed it. Naming that approval is what makes the replacement governed: the replacing proposal still carries its own approvals and SCM evidence, and one that names a different proposal, or none, is refused. This is how a reviewed record follows a domain that changed; a record whose cited line moved is corrected this way rather than left stale.
