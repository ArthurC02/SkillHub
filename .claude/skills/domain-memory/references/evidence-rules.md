...
Build every citation with `cite`. Its digests are computed exactly as `verify-evidence` recomputes them, over the whole file and over the cited lines; a citation assembled by hand with a different line-splitting rule verifies as `stale`, which reads as a source that changed rather than a citation that was never right.
