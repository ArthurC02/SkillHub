...
[模型閘道與可觀測性](../../docs/adr/README.md#模型閘道與可觀測性)把三種能力切開，這裡只涵蓋第一種：
...
**告警不得依賴 Run Trace。** Trace payload 大量來自 Sandbox，是使用者可操控的輸入；讓告警讀它等於把觸發權交給不受信任的來源。`alerts.yml` 的每一條規則都只讀服務自報的指標。
...
後三個是 **SBX-012**（[Sandbox 隔離與執行安全](../../docs/adr/README.md#sandbox-隔離與執行安全) X-03／X-04 的量測前提）：
...
**掛法**：`task dev:observability` 起一個 Prometheus（`infra/compose/docker-compose.yml` 的 `observability` profile），設定在 [`prometheus.yml`](prometheus.yml)，`rule_files` 指向本目錄的 `alerts.yml`，UI 在 `127.0.0.1:9095`。

**規則曾經寫好而沒有任何東西載入它們。** 指標寫了、規則寫了、門檻由[Sandbox 隔離與執行安全](../../docs/adr/README.md#sandbox-隔離與執行安全)定值了，而中間沒有 evaluator——**X-02～X-04 因此沒有觸發路徑，缺的不是門檻是求值**。載入由 `promtool check rules` 與實際 `/api/v1/rules` 各驗一次。**現為 7 群組 33 條**（`skillhub-run-funnel`、`skillhub-provider-health`、`skillhub-cleanup-and-leaks`、`skillhub-trace-pipeline`、`skillhub-foundation-signals`、`skillhub-model-gateway`、`skillhub-database`）。後兩群讀的是部署在控制平面上的探測與 exporter（blackbox-exporter 對閘道健康端點的 `probe_success`；postgres-exporter 的 `pg_up`、`pg_stat_archiver_*`），不是平台程式自報的指標。條數會再長，**以 `alerts.yml` 的列為準，本檔的數字只是當下的快照**。
...
**例外：`skillhub-cleanup-and-leaks` 群組的門檻已定值**——SEC-002 的六項門檻（威脅模型 Q18）由[Sandbox 隔離與執行安全](../../docs/adr/README.md#sandbox-隔離與執行安全)第二部分定案（X-02 每 5 分鐘、X-03 同一筆連 2 輪、X-04 單節點 50%／全池 25% 下限 2 筆、6b 連 3 輪撤銷失敗）。**該群組的規則已全部是正式形式**（SBX-012 落地後）：`LeakedSandboxStillPresent` 改讀 `skillhub_orphan_sandbox_persistent`、`CredentialRevokeFailing` 改讀 `skillhub_gateway_revoke_failed_total`，原本以累加計數器近似「同一筆連續 N 輪」與「哪一種資源撤不掉」的兩條過渡規則已移除近似。**[Sandbox 隔離與執行安全](../../docs/adr/README.md#sandbox-隔離與執行安全)定的動作（drain 節點、暫停整池派送、暫停 P-03 例行重建）屬平台實作，不是 Alertmanager 的職責。**（前兩個動作已由 `SEC-012` 落地——`dispatch_halts` 與 `internal/trial/execution/halt.go`，Reconciler 直接撥開關；本檔因此新增 `DispatchHalted`／`DispatchHaltedForIncident` 兩條，用途是**讓人知道平台已經自己停下來了**，仍然不是由告警去執行動作。**P-03 例行重建的暫停仍未做**：節點重建是部署期的動作，平台這一側沒有它的控制點。）

`CleanupBacklogGrowing` 的門檻不在[Sandbox 隔離與執行安全](../../docs/adr/README.md#sandbox-隔離與執行安全)的六項之內，但**依封測容量校準過**：`> 5` → `> 2`。舊值大於整池 4 個 slot，代表整池沙箱全數洩漏都還低於門檻；新值取封測 4 slot 的 50%，與 X-04 單節點 drain 用同一個比例。池容量改變時要重推。
