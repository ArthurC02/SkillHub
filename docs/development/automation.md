...
2. **未匯出的名字也算。** 反引號裡第一個字母小寫、後面帶駝峰的名字，與匯出名字一樣要存在——寫這一段的當天就踩到：一份 ADR 引用 `applyCleanModePool`，而原本的正規式只認大寫開頭，所以它引用一個被改名的函式也不會紅。沒有駝峰的小寫字（`clean`、`gvisor`）仍然是散文，不是符號。外部函式庫的 API 名（React、TanStack Router、Playwright）由 `allowedDocWords` 記著理由。
