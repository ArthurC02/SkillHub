...
| `adr-citations` | 索引裡 `Superseded` 的狀態格要點名取代者；ADR 檔頭 `- 取代：`／`- 修訂：`（第一個「（」或「；」之前）點名的較舊 ADR，索引那一列要寫出這份新的；`docs/adr/`、`docs/plans/mvp/` 以外引用 `Superseded` ADR 的那一行要同時寫出取代者；非 markdown 檔（不含 `.jsonl`、generated 與凍結語料）的 ADR 編號總數等於 `adrCitationCeiling`，多了是新引用、少了要把上限調低（ADR-086） | `tools/devctl/adr_citations.go` |
