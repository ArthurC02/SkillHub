...
DISC-005 的七項允收已各有具名測試、已提交，且該提交的完整 SHA 遠端 CI 全綠；中文詞法的明文決定是「只走向量腿」。**這份提案本身仍是 draft**：格式驗證通過不等於已審查，`registry_updates` 是空的，因此沒有任何程式依賴未審查的紀錄。<br>仍有缺口而不屬於本需求的：未記帳並行費用的預留保證、其他能力服務啟動入口的相同成本邊界、正式 TLS 與部署代理下的端到端，以及內容審核與曝光資格的設計（後者見 `04` 的內容審核殘項與 `catalog-exposure-review`）。投毒量測維持常設紅線，它的成立條件不含「通過」。
