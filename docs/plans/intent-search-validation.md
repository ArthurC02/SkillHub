...
DISC-005 的七項允收各有具名測試，對照見[需求 ID × 測試對照表](requirement-test-matrix.md)。中文詞法的明文決定是 `02:DISC-005` ⑤ 的 (b)「中文查詢只走向量腿」——**不是因為沒有量，而是因為量過的每一個候選都不及下界，或者以投毒紅線換取召回**（見〈已量測並否決的中文詞法候選〉）。

這份文件量的是檢索行為與它的邊界，**不是理解品質**：沒有帶標註的任務描述，golden query set 量的是召回。人工策展的辨識能力、以及正式 TLS 與部署代理下的端到端，都不在它的範圍內。

領域變更交接集中於 [Domain Memory 提案草稿](../domain-memory/changes/catalog-correctable-intent/draft-pr.md)，狀態仍是 draft；格式驗證通過不等於已審查或允收完成。
...
### bigram 正規化後的 trigram 候選（達下界但否決）
...
另以 `search-intent/v3-experiment` 只把關鍵詞改為英文，五欄仍要求引用原句；正式提示沒有變更。相同 60 題的一次取樣有 58 題合法、2 題由現有驗證拒絕，Gateway 回報 US$0.0229604，獨立限額金鑰已撤銷。現行詞法路徑的中文 Top-1 為原始增強 1／30、v7 3／30，中英干擾題仍各拒答 6／6；加入投毒後兩份語料皆有中文 5 題、英文 4 題出現投毒 Top-3。這個提示變體未採用。

**這些候選合起來就是 (a) 被否決的理由**，`02:DISC-005` ⑤ 據此裁定 (b)。裁定沒有關閉 (a)：`pg_bigm` 或 `pgroonga` 進得了自架映像，或有人提出同時過得了 24／30 下界與投毒紅線的詞法規則時，要重開。下一個候選必須同時重跑固定品質與投毒對照，並確認名稱／特定詞與混合向量搜尋沒有回歸。v2 的「不要按任務主題擅加分類」已在這 60 題取樣中維持空篩選，但仍需含明確篩選要求的正例、重複取樣及人工意圖欄位判讀；不能推論為所有輸入都正確。
...
`NFR-001` 第 5 條對未登入搜尋要求的是**速率限制與濫用防護**，兩條匿名搜尋路由都掛在同一個位址限流器上；`TestAnonymousIntentAnalysisRecordsVersionedCostEvenForInvalidOutput` 斷言 GET 與 POST 超限後都回 429 帶 `Retry-After`，**而且分析呼叫次數停在一次**——限流擋掉的是第二次花費，不只是第二個請求。

這與「花費前的預算阻擋」不是同一個保證。費用記帳、請求限流、呼叫逾時與預算預留各自獨立：成本事件是從回應的用量寫出來的，因此必然在呼叫之後；`modelbudget` 的 endpoint 設定管理逾時，不是金額預留。下面兩節是這條邊界的實際形狀，**不是宣稱匿名搜尋絕不超支**。
...
跑法：`task test:platform` 需要獨立 `_test` 測試庫並設 `SKILLHUB_REQUIRE_DB=1`；四個 Go／Python 搜尋情境另需 `SKILLHUB_CREATION_PYTHON` 指向 Python 執行檔並設 `SKILLHUB_REQUIRE_CREATION_PYTHON=1`，否則它們會跳過而不是失敗。`task test:web` 與 `task test:llm` 不需要資料庫。

**跳過的案例不算通過**：平台側的跳過都是需要語料、真實物件儲存、Sandbox、live fetch 或付費模型的驗收；Python 側是兩個沒有 schema model 的 liveness 契約案例與四個未啟用的付費 Gateway 案例。`task lint:web` exit 0 但仍列 React 與 TypeScript 警告，不宣稱零警告。每次跑要核對通過筆數不是零，`go test ./...` 在沒有測試資料庫時會整批跳過卻回 ok。

跨套件重建 schema 由既有資料庫鎖序列化，等待鎖不能誤判為服務失效。`task gen:check` 只證明四組產物與契約一致，`automation-check` 只證明文件與所有權對帳；兩者都不解除本文其餘各節的量測邊界。
