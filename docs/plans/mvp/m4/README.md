...
- 日期：2026-08-17（計畫）
- 狀態：**計畫，未開工。** 本文件是 M4 的入口：範圍、接點對應、批次分解、程式／人的分界、未決點與檔案地圖。
...
| **PDM-009** 封測人數、招募方式、成功門檻 | **完全沒有提案**——`m0/pdm-proposals.md` 沒有 PDM-009 這一節；`pdm-proposals` 假設 30 人、`cost-estimation` 用 20 人，兩份文件互斥 | `BETA-001`／`005` 與 `RELEASE-009` 全部不可判定。**這是 M4 唯一一個連提案都沒有的決策**，建議在批 1 之前先補一份提案（agent 寫得出提案，拍板仍在人） |
...
| **乙-14（新，本計畫提出）** | **甲類四項是否在封測前到期**（§4）。建議「到期」；替代路徑「只讀不跑的封測」的代價已在 §4 寫清楚 |
...
**要拍板的是兩件事**：①欄位放 `skills`（可撤銷）還是 `skill_versions`（不可變）——建議 `skills`，理由同 `0023` 的既有裁定；②預設值是 `unknown`（保守，未知即擋）還是 `allowed`（寬鬆）——**建議 `unknown`**，因為 ADR-021 §5.3 記錄的那個真實誤判錯的正是放行方向。建議併入 ADR-027。
