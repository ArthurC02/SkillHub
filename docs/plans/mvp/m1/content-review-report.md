...
| [`content-summaries.md`](../content/content-summaries.md) | 這 11 筆的線上文字已換版；§2.4 的非決定性上限再添一批實例。**審核紀錄本體不得就地改寫**（`02` §4.7），故本節不動它 |
...
**(2) 自利風險確實存在，且比報告自陳的更大。** 報告只寫「形式上有自利之嫌」。實際上：`excel-format` 是 `documents` 的 **D-4 精選**，正是 2026-08-15 補足「`documents` 精選 3 → 4」缺口的那一筆（[curated-skill-list §9](../content/curated-skill-list.md)）。若它退回 `需修改` 並走下架，`documents` 精選掉回 3，**跌破 `CONTENT-001` 每類 4–6 的下限**。修正的受益方與修正的作者是同一個。**這一點必須先講明，追認才有意義。**
