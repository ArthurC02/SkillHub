...
- 日期：**2026-08-15**
- 對應：[意圖搜尋](../../../adr/README.md#意圖搜尋)「待決策」第三項（golden query set 移交 M1，含跨 repo 抽樣 ≤ 20% 限制）、DISC-001／DISC-002、M1 驗證閘門
...
| 索引欄位 | `summary`＝`name: description`（frontmatter）。這是[意圖搜尋](../../../adr/README.md#意圖搜尋)定案的「索引時 LLM 增強產出」的**代理**，非真實產出（見 §6 限制 1）。另跑 `fulltext` 對照 |
...
**這條限制的實際代價**：`data` 類別原本被點名為同質風險最高者，但真正被限制卡住的是 `documents` 與 `writing`——[content-candidates.md](../content/content-candidates.md) 記錄的確認候選是 `documents` 4 個、`writing` 2 個，**全部來自 `anthropics/skills` 單一 repo**。若只用該清單出題，這兩類的 repo 占比是 100%，直接違反這條跨 repo 抽樣限制。為了讓量測成立，本次額外納入 9 個 repo 的語料（§2）——這同時說明 CONTENT-003 記錄的「`documents`／`writing` 供給缺口」不只影響目錄豐富度，**它會讓這兩類的 recall 量測本身失去鑑別力**。
...
語料放大到 31 份後，摘要索引的優勢**由 Spike 的「平手」變成明顯領先**（Top-1 +21 個百分點）。Spike §9.3 提醒的「fulltext 已被截斷、低估長文稀釋效應」在這裡得到證實：語料愈長、愈雜，全文索引愈稀釋。這是[意圖搜尋](../../../adr/README.md#意圖搜尋)「索引時摘要是必要項」判斷的直接量化支持。
...
兩條 miss 全部落在 `documents` / 繁中 / 未明說格式的情境，且**兩條都指向同一個修補**：[意圖搜尋](../../../adr/README.md#意圖搜尋)定案的索引時「適用任務範例句」。D02 需要 `pdf` 的範例句寫出「從掃描件抽表格」，D09 需要 `pitchcraft` 有中文任務範例句。裸 frontmatter 補不了這個缺口。

### 3.7 對 RRF 判斷的再確認——負面訊號比 Spike 更強
...
**建議（僅建議，不改 ADR）**：MVP 首發不要把等權 RRF 當預設排序器。混合檢索的價值維持[意圖搜尋](../../../adr/README.md#意圖搜尋)已定案的說法——**召回覆蓋**，而非排序增益；實作上應以「兩腿取聯集擴大候選集、再依向量分數排序」取代「以 RRF 分數排序」。若要保留 RRF，需先做依查詢語言／腿得分的動態調權，而該決策已明確把加權融合列為「後續優化、非 MVP 首發假設」。
...
[意圖搜尋](../../../adr/README.md#意圖搜尋)移交的目標「干擾 ≥ 75% 正確回無結果、召回損失 ≤ 5%」**達標，而且有大幅餘裕**：召回損失預算內可達到的最高拒答率是 100%（門檻 0.285 以下皆然）。

### 4.3 建議門檻

| 候選 | 餘弦相似度下限 | 餘弦距離上限 | 干擾拒答 | 召回損失 | 何時選它 |
| --- | --- | --- | --- | --- | --- |
| A（保守） | 0.16 | 0.84 | 75%（9/12） | 0% | 目錄規模快速擴張、寧可多回幾筆弱相關也不願誤殺時。剛好踩線達到意圖搜尋移交的目標 |
...
> pgvector 的 `<=>` 運算子回傳餘弦距離＝1 − 餘弦相似度，因此 SQL 端的條件是 `embedding <=> :q <= 0.79`。門檻只適用於 `text-embedding-3-small` 的 1536 維向量；換模型必須整組重測（[意圖搜尋](../../../adr/README.md#意圖搜尋)已記錄「Embedding 模型更換需全量重建索引」）。
...
2. **索引欄位換成真實的 LLM 增強產出**。本次量的是裸 frontmatter。[意圖搜尋](../../../adr/README.md#意圖搜尋)定案的索引欄位是 LLM 生成的白話摘要＋適用任務範例句，會**同時**推高正解相似度（查詢與範例句同構）與干擾相似度（摘要更長、更泛化）。門檻必須在 CONTENT-005／索引管線就緒後重新推導，**不可沿用本數字**。
...
前兩項都由已定案的[意圖搜尋](../../../adr/README.md#意圖搜尋)覆蓋。**建議：M1 驗證閘門的使用者測試排在 CONTENT-005（白話摘要）與索引時增強管線就緒之後**，否則測到的是缺工序的中間態；若要提前測，`documents` 類別的結論不具代表性。

---

## 6. 限制（哪些沒驗證到）

1. **索引時 LLM 增強仍是模擬**。索引欄位是裸 frontmatter，不是[意圖搜尋](../../../adr/README.md#意圖搜尋)定案的「LLM 生成白話摘要＋適用任務範例句＋標籤」。所有數字（尤其門檻）都是**該工序缺席時的下界**。
2. **查詢改寫未做、符合原因未做**。[意圖搜尋](../../../adr/README.md#意圖搜尋)定案的改寫與理由生成／潤飾都未呼叫 LLM。
3. **未經 LiteLLM 閘道**。本工具直接呼叫供應商 Embedding API。這對離線評測工具可接受，**但產品實作不得比照**（AGENTS.md 鐵律 8、[模型閘道與可觀測性](../../../adr/README.md#模型閘道與可觀測性)）。
4. **未量測延遲**。NFR-004（p95 < 2 秒）需要 pgvector 實際規模與改寫呼叫的端到端量測，仍待[資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)就緒。
...
`--selfcheck` 檢查的是**資料集本身的完整性**，不是檢索品質：查詢 ID 不重複、每類別 20 條、繁中 12 條、干擾 ≥ 4 條、所有 gold 都存在於語料、干擾查詢確實沒有 gold、**跨 repo 抽樣限制的 20% 上限未被違反**，外加檢索與門檻計算的最小斷言。它不需要金鑰、不需要網路、跑不到一秒。
...
[意圖搜尋](../../../adr/README.md#意圖搜尋)「待決策」已指出回歸節奏與 Judge 品質回歸可由 Langfuse Datasets/Evals 承載（見[模型閘道與可觀測性](../../../adr/README.md#模型閘道與可觀測性)）。本工具的 `queries.json` 結構（query／gold_primary／gold_acceptable／category／lang／kind）可直接映射為 Langfuse Dataset item。**建議在索引管線與 LiteLLM 閘道就緒後遷移**，屆時本工具退回為「離線、無依賴的門檻推導器」，不再擔任回歸把關。

---

## 9. 摘要：可以直接被引用的三個結論

1. **`text-embedding-3-small` 的原句跨語言召回在 31 份／13 repo 的語料上仍然成立**：繁中原句對英文語料 recall@5 93%、Top-3 93%，不需要查詢改寫就已達可用水準。[意圖搜尋](../../../adr/README.md#意圖搜尋)判斷向量腿是跨語言召回的承載者，在放大 6 倍的查詢集上重現。
...
3. **等權 RRF 在這個查詢量下明顯有害**（Top-1 −11 條、recall@5 −5 條），比 Spike 觀察到的「倒退一名」嚴重得多。[意圖搜尋](../../../adr/README.md#意圖搜尋)已寫明「RRF 只是召回覆蓋手段，不是排序品質來源」——建議實作時據此把排序權交給向量分數，BM25 只擴大候選集。

---

## 10. v2：增強索引重測（2026-08-15）

- 觸發：**§4.4 條件 2**——索引欄位由裸 frontmatter 換成[意圖搜尋](../../../adr/README.md#意圖搜尋)定案的真實 LLM 增強產出。索引管線（`services/platform/internal/ingest/enrich.go`）已上線，§3／§4 的數字自此為基線對照，不再是現行值。
...
[意圖搜尋](../../../adr/README.md#意圖搜尋)「索引時摘要是必要項」的判斷在真實產出上得到最強的一次支持：增強索引比裸 frontmatter 高 15 個百分點、比全文索引高 36 個百分點。**「摘要優於全文」不是因為短，而是因為它是為檢索寫的**——全文仍是最差的一欄。

### 10.4 D02／D09：§3.6 的預測成立

§3.6 對兩條 recall@5 miss 的診斷是「兩條都指向同一個修補：[意圖搜尋](../../../adr/README.md#意圖搜尋)定案的索引時適用任務範例句」。**兩條都解掉了**：
...
**注意舊門檻 0.21 在 v2 已不足**：干擾拒答只剩 8/12（67%），**跌破[意圖搜尋](../../../adr/README.md#意圖搜尋)移交的 75% 目標**。沿用 v1 數字會讓「無結果」能力實際退化——這正是 §4.4 要求重測的理由。
...
達成[意圖搜尋](../../../adr/README.md#意圖搜尋)的目標（拒答 ≥ 75%、損失 ≤ 5%）且餘裕兩側都比 v1 大。各拒答率的最低門檻：75% → 0.205、90% → 0.215、100% → 0.220；召回損失預算 5% 內可推到 0.470 仍 100% 拒答，餘裕極大。
...
1. **未經 LiteLLM 閘道（dev 取徑）**。本次為離線評測，把 `services/llm` 的 `LITELLM_BASE_URL` 直接指向 OpenAI、`LITELLM_API_KEY` 由 repo 根 `.env` 的 `OPENAI_API_KEY` 注入**進程環境變數**（不落任何檔案）。這與 §6-3 同性質：**離線評測工具的例外，產品實作不得比照**（AGENTS.md 鐵律 8、[模型閘道與可觀測性](../../../adr/README.md#模型閘道與可觀測性)）。金鑰不在 `corpus_enriched/`、`results_v2_enriched.txt` 或本報告中。
