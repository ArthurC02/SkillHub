...
## 10. 回填 `artifacts` 之後重評（2026-08-23，同日第四輪）

丙-13 回填完成（`tools/content/backfill_artifacts.py`，123 個封存 → **177 列**，涵蓋 123 個 Run），把第 9 節那 53 筆重評了一次。

### 10.1 判定改變了，而且改變的方向正是丙-13 預測的

| `overall`（各 Run 的最新 revision） | 回填前 | 回填後 |
| --- | --- | --- |
| `met` | **0** | **18** |
| `partially_met` | 23 | 5 |
| `not_met` | 4 | **0** |
...
**18 筆從「沒達成／部分達成」變成「達成」。** 那些 Run 一直都有產出檔案——中位數 1 個、最多 6 個——只是平台從沒把清單寫下來，於是 Judge 在「這個 Run 什麼都沒產出」的前提下判了它們。

**這 18 筆不是被修正的錯字，是被修正的判定**，而且在回填之前它們已經以不可變 revision 的形式存在了幾個小時。第 9 節那句「錯的那一份會永遠留在 revision 清單裡」是字面成立的：**它們還在，只是不再是最新的那一份。**

「reported success and no output files were recorded」這條 finding 從 **84／84** 降到 **3／53**——而那 3 筆逐一查過，**物件儲存裡確實沒有它們的封存**（`run-artifacts/<run_id>/` 整個前綴不存在）。**對這 3 筆而言那句話現在是真的**，這正是它該有的樣子。
...
**最該讀的是「模型提了 0 條」那一列：從 0 變成 15。** 回填之前，每一次呼叫模型都看得到一個空的產出清單，於是每一次都有話說；回填之後有 15 次它看完說沒什麼要改的。**第 9 節說本報告的提案「主題被空 artifact 綁架」，這一列是那句話的量化。**

`stored` 的 65% → 50% 不要過度解讀：分母從 37 掉到 18，兩個比率的信賴區間重疊得很厲害。

### 10.3 ③ 重測

| 輪次 | `stored` | Skill 已軟刪除，無法預覽 | 預覽 | `applicable` | `blocked` |
| --- | --- | --- | --- | --- | --- |
| 第二＋三輪 | 24 | 8 | 16 | 16 | 0 |
| 第四輪 | 9 | 2 | 7 | 7 | 0 |
| **合計** | **33** | **10** | **23** | **23** | **0** |

**`blocked_reason` 分布仍是空集合，n 從 16 變成 23。** 目標檔案仍然 **23／23 全是 `SKILL.md`**——回填沒有改變這一點，所以 5.1 節那句從「提案主題集中導致」升回一個獨立的觀察：**v2 就是只改 `SKILL.md`。**

### 10.4 這一輪的成本

53 次 judge $0.9099、33 次 suggest $0.8689，本輪 **$1.7788**；本日累計 **$5.2647**。

### 10.5 §9.2 的截斷沒有變

`trace_digest.entries` 的截斷 **30／53**，而 `undetermined` 也是 **30**——回填前是 26／26，回填後是 30／30，**相關性原封不動**。丙-13 與丙-47 是兩件事，這一輪把它證實了：**回填 `artifacts` 一點也沒有動到它。**
