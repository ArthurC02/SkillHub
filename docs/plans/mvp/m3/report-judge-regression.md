...
> [引用回驗依內容本身而非宣稱來源判定的決策](../../../adr/README.md#評估判定與-judge-信任邊界) 之後，`judge.go` 多了
...
> （它跟了引用回驗改比對解碼後值的那項決策，漏了引用依內容本身而非宣稱來源判定的那項決策）。
...
- 日期：**2026-08-17**
- 對應需求：[`02:EVAL-013`](../../02-specifications-and-acceptance-criteria.md)（Judge 判準驗證）；相關 `02:EVAL-001` 第 5 條、[評估判定與 Judge 信任邊界](../../../adr/README.md#評估判定與-judge-信任邊界)
...
這不是標註可議，是**兩個問題兩個答案**：`runs.status` 回答「這次執行發生了什麼」，驗收條件回答「這條做到了沒有」。把 Run 終態與驗收條件拆成兩欄的那項決策，其理由在這一筆上是看得見的。Judge 兩輪都判 c1／c2 `passed`、c3 `failed`（最終回覆是空的），整體提案 `partially_met`——完全正確。
...
**評分的是「儲存後的判定」，不是模型的原始回答。** harness 逐條重做 Go 側的兩個降級（評估判定信任邊界第 3 條防線的引用回驗、以及證據不完整／截斷時 `passed` 一律降 `undetermined`），因為那才是使用者會看到的東西。
...
**成本歸因是可查的**：每次呼叫都帶 `run_id`／`evaluation_id` metadata，服務層回報值與閘道 per-key spend 差 $0.0001（浮點捨入），**權威來源仍是閘道**（[模型閘道與可觀測性](../../../adr/README.md#模型閘道與可觀測性)、丙-3）。這條路徑本身是本次一併補上的——`JudgeRunResponse.usage` 在此之前不存在於契約與 Python 端，而 Go 呼叫端（`internal/eval/judge.go`）早就在讀它了。
...
| 2 | **補一組注入樣本進回歸集**：現有 45 筆沒有一筆敵意內容，而 評估判定信任邊界決策第 3 條的四條防線目前只有第 1、3 條有實測。樣本可合成（不需要真的跑 Run），但要與 45 筆分開統計 | ~~M3 第 7 批或 M4~~ **✅ 2026-08-17 完成，見 §12**（13 樣本、27 條判定、讓步 0；與 45 筆分開統計、分開落檔） |
...
結果**追加**到 `tools/eval-regression/results.jsonl`，一列一個 Run。**不要覆寫該檔**：`regression_id` 是區分兩輪的唯一依據，覆寫等於讓「尺變了」變成看不見的事（評估判定信任邊界決策第 1 條）。
...
這是 `writing-rubrics.md` §6.2 **G5／G6 的延伸，但不是同一件事**——G5／G6 說的是「Judge 讀不到 artifact 內容」，這裡說的是「**在讀不到的前提下，平台仍會接受一段宣稱來自 artifact 的引文**」。記為 **G7**，處置建議兩選一，**本批不改，因為它動的是 評估判定信任邊界第 3 條防線的判準**：
...
### 12.2 攻擊型別覆蓋（對映評估判定信任邊界決策第 3 條的四條防線）
...
**這個 0 要照它的範圍讀。** 它是「這 13 個攻擊型別、這個 prompt 版本、這個模型」上的 0，不是「注入無效」。評估判定信任邊界決策的影響分析已經寫明四條防線「降低而非消除」成功率，本節給那句話一個可重跑的數字，沒有改寫它。
...
**還有一件同樣值得記的**：5 筆裡有 **3 筆**同時附了一則 `agent_output` 引文，而那一則**回驗通過**（`rejected: null`）。**一則壞引用會拖垮整條準則，即使同一條準則另有一則驗得過的引用。** 那是評估判定信任邊界第 3 條防線現行的行為，本報告只記錄，不在同一批改動它——它是一道有 ADR 的防線，而今天這份量測正好是任何修改該有的依據。
...
## 16. C 輪：引用回驗改比對解碼後值的決策 ＋ 截斷預算調整後重跑（2026-08-23）

同一批 5 筆 Run，第三次判定。兩個改動同批進去：引用回驗改比對解碼後的值的那項決策與 `maxDigestEntry` 2000 → 8000（§15.3 的決策）。
...
### 16.2 歸因：是引用回驗改比對解碼後值的那項決策，不是那個數字
...
引用回驗改比對解碼後值那項決策裡「明確不做的兩件事」寫的就是這個判斷——**支持放寬的證據，在放寬之前就被移除了**。維持現狀的實測代價現在是 **0／39**。
...
## 17. D 輪：引用依內容本身而非宣稱來源判定那項決策的四項行為鏡像之後的重量（2026-08-25，`04` 丙-68）

**這一輪買的是「新尺量出來是多少」，不是「跟舊尺比」。** 本報告開頭的 2026-08-24 補記逐字寫著
§5.1 的 90／90 是歷史證據、用今天的 harness 重跑不會等值重現，而在四項行為鏡像過去之前，
`EVAL-013` 沒有可重跑的回歸閘門（[`04` 丙-58](../../04-backlog-and-handoffs.md)）。四項已於 08-25 補齊，這是補齊之後的第一次量。

**參數**（換其中任何一項就是另一次回歸，同 §4 的規矩）：`judge_model=gpt-5.6-terra`、
`judge_prompt_version=judge-run/v2`、`run_selection=m2_latest_compatibility`、45 筆、`rubric_version=None`、
`note` 逐字為 `2026-08-25 EVAL-013 re-measurement on the mirrored-behavior harness (04 丙-68)`（`results.jsonl` 以此可篩出這 45 列）。
...
| --- | --- | --- |
| 可計分條目 | 90（45 筆 × c1、c2） | 90 |
| **一致（符合率）** | 90 ／ 90 ＝ 100.0% | **70 ／ 90 ＝ 77.8%** |
| **判錯（mismatch）** | 0 | **0** |
| **降為 `undetermined`（不計入判錯）** | 0 | **20** |
| 不計分的 c3 | 43 passed、2 failed | 32 passed、10 undetermined、3 failed |
| 證據引用 | 142 ／ 142 通過回驗 | **141 條：`exact` 135、`not_checked` 6** |
| 成本 | 見 §16 | **$0.8712／45 筆（$0.0194／筆）** |

### 17.2 那 20 條降級有一個完整的解釋，而且它不是 Judge

**20 條全部落在同樣的 10 支 Run 上**（10 支被截斷 × 每支 2 條計分準則 ＝ 20），
降級原因**全部**是 `incomplete_evidence`，而**那 20 條裡模型自己的 `model_result` 全部是 `passed`**。

換句話說：**新尺沒有比舊尺判得差，它是在證據不完整時拒絕採信模型說的 `passed`。**
那正是引用依內容本身而非宣稱來源判定那項決策的意圖——`evidence_required` 不能被一份讀不完的 trace 滿足。
77.8% 與 100% 的差距**完全由截斷解釋**，沒有殘量：45 支裡 35 支未截斷，那 35 支的 70 條計分準則一條都沒有降級。

**因此 D 輪對 Judge 可信度的結論與 §8.1 不衝突**：mismatch 仍然是 0，142 → 141 條引用裡 135 條 `exact`、
6 條 `not_checked`（三態的第三態，見引用依內容本身而非宣稱來源判定那項決策 §4），沒有任何一條引用是回驗失敗。

### 17.3 這一輪指出的下一件事（不在本報告處理）

截斷是 D 輪唯一的降級來源，而截斷預算在 §15 量過、`maxDigestEntry` 於 `04` 丙-47 從 2000 調到 8000。
**10／45 ＝ 22% 的 Run 仍然截斷**，而截斷現在的代價比當時高：以前它讓 Judge 少看一點，
現在它讓那支 Run 的每一條 `evidence_required` 準則直接降級。**這是一個可以量的比率，不是一個判斷**，
所以它屬於下一次調整的輸入，不屬於本輪的結論。
