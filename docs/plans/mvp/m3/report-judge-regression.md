...
## 17. D 輪：ADR-043 的四項行為鏡像之後的重量（2026-08-25，`04` 丙-68）
...
`note` 逐字為 `2026-08-25 EVAL-013 re-measurement on the ADR-043 harness (04 丙-68)`（`results.jsonl` 以此可篩出這 45 列）。
...
那正是 ADR-043 的意圖——`evidence_required` 不能被一份讀不完的 trace 滿足。
...
6 條 `not_checked`（三態的第三態，ADR-043 §4），沒有任何一條引用是回驗失敗。
