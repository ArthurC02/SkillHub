...
- 工作項：[`03` §19](../../03-work-items.md)（`GEN-001`～`011`）——**2026-08-23：11 項中 9 項已勾，2 項 ◐**——`GEN-009` ③④（試跑後的評估判定分布、人看了會不會留著）**要 Sandbox ＋評估管線＋人**；`GEN-008` 畫面做完了但缺「生成前的預估成本」，而那缺的是一個沒人核定過的數字（`04` 丙-50）
...
**同日的對抗式稽核抓到五個缺陷，全部已修**（逐條記在 `03` §19 的行內）。最嚴重的那個兩個方向都沒有症狀：`skills.redistribution` 是建立那一列時決定的、之後永不重算，而 `GEN-007` 的搜尋排除鎖的正是這一欄——**同名碰撞會讓生成物變成搜得到的（包括它自己的作者），或讓使用者自己上傳的東西從此搜不到**。處置是在 `importZip` 拒絕碰撞而不是合併。
