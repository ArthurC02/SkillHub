...
**2026-09-05 狀態更新**：自建 Skill 的三種輸入方式（只用白話描述、上傳流程圖、參考既有 Skill＋描述）皆已立需求 ID 並開工，依 [ADR-066](../../../adr/ADR-066-generation-takes-a-diagram-or-reference-skills-as-input.md)（負責人裁定「自建方式必須三種都徹底實現」，一次凍結例外）——見 [`02:GEN-005`](../../02-specifications-and-acceptance-criteria.md)（流程圖）與 [`02:GEN-006`](../../02-specifications-and-acceptance-criteria.md)（參考既有 Skill）、[`03` §19](../../03-work-items.md) 的 `GEN-012`～`014`。曝光邊界不受影響：兩種新模式一樣掛在下方「⛔ 這個放行沒有授權曝光」那條邊界後面。<br>**同日深化批**：負責人要求再深化，先以工作流做四線稽核（每條發現都經反駁），十五條通過反駁的發現同日修完——三個接縫缺陷（參考截斷含標記超上限、圖旁短標題被 Python 備援拒掉、選擇器的搜尋寫漏斗事件）、前端讀圖失敗無聲、以及 provenance 讀取面（`04` 丙-159 結案）；`03` 的 `GEN-015` 記著逐項與測試。
...
- 工作項：[`03` §19](../../03-work-items.md)（`GEN-001`～`011`）——**◐ 的兩項**（勾選數以 [`03` §19](../../03-work-items.md) 的 checkbox 為準，此處不複述；那個數字曾經散在五份文件裡並三度彼此不符）：`GEN-009` ③④（要 Sandbox ＋評估管線＋人）、`GEN-008`（缺「生成前的預估成本」的**金額**；三個被強制的界 08-24 已上畫面、成本位置印 `尚未定值`；剩下的唯一輸入是單價，要不要釘進設定是 `05` R-10 的事，在 `GENERATE_SKILL_EXPOSED=on` 之前不擋人）。`GEN-003` 於 2026-08-24 隨失敗紀錄讀取面（`GET /skills/generate/failures`，`04` 丙-55 結案）重新勾選
...
**08-24 起進入「建→對抗審→修」迴路（每輪一到兩位獨立審查者），發現數逐輪遞減（29 → 18 → 27 → 9 → 1），全部處理**（`ba1a718`／`fcc9238`／`81ae767`／`0f9e709`／其後——**此清單不再逐輪維護**，以 `fix(m5)` commit 系列為準）。最重的三件：同名生成→生成的碰撞會把第二次生成接成舊 Skill 的 version N、或以重複內容早退而**不計額度**；`packages/api-client-ts` 自 `838b1b4` 起建不起來（生成器對 `oneOf` 含 `Error` 少一個 import，CI 紅了一天沒人看到）；成功後 invalidate 錯的 query key 讓首頁**每次成功多記一筆 `search_performed`**——正是 ⛔ 邊界在保護的數字。M5 新增的檔案：`apps/llm/tests/test_strict_schemas.py`、`db/migrations/0038`、`apps/web/src/components/GeneratedNotice.tsx`、`generateFailureSentence.ts`、`GET /skills/generate/failures`（handler 在 `skill/admission/http.go`）。
...
**第二輪最嚴重的三個都不是邏輯錯誤，是缺席的東西**：①**整條生成請求在 Go 這一側沒有任何 deadline**——`llmclient` 刻意不帶自己的 timeout（它的註解逐條列出每一個帶 deadline 的呼叫端），而生成就是那個忘了的呼叫端，於是一個卡住的 `apps/llm` 會把 goroutine 與連線釘住到瀏覽器放棄為止；②**內部服務的回應沒有上限**——「內部」不等於「可信」，而 `02:GEN-003` 逐字這樣寫過；③**沒有任何東西限制生成的次數**，而它們走的是**共用的閘道 key**——把餘額用完會連 `/embed`、索引增強與每一次 LLM Judge 一起停掉。三個都已修，第三個當時只修到「一個工作區同時一次」。~~真正的速率限制記在 `04` 丙-54，是 `GENERATE_SKILL_EXPOSED=on` 的前置。~~ **（2026-08-25 訂正：這句寫下的當時就指錯了地方，而且從此沒有跟上程式。）** 丙-54 於 08-24 結案時把 `limited()` 掛在 `02:NFR-001` 第 5 條點名的三條路由上（未登入搜尋與兩條 import），**生成端點不在那三條裡面**——token bucket 只會被它包住的 handler 扣，所以一個只打 `POST /skills/generate` 的迴圈**一格都不扣**，而那正是要防的形態。真正把限制器接到生成路由上的是 `403b385`（2026-08-25）：[`router.go:102`](../../../../apps/platform/internal/entrypoint/api/apiserver/router.go) 的 `limited(d, auth.RequireSession(auth.RequireInvited(d.Importer.Generate)))`，`limited()` 在兩層 auth **外面**，所以認證路徑本身也擋；`GET /skills/generate/failures` 刻意不包（工作區範圍的讀、一次查詢、零模型呼叫）。**有牙齒的證據**：`apps/platform/internal/entrypoint/api/apiserver/generate_integration_test.go:837` 的 `TestGenerationIsRateLimitedWhenALimiterIsConfigured`，對 burst 2 連打五次 POST，沒看到帶 `Retry-After` 的 429 就 fail。**所以現在的事實是：生成路由已有速率限制且被測試釘住**，而三份文件對同一件事曾經給出三種說法（`04` 丙-54 說已結案、本節說還開著、`.env.example` 的理由句是錯的）。**後續的 `4db132d` 只訂正了 `.env.example` 那一句**，並在 `04` 加了一列與此無關的丙-62——這一段當時沒有人回來改。
...
~~**這不表示 M5 開工了。**~~ **（2026-08-23 同日失效：三個啟動條件全部暫時放行，見下方 §啟動條件；本節保留當時的判斷做為紀錄。）** 原文：`01` §10 的啟動條件三項一項都還沒成立：MVP 封測尚未結束、漏斗第一段沒有讀數、`GEN-009` 的另外三分之二（試跑後的評估判定分布、人的接受率）沒有跑。
...
| 3 | ~~`GEN-009` 生成品質基線~~ **已放行** | **①②與 (甲)(乙) 已完成** → `report-generate-spike.md`（A 輪）、`report-generate-baseline.md`（B 輪＋mini）。~~**剩 ③④**——要 Sandbox、評估管線與人~~ **2026-08-28：③ 完成**（D 輪，20／20 生成＋試跑＋判定，報告 §9、原始資料 `gen009-round-d/`）。~~**剩 ④，它要人**~~ **2026-08-28：④ 也完成，整列勾選**——負責人逐份看過二十份，**留 15、不確定 3、不留 1（分母 19）**，報告 §10。**交叉對照才是重點**：機器的 `met` 12／12、`not_met` 1／1 與人一致，**中間帶 7／20 全是分歧**。 |
...
| `gen-modes-batch/` | 2026-09-05 收尾：兩種新模式各 20 段的語料、畫圖腳本與 harness 跑法；**2026-09-06 跑了**（流程圖 20／20、關鍵詞 19／20 全到；參考 20／20、標記句 0／60 被抄），輸出在 `run-2026-09-06/`，見 [報告 §8](report-generate-modes.md) |
| `creation-measure/` | 2026-09-06：互動創作 15 場（三入口各 5）＋同 15 題單次對照的 harness、跑法與七次實跑輸出（a→b→c 各修一次；d→e 接上 Run 階段，`met` 第一次有數字；f→g 沿丙-175 調提示，`met` 2→5／13；h→i 補五個吃步數的迴圈，乾淨 mini 基線 4／14；j 只換寫 Skill 的模型為旗艦，0／14 但 review 相 14／14 改稿；裁定後 k 改成一輪修訂內計 `met`，文字＋參考 4／10；l 換旗艦跑 Skill，2／10——執行者不是槓桿；m→o 照「每輪試跑、給建議、直到可接受」改 review 相與 harness，o 文字＋參考三輪內 7／10 過門檻；p 加試跑後先問人與經同意的連網；q 五題連網語料，語意檢索 10／10 vs 詞彙 0／10；goldenset 60 題重放：詞彙 F1 0.02、向量 0.61、截斷 0.55 後 0.77；hybrid 在名稱＋特定詞兩組補上向量的洞，三組合計 0.877；r 讓模型自己搜參考）；結果與 R-45 門檻對照見 [report.md](creation-measure/report.md) |
