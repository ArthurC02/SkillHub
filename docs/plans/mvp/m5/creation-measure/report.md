...
## 14. Re-Use 三關卡：公開搜尋的覆蓋腿、第一則訊息查目錄、保存前查重

**2026-09-06 深夜，負責人授權代理依最佳實務裁定（`05` R-48／R-49／R-50，`04` 丙-178）。** §13 把 bigram 詞彙腿與 hybrid 規則做進了創作工具；這一節把同一個判斷延伸到兩個地方——公開搜尋今天用的是英文 tsvector，同樣看不見中文；而檢索流程本身（§13.4）在會話裡不會自己被觸發，得有人在正確的時機先查一次。三關卡各自封一個「本來會被漏掉」的縫：目錄裡其實已經有答案，但公開搜尋找不到、會話一開始沒有人問、保存前也沒有人再看一眼。

### 14.1 公開規則離線重放：為什麼是「覆蓋腿」不是「等權融合」

golden set 60 題（沿用 §12／§13 的三個查詢集）在**公開搜尋的 SQL**（`PublicHybridSearchSkills`）上重放，比較三個版本：

| 版本 | Golden Top-1／Top-3／recall@5 | 干擾拒答 | 名稱 Top-1 | 特定詞 Top-1／Top-3／recall@5 | 判斷 |
| --- | --- | --- | --- | --- | --- |
| **現況（無 bigram，英文 tsv）** | 44/48／48/48／48/48 | 12/12 | 30/31 | 7/25／10/25／11/25 | 詞彙腿對中文近乎無用，等同單靠向量（同 §11／§12 量到的 F1 0.02） |
| **A：等權 RRF 融合（未在此語料重跑）** | — | — | — | — | **不採用**：援引 §13.2 同語料同分詞的等價實驗——RRF 把向量排名稀釋，golden F1 由 0.774 掉到 0.54，比單腿還差；沒有理由預期公開 SQL 會有不同結果 |
| **B：覆蓋腿（查詢全部 token AND 命中不受 0.75 截斷、排在向量命中前；名稱完全命中再置頂）——已採用** | **44/48／48/48／48/48** | **12/12** | **31/31** | **23/25／25/25／25/25** | golden 與干擾三個數字**完全不變**（不需依 `gate-test §3.2` 分開統計）；名稱與特定詞兩組拿到新增召回 |

選 B 不選 A 的理由與創作工具那次（§13.2）相同：這裡的訊號是「查詢的每一個詞都出現在這個 Skill 裡」，那是一個**集合事實**，把它拿去跟一個連續分數做加權平均，等於把一個確定的訊號稀釋成一個不確定的訊號。B 型的覆蓋規則不做這種稀釋——它只回答「要不要把這一列提前看見」，向量距離仍然決定其餘的每一個名次（ADR-013 定案調整 5 記了這件事跟「排序權在向量」不衝突的理由）。

輸出檔：[`search-f1/results-public-rule-2026-09-06.txt`](search-f1/results-public-rule-2026-09-06.txt)。既有列由 `cmd/reindex` 的 `BackfillBigram` 回填，不寫資料遷移。

### 14.2 三關卡的形狀與測試

三關卡共用同一個判斷——**目錄裡可能已經有答案，讓人決定要不要用它，而不是讓系統自己猜**——落在三個不同的時機：

1. **公開搜尋的覆蓋腿**（14.1）：任何人查詢，覆蓋列提前被看見。`TestPublicSearchKeepsACoveredLexicalHitPastTheCutoffAndPinsTheExactName` 釘住兩件事——距離 > 0.75 但覆蓋全部 token 的列仍然出現且排在向量命中前，以及名稱完全命中排在覆蓋列之上；拿掉覆蓋規則或拿掉置頂任一半，測試就紅。
2. **會話第一則訊息查目錄**：`05` R-49 的三選一（`adopt_reference`／`confirm_references`／`decline_references`）。`TestCreationFirstMessageCatalogueCheckOffersAdoptKeepOrDecline` 釘住：第一則訊息命中目錄時會話停在 `confirm_references` 而不進模型、`adopt_reference` 帶著恰一個 reference 時把候選變成 fork 並以 `state=saved` 收尾、`decline_references` 清空參考並讓會話繼續進模型；拿掉命中判斷、拿掉三選一任何一支，或讓 adopt 意外消耗模型呼叫，測試就紅。
3. **保存前查重**：`05` R-50 的 `confirm_duplicate`。`TestCreationMaterializeHoldsForADuplicateUntilAdoptedOrConfirmed` 釘住：草稿 name／description 命中既有 Skill（距離 ≤ 0.55）時 `materialize`／`finalize` 不執行、落 `pending_action=confirm_duplicate`；`confirm_duplicate` 帶正確 `content_hash` 時執行原本被扣住的命令並標 `duplicate_acknowledged=true`；草稿改變後重查會清掉舊的 `duplicates`。拿掉查重呼叫、拿掉命令扣留、或讓草稿改變後沿用舊判定，測試就紅。

**四處突變都紅過**（覆蓋腿的距離判準、覆蓋腿的排序位置、adopt 的 fork 路徑、confirm_duplicate 的命令扣留）——綠燈證明測試存在，這四次紅燈證明它們真的守著對應的規則，不是空跑一遍就過。

### 14.3 run s：三關卡接上後跑一次（mini／mini、三輪、`CREATION_MEASURE_SEARCH=1`）

| | run r | **run s（首則訊息查目錄、查重守門、參考先入目錄）** | 門檻 |
| --- | --- | --- | --- |
| 文字：第一次試跑 `met`／三輪內 | 4／5 → 4／5（R07 空 brief） | 3／5 → 3／5 | — |
| 流程圖組（另計） | 2／5 → 2／5 | 1／5 → 1／5（D04 停在草稿） | — |
| 參考：第一次試跑 `met`／三輪內 | — | 3／5 → 3／5 | — |
| **文字＋參考三輪內 `met`** | 6／9 | **6／10** | ≥ 6／10 |
| 參考組：Go 首則訊息查目錄找到匯入的參考（`search_hit`） | 0／4（模型沒搜） | **0／5** | — |
| 文字／流程圖組被目錄命中而扣住（`catalog_offers`） | — | 0／10 | — |
| 每場會話成本中位（文字／流程圖／參考） | $0.017 | $0.015／$0.029／$0.017 | ≤ $0.50 |

輸出在 `run-2026-09-06-s/`。**`search_hit` 0／5 不是檢索的錯，是 harness 的錯**：匯入參考的輔助函式（`importFiles`）自己建一個沒有 LLM 的 ingest service，所以匯進去的參考在 `search_documents` 是 `pending`——沒有白話摘要、沒有向量。首則訊息的目錄查詢只採語意答案（降級的純詞彙答案不算，整句任務描述也不可能被 bigram 全覆蓋），於是它永遠看不到那份參考；run r 的 0／4 也有同一個成因在裡面（那時還在模型自己搜的前提下）。修法：`importFilesEnriched` 在上傳時就做 index-time enrichment（含 embedding），參考題先匯入、標成目錄工作區，再開會話。**run t 重量**，見 §14.4。

`catalog_offers` 0／10 同理：測試庫的目錄裡只有歷次 harness 匯入的參考，全是 `pending`，沒有向量可以命中。三選一的實際分布與「採用是否比整場走完更快」因此還沒有樣本。

### 14.4 run t：參考先增強再開會話（mini／mini、三輪）

| | run s | **run t（`importFilesEnriched`：參考在開會話前已有向量）** |
| --- | --- | --- |
| 文字：第一次試跑 `met`／三輪內 | 3／5 → 3／5 | 2／5 → 2／5 |
| 流程圖組（另計） | 1／5 → 1／5 | 1／5 → 1／5 |
| 參考：第一次試跑 `met`／三輪內 | 3／5 → 3／5 | 0／5（兩場沒跑到，見下） |
| **參考組：Go 首則訊息查目錄找到匯入的參考（`search_hit`）** | 0／5 | **2／5（R08、R09）** |
| 文字／流程圖被目錄命中而扣住（`catalog_offers`） | 0／10 | 0／10 |
| 每場成本中位（文字／流程圖／參考） | $0.015／$0.029／$0.017 | $0.014／$0.021／$0.016 |

輸出在 `run-2026-09-06-t/`。**R-49 的流程第一次在會話裡量到了**：五場參考題裡兩場，Go 在任何模型呼叫之前就把匯入的參考端到人面前（`catalog_checked`＋`confirm_references`），harness 依「以它為參考」往下走；另外三場的參考在 0.55 以內找不到——它們的任務描述與參考 Skill 的索引文本語意距離超過創作工具的截斷（這是資料端的問題，見 §15 的 F1 迴圈，不是流程）。
...
### 14.6 run v：規則變更後（創作搜尋＝公開規則）——跑到 R08 中斷

確認被拒不再致命之後，run v 跑了 12 場：文字第一次試跑 `met` 3／5、三輪內 3／5；流程圖 0／5（D02 停在 `needs_reupload`）；**參考組跑到的兩場（R06、R07）Go 的首則訊息查目錄都找到了匯入的參考，兩場都 `met`**（`search_hit` 2／2、calls 3）。R08 在 materialize 撞到「工作區已有同名 Skill」的 422——harness 每個任務固定同一個使用者，run t／u 留下的候選就在同一個工作區裡，模型又取了同一個名字。修法：每次量測一個新工作區（`measureRunNonce`）。輸出在 `run-2026-09-07-v/`（12 場）。**run w 重跑**，見 §14.7。

### 14.7 run w：每次量測獨立工作區——跑到 R09 中斷

13 場：文字第一次試跑 `met` 2／5（三輪內 2／5）；流程圖 0／5；**參考組跑到的三場 Go 的首則訊息查目錄全部找到匯入的參考（`search_hit` 3／3）**——R06 `met`、R07 在第二步 `failed`（模型端 502）、R08 撞到查重守門（`duplicate_offers` 1，harness 仍然建立）之後試跑 partially_met。R09 又是 422「同名」——這次不是跨 run 的殘留，而是**模型把草稿取了和參考 Skill 一樣的名字**，Go 拒絕同名是對的（GEN-010 一個工作區一個名字）；harness 的保存輔助函式改成被拒只算那一場。輸出在 `run-2026-09-07-w/`。**run x 重跑**（§14.8）。

三次跑下來 R-49 在會話裡的證據：參考先入目錄之後，Go 的首則訊息查目錄在 **7／8 場**找到了匯入的參考（t 2／5 是 v6 增強＋0.55 截斷；v、w 是 v7 增強＋0.75 截斷各 2／2、3／3）——資料端（§15）與規則端一起讓這件事從 0 變成幾乎每場。

### 14.8 run x：完整十五場（mini／mini、三輪、v7 增強、創作搜尋＝公開規則）

| | run r（09-06） | **run x（09-07）** | 門檻 |
| --- | --- | --- | --- |
| 文字：第一次試跑 `met`／三輪內 | 4／9（含參考） | 4／5 → 4／5 | — |
| 參考：第一次試跑 `met`／三輪內 | — | 3／5 → 3／5 | — |
| **文字＋參考三輪內 `met`** | 6／9 | **7／10** | ≥ 6／10 |
| 流程圖組（另計）：第一次／三輪內 | 2／5 → 2／5 | 0／5 → 1／5（D03 第二輪達成） | — |
| **參考組：Go 首則訊息查目錄找到匯入的參考（`search_hit`）** | 0／4 | **5／5** | — |
| 查重守門發動（`duplicate_offers`） | — | 1／5（R09） | — |
| 每場成本中位 | $0.017 | $0.015（p50 4.6 s／呼叫、p95 7.1 s） | ≤ $0.50 |

輸出在 `run-2026-09-07-x/`。**R-49 在會話裡成立了**：五場參考題，Go 在任何模型呼叫之前都把匯入的參考端到人面前；三場第一次試跑就 `met`（calls 3～4）。R09 是唯一沒跑到試跑的：查重守門發動（草稿與參考幾乎一樣）、harness「仍然建立」之後，Go 以「同名」拒絕保存——模型把草稿取了參考的名字，而 harness 把參考匯進了同一個工作區；真實使用時參考在目錄工作區、草稿在使用者工作區，不會同名相撞，但**「仍然建立」之後模型該改名**這件事值得寫進提示（記在 §15.6）。

t～x 四次合計：參考先有向量、進了目錄之後，首則訊息查目錄 **12／13 場找到**（t 2／5 是 v6 增強＋0.55 截斷的最後一次）。
...
再加兩條例句規則——點名執行環境（.NET 服務、Python 腳本）、以及「只描述輸出的形狀（一份簡報、一個可分享的網頁、一份分節的報告）而不提 Skill」——重做 31 份（`corpus_enriched_v7_1/`）之後 **all F1 0.955→0.937**（golden 0.914→0.878）。多出來的句子把近義對的文件拉到彼此的查詢上（`pptx`、`deck-publisher`、`report-designer` 同時都在說「一份簡報」）。提示退回 v7，語料與結果檔保留為證據。教訓：例句要多樣，但**每一句都得是這份文件獨有的講法**；泛用的「形狀」句子是噪音。

### 15.6 還沒做的與要裁的

- **「仍然建立」之後的同名**（run x R09）：查重守門之後若草稿與現有 Skill 同名，保存會被 GEN-010 的同名規則拒絕；提示應在 `confirm_duplicate` 之後的下一輪要求改名，或 Go 在扣住時就把同名寫進觀察。小事，待做。
