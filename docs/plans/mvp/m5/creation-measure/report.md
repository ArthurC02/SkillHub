...
- 旁證一件：Judge（terra）有一場摘要寫成簡體（i 的 R05、j 的 R05 都是同一題）。不影響判定，記在[評估判定與 Judge 信任邊界](../../../../adr/README.md#評估判定與-judge-信任邊界)的待決策旁邊。
...
十二題干擾在所有截斷都全數拒絕。**落地**：創作工具自己的截斷 `CreationMaxDistance = 0.55`（公開搜尋的 0.75 不動，那是[意圖搜尋](../../../../adr/README.md#意圖搜尋)的裁定）；`search_catalog` 與 `search_knowledge` 在 Worker 都走語意優先、詞彙只當沒有 embedding 服務時的退路——模型不再二選一；embedding 費用（`/embed` 回的 usage）記到會話的 `spent_usd`。結果最多三筆、依距離排序，人確認。
...
選 B 不選 A 的理由與創作工具那次（§13.2）相同：這裡的訊號是「查詢的每一個詞都出現在這個 Skill 裡」，那是一個**集合事實**，把它拿去跟一個連續分數做加權平均，等於把一個確定的訊號稀釋成一個不確定的訊號。B 型的覆蓋規則不做這種稀釋——它只回答「要不要把這一列提前看見」，向量距離仍然決定其餘的每一個名次（[意圖搜尋](../../../../adr/README.md#意圖搜尋)記了這件事跟「排序權在向量」不衝突的理由）。
...
- 負責人的順序「資安確認→LLM 抽 metadata→入庫」：匯入路徑今天就是這個順序（驗證與掃描擋在版本之前、增強在同一交易），但**增強失敗的文件仍會入庫成 `pending`**（[意圖搜尋](../../../../adr/README.md#意圖搜尋)刻意的可用性選擇），而**創作出來的候選完全不抽 metadata**（GEN-007）。兩件都要裁（`05` R-52）。
- golden 的天花板：近義對第二名。要再往上只能靠查詢改寫（[意圖搜尋](../../../../adr/README.md#意圖搜尋)那條，延遲預算未量）或人工標註更嚴（單人標註，§6 已記）。
...
## 17. 流程圖「先描述、再拆解、不懂必問」第一次走完（run y～ab，2026-09-26）

設定同 run x：寫與跑都是 mini、三輪、`CREATION_MEASURE_SEARCH=1`。harness 在「回答不確定處」這一關扮演看得懂自己那張圖的人，每一條都以語料裡那張圖的節點清單作答。run ab 只跑流程圖組（`CREATION_MEASURE_ONLY=diagram`）。四次的閘道實際花費合計 US$6.66（863 次呼叫，含試跑的 Agent 與判定）。

### 17.1 流程圖組：三次全滅之後，五場全部走到試跑

| | run y（v17） | run z（v18） | run aa（v19） | **run ab（v19＋最後一批修正）** |
| --- | --- | --- | --- | --- |
| 走到草稿與試跑 | 0／5 | 0／5 | 0／5 | **5／5** |
| 三輪內 `met` | — | — | — | **2／5**（D01 第一輪、D05 第二輪） |
| 其餘三場 | 第二步被拒 | 第三、四步被拒 | 最後一步被拒 | `partially_met`（D04 修稿後的步驟沒在 harness 的步數內結束） |

**這條流程在 9/24 落地時只用假模型測過，而真實路徑上連一場都走不完。** 三次失敗各卡在不同關卡，全是同一類：Go 與 Python 各自照規則做，合起來就錯。

- Python 在模型沒給描述時，會回填已確認的描述；Go 則把任何非空描述當成新的描述，只准在上傳那一步出現（run y，五場都卡在拆解）。
- 沒有分支的流程圖，空清單在 Go 送出請求時被序列化成 `null`，Python 的 schema 回 422。儲存那一層修了（run z）還不夠，組請求那一層又做出一次 nil（run aa）；最後改在 llmclient 那個所有請求都會經過的地方補成陣列。
- 圖確認之後，提需求那一相的指示仍寫「上傳的圖要先描述」，模型就又要求確認一次圖；交草稿時，模型也會把已記錄的拆解結果或舊流程的理解欄位一起回傳。在 Go 眼中這些都是「新的」。現在只有帶新圖的那一步收描述、只有拆解那一步收拆解結果，其餘一律丟掉回聲。

每一件修正都有一條測試、都拿掉一次看過它變紅。這三批 commit 是 aa2595b、1ba8f8a、2111e3f。

### 17.2 文字＋參考：四次讀數在門檻上下擺動

| | run x（09-07） | run y | run z | run aa | 門檻 |
| --- | --- | --- | --- | --- | --- |
| 一輪修訂內 `met` | 7／10 | 6／10 | 5／10（第一輪 4） | 4／10 | ≥ 6／10 |
| 每場成本中位 | $0.015 | $0.013 | $0.014 | $0.013 | ≤ $0.50 |
| 每次呼叫 p50／p95 | 4.6／7.1 s | 3.8／6.0 s | 4.5／7.0 s | 4.0／5.9 s | 60／90 s |

v18 與 v19 對文字與參考路徑只多了一句「使用者沒寫字時，用圖上的語言回覆」，其餘改動都在流程圖相位。所以 y→z→aa 逐次少一場，比較像 n＝10 的取樣起伏，而不是提示退步。但這件事沒有被證明，只是沒有證據支持另一種解釋。**x 的 7／10 只是單次過門檻，不代表穩定**：同一套設定四次讀數落在 4～7 之間。

### 17.3 現在站在哪

- 流程圖組有了自己的數字：三輪內 2／5。
- 文字＋參考還沒有穩定地站在門檻上。要判斷「穩定」，得先定每次取幾個樣本、用哪個統計量。這是 `05` R-45 的延伸，不是再跑一次就能回答的。
- `kept`（真人願不願意採用）仍然沒有人讀。
- 輸出在 `run-2026-09-26-y/`、`-z/`、`-aa/`、`-ab/`。

## 18. 對抗題：專找失敗，而不是量平均（2026-09-27）

原本的 20 題量的是平均。這一次另出 10 題（[`corpus-adversarial.json`](corpus-adversarial.json)），每題對準一種失敗方式：分支與邊界、材料在外部、試跑環境做不到的自動化、需求模糊、語言漂移、要算對的數字、自相矛盾的需求。寫 Skill 的作者換了三個等級（mini、Luna、Sol），跑 Skill 的一律是 mini，判定一律是 `gpt-5.6-terra`。閘道實際花費合計 US$6.80。

| | run adv-a（mini，v21） | run adv-b（mini，v22） | run adv-c（Luna，v22） | run adv-d（Sol，v23） |
| --- | --- | --- | --- | --- |
| 一輪修訂內 `met` | 2／10 | 8／10 | 7／10 | 2／2（第三題撞上會話預算，整批中止） |
| 第一次試跑就 `met` | 2 | 7 | 7 | 2 |
| 每場成本中位 | $0.016 | $0.016 | $0.007 | $0.11～0.15；A03 走 11 步花 $0.95 |

v21 與 v22 的第一次試跑之前，程式、作者與題目完全相同，第一次就過的場次卻從 2 變 7。**n＝10 的單次讀數，光是取樣起伏就能差 5 場**，比任何一次改動想量出的效果都大。

### 18.1 量測本身的洞（這一批已修）

- **通過的場次沒有留下證據**。評估只在「沒過、要改稿」時貼回對話，第一次就過的場次連條件與輸出都沒有存；測試資料庫每次重置，事後查不回來。現在每一輪都存 `<id>-trial-r<N>.json`：樣本、條件、Skill 的最後輸出、產出檔名、完整評估，讀取失敗也寫進去。
- **只改樣本的修法從來沒被重跑**。harness 只比對草稿雜湊；模型照「驗不到就補樣本」只改了樣本時，被當成「沒改稿」而放棄（adv-a 的 A01：樣本已補成 499、500、999、1000 四筆，卻沒有第二次試跑）。現在草稿、樣本、條件任一個變了都算改稿。
- **扮演使用者的回覆邀請改條件**。舊回覆是「條件或範例輸入驗不到的，就改條件或範例輸入」，第二、三輪的 `met` 可能是在改過的條件上判的。回覆改成只准補樣本，`met_on_changed_criteria` 另計。
- **502 的原因沒有留下來**。apps/llm 有六種原因都回 502，Go 端只記「returned 502」。現在帶上 apps/llm 自己寫的固定原因；422 會回顯請求內容，所以不帶。

### 18.2 分數高，是因為考題被出簡單了

逐場讀 adv-b 與 adv-c 的通過場次，多數通過的是作者自己出的簡單版本：

- **核心能力沒被考**：A03（客訴分類統計）兩個作者都出「每類剛好一件」的信，歸類計數從來沒被考；A09（加班時數）mini 把輸入換成每天已算好的加班時數，真正要算的上下班時間不見了；A10（3000 字壓成 50 字）原文只有約 100 字，矛盾沒出現；A06（模糊的自我介紹）樣本把姓名、科系、專長都給齊了。
- **條件縮成只考這一份樣本**：Luna 的 A01 三條條件都寫成「對輸入 780 元的試跑…」，Sol 的 A01 只考 500 元；499、999、1000 三個邊界沒有一條。這是「條件必須在這份樣本上可判」照字面執行的結果：mini 想把分支寫進條件，驗不到而判「無法判定」；Luna 與 Sol 把考試縮到樣本那一筆，然後通過。

所以 8／10 與 7／10 大多證明的是「簡單版本做得到」，不是使用者的需求被滿足。

### 18.3 一份樣本考不完的任務

A01（依金額回一句運費）三個作者都只考到一段。這類任務一次處理一筆，把四個金額塞進同一份樣本，又會違反「只回一句」的條件：**一份樣本在結構上考不完**，換作者也一樣。能逐筆列表的任務就沒有這個問題：A02（請假審核）在 mini 第二輪、Sol 第一輪把每個分支各放一筆，逐筆判斷全對；Sol 還刻意放了「剛好提前 3 天」「只提前 2 天」這類邊界值。

### 18.4 判定模型的問題

- **判反了**：adv-a 的 A09 第一輪算出 2.5、1.5、0 小時（每天 8 小時工時下是對的），因為「自行假設 8 小時」被判不過；改稿後輸出變成整排「無法計算／not given」，五條全過。
- **條件式條件的觸發情境沒出現也給過**：A10「當原文較長時仍需壓到 50 字」在 101 字原文上給過，A06「當背景資訊不足時」在資訊齊全的樣本上給過；A01 遇到同樣情形卻給「無法判定」。
- **不可能的資料沒被發現**：Luna 的 A09 樣本裡有人一天實際工時 31、32 小時。
- **少判一條**：adv-a 的 A06 有一條條件沒有判定結果（「the judge returned no verdict for this criterion」）。

### 18.5 產品本身的缺陷

- **改稿後原地打轉**：aa 的 R10、adv-a 的 A04、A08、A10 在改稿後反覆「驗證草稿→小改→請你接受」6～7 次，沒有回到試跑，最後用完步數，只對使用者說「目前無法驗證草稿，請補充需求或稍後再試」。
- **compose 提示自相矛盾**：同一段指示要 Skill「選合理的預設並寫出來，不要反問」，又要求逐字寫入「只用輸入給的內容、不加任何假設、沒給的寫 not given」。adv-a 的 A09、z／aa 的 R10、A05 的「not given」空殼都出自後者。
- **Skill 宣稱自己做得到做不到的事**：adv-a 的 A04 條件要求「表達這是自動化流程」，改稿後 Skill 寫成「每週五下午自動執行、無需手動寄送」；Skill 不能排程也不能寄信。Luna 在同一題則把輸出標成「待發送／待發布」。
- **作者模型的代價**：Luna 每場比 mini 便宜，但十場裡兩場在改稿那一步 502 整場失敗；Sol 一場 $0.11～$0.95，以目前每場 US$1 的會話預算跑不完材料多的題目。
- **harness 在一場撞上預算時整批中止**，其餘題目沒有數字（adv-d）。

### 18.6 現在站在哪

- 標準題的 7、6、5、4 與對抗題的 2、8、7，都不能拿來判斷有沒有到 9 成：前者的考題會被出簡單，後者顯示單次起伏可達 5 場。
- 要讓分數有意義，得先處理兩件事：一個 Test Case 能帶多份樣本，而且條件寫的是需求本身，不是這份樣本；判定模型對「情境沒出現」一律給「無法判定」。
- 輸出在 `run-2026-09-27-adv-a/`～`-adv-d/`。
