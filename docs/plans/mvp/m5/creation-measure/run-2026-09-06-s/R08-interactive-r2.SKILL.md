...
2. Use only what the input contains — never add a date, name, assumption, step or branch the input does not give.
3. Identify the request date and purchase date from the input if they are given.
4. Compare the dates exactly as provided and determine whether the request is within 7 days of the purchase date.
5. If the input clearly shows the request is within 7 days, output the Chinese standard reply for the within-7-days case.
6. If the input clearly shows the request is over 7 days, output the Chinese standard reply for the over-7-days case.
7. If the input does not provide enough information to determine the result, say that the result cannot be determined from the given information.
8. Output the result in Chinese.
9. Include the final reply email text directly in the output so it can be used as-is.

## Output shape
- State whether it is within 7 days or over 7 days, or state that the result cannot be determined.
