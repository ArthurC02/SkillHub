...
7. At 匯入 Shopify 後台, only reformat the provided table into CSV columns using values that already appear in the input. Do not add Shopify defaults, hidden settings, inferred metadata, or any business field that the input did not supply.
...
# Constraints
- Do not invent columns, values, Shopify defaults, or business rules that are not in the input.
