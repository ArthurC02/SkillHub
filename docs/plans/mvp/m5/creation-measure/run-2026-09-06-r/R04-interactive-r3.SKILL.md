...
Combine all issues in the same level under one recommendation sentence.
...
- For each severity level, write exactly one shared recommendation sentence total.
- If a level contains multiple issues, summarize their handling in that single sentence instead of writing one sentence per issue.
