...
5. If the confirmed input is incomplete, state that only the confirmed content can be used and stop there.
...
- Include no extra workflow branches or unconfirmed implementation details.
- If no tools are required, say so explicitly in the skill metadata or prose.
