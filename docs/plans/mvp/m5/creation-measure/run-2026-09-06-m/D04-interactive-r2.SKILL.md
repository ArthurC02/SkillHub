...
2. 篩出 ERROR 行。
3. 判斷錯誤是否超過 10 筆。
4. 當 ERROR 總數超過 10 筆時，建立 Jira 問題單。
5. 當 ERROR 總數超過 10 筆時，通知值班工程師。
6. 寫入每日摘要。
...
若 ERROR 總數不超過 10 筆，仍保留「Jira 問題單」與「通知值班工程師」小標題，但內容只寫 `不適用`，不要寫任何建立或通知動作；其他小標題則依輸入內容照實輸出，必要時寫 `not given`。
