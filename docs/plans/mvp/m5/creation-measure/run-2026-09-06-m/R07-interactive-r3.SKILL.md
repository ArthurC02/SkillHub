...
- 不改動非 docstring 的程式碼。
- 若輸入沒有 docstring，就產生一個新的 Google 風格 docstring，且內容只使用輸入中可直接確認的函式名稱、參數與回傳資訊。
- 針對沒有 docstring 的函式，輸出仍需提供一個可直接貼回程式的完整 Google 風格 docstring，至少包含摘要與可由輸入推知的區段。
- 若輸入已有完整且符合 Google 風格的 docstring，就原樣保留其段落順序、區段名稱與文字內容；只在使用者明確要求補寫時，才加入新內容。
- 當既有 docstring 已完整且符合 Google 風格時，輸出必須保持原有段落順序、區段名稱與文字內容，不新增、不刪除也不重寫任何段落。
