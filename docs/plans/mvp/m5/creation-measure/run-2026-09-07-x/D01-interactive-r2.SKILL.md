...
8. 未明示回圈/後續流向
   - If the input says to return for correction, resubmission, or another action but does not specify what happens next, list that missing follow-up under uncertainties as "not given" instead of continuing the flow.

## Output shape

Return exactly four sections with these headings, in this order: ## nodes, ## conditions, ## branches, ## uncertainties. Do not replace them with other headings or a narrative summary.
