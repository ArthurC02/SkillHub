...
7. If the input gives no detail for a field, write `not given`.
8. Preserve the meaning of the input and do not invent behavior, types, names, or details.
9. Output the completed docstring itself only.
...
- If the input is silent about a detail needed for the docstring, write `not given` rather than guessing.
