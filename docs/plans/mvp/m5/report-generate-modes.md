...
[從描述生成 Skill](../../../adr/README.md#從描述生成-skill) 落地當天的付費驗證。三種模式（`02:GEN-001` 純文字、`02:GEN-005` 只有流程圖、`02:GEN-006` 描述＋參考 Skill）各對真實 LiteLLM 閘道跑一次，走的是 Go 的 `ingest.Service.GenerateSkill` → `apps/llm` → 閘道 → 打包 → `skillpkg.Validate` → 建版這整條路，沒有任何一段是替身。這份報告只回答兩個問題：**路通不通**，以及**模型有沒有真的讀那張圖／那份參考**。品質不在此判定——三份產出都沒有被任何人試跑過，`GEN-004` 的「尚未試跑」狀態逐字適用。
...
**帶圖片的那一次比純文字便宜**——圖片的 token 遠少於模型自己寫出來的文字，成本主要在輸出端；`GenerateSkill.tsx` 的成本區間（$0.003～$0.03）三次都落在裡面，[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)「估計值可能不再準確」那條顧慮這一批沒有兌現，但三筆不構成分布。
...
**參考的判定**：借到的是**形狀**——「步驟」與「輸出格式」兩個段落標題、編號步驟、固定的 Markdown 輸出格式；沒有一句是參考本文的複述，「三欄分組」「每欄最多五條」「下週要追的事」這些參考特有的內容一個都沒有出現。這正是[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 2 說的「參考是模型讀過的材料，不是套件裡的位元組」在一次實跑上的樣子。單一樣本，不外推。
...
## 7. 分布的 harness（2026-09-05 收尾；隔日跑了，見 §8）
...
## 8. 跑了：兩種模式各 20 段（2026-09-06，負責人「可以花錢」）

`TestTheTwoNewerModesTwentyTimesEach` 對真實閘道（`gpt-5.4-mini`）跑完，逐項輸出在 [gen-modes-batch/run-2026-09-06/](gen-modes-batch/run-2026-09-06/)（`results.json` 與 40 份 `SKILL.md`）。

| 模式 | 生成／20 | 阻擋 | provenance 寫入 | 成本中位（每次） | 總成本 |
| --- | --- | --- | --- | --- | --- |
| 只有流程圖 | **20** | 0 | 20 | US$0.0033 | US$0.074 |
| 描述＋一個參考 Skill | **20** | 0 | 20 | US$0.0042 | US$0.082 |

- **流程圖有沒有被讀進去**：每張圖每個節點帶一個「照圖寫一定會出現」的關鍵詞，20 張裡 **19 張全部關鍵詞都在**，1 張（D17，一條直線的 Podcast 流程）4／5——生成物裡完全沒有「寫 100 字節目簡介」這一節（連「簡介」二字都不在）——五節漏了一節，是真的漏讀，不是換寫法。20 張裡的 1 張；§6 第一條問的分布，答案是「19／20 全讀進去、1／20 漏一節」。這是 §6 第一條問的分布；三次各一筆變成 20 筆之後，答案是「讀得進去」。
- **參考有沒有被照抄**：每份參考帶 3 句只有抄了才會出現的標記句，20 份 **0 句被抄**（0／60），生成物與參考 `SKILL.md` 的最長共同字串 5～12 字（都是「## 輸出格式」這類標題）。GEN-006「不得把參考的指令當服務政策」在這 20 筆裡沒有反例。
- **量到的與沒量到的**：這 40 筆量的是「有沒有讀進去、有沒有抄」與成本；**沒有量的是品質**——沒有跑試跑、沒有人看過這 40 份。§6 的其餘四條照舊。

同一天的互動創作 15＋15 場見 [creation-measure/report.md](creation-measure/report.md)。
