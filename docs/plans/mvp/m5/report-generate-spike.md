...
- 日期：2026-08-23
- 依據：[README.md §前期驗證](README.md) 的前期驗證 1／[`03` GEN-009](../../03-work-items.md) 的前三分之一（[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 3 與決策 6 各壓了一個經驗假設，這批就是去驗那兩個）
...
- 模型：`gpt-5.6-sol`（flagship）。**不用 `gpt-5.6-terra`**——[litellm-config.yaml](../../../../infra/compose/litellm-config.yaml) 逐字寫它是「judge tier, **separated from the generating model**」，拿評審的模型去生成會把 [評估判定與 Judge 信任邊界](../../../adr/README.md#評估判定與-judge-信任邊界)想分開的兩件事併回去。
...
[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 3 與決策 6 各壓了一個經驗假設，寫 ADR 當下**只有推理沒有數字**：
...
[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 6 的查證補記推論「阻擋級檢查全是結構性的，一個被告知格式的模型幾乎必定全數通過」。**實測 80%**，而四次失敗拆開來是兩種，兩種都不是能力問題：
...
**而平台的 Sandbox 預設封鎖網路**（[Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)，沙箱層 nftables default-deny，**恰好放行一個目的地：LiteLLM 閘道**）。**這是決策不是已部署的事實**——repo 裡沒有任何 nftables ruleset，只有 `infra/egress/allowlist.yaml` 這份由 IaC 在節點建置時 render 的來源，而 SEC-009 的真實 Linux／gVisor 驗收仍在甲類未到期。所以這是一個：使用者付了錢、通過了每一道檢查、寫進了他的工作區、**而且在這個平台的執行環境裡結構性地跑不起來**的 Skill。

這就是[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 6 那條補記說的「拒絕率接近 0，門幾乎不擋東西」**具體長什麼樣**。驗證器沒有任何一條能發現這件事，因為它不是格式問題。**`GEN-004` 那句「沒有經過任何人工檢視、沒有任何試跑證據」不是禮貌用語，它是這個產品對 X-1 唯一說得出口的話。**
...
**[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 6 的「生成的 Script 不享有任何寬待」不是理論條款**——四分之一的產出用得上它。**而它漏了唯一一條會因為 Script 內容而阻擋的規則**：`possible-secret`（同一份決策）。這批 20 個樣本一次都沒觸發它。

## 4. 對[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)的回寫

| 原文 | 實測 | 處置 |
| --- | --- | --- |
| 決策 6 補記：「通過率預期接近 100%」 | **端到端 80%／驗證器那道門 84.2%** | 已回寫為實測值。**同一段的列舉還漏了 `possible-secret`**，見[從描述生成 Skill](../../../adr/README.md#從描述生成-skill) |
...
| （無）成本 | **$0.1184／次**（生成的單價，不含連通測試） | 新增：生成比一次 mini 級試跑貴，[身分、Workspace、准入與額度](../../../adr/README.md#身分workspace准入與額度)的配額設計要知道 → 已由[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 5 裁定為獨立額度 |

## 5. 這批**沒有**回答的

- **生成的 Skill 跑起來好不好用。** 要 Sandbox ＋ 評估管線，是 `GEN-009` 的第②③項。
- **人會不會留著它。** 要人（`GEN-009` 第④項），刻意不用機器指標頂替——同 `04` 丙-38 的既有紀律。
- **`possible-secret` 的命中率。** 20 個樣本一次都沒觸發，所以[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)那條不重試規則目前是一段沒有人走過的分支。
...
**`license` 那一條的遵守率是 19／19。** 但那只證明 prompt 有效，**不證明機制**——`03:GEN-001` 要求這條在 schema 上成立而不是靠 prompt，理由現在有了：一批沒違反不代表下一批不會，而這個欄位一旦寫進去，它佔用的是「已宣告」那個狀態（[從描述生成 Skill](../../../adr/README.md#從描述生成-skill)決策 5）。
