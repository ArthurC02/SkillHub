...
| **④（Script ≤ 300 行、三項禁止、人工逐行）** | **14／15 pass，`data-analyst` FAIL** |
...
**`02:497` 只寫了「機械量測通過但人工未審 ⇒ 維持 `pending`」，沒有寫機械量測 FAIL 的處置**，因為寫的時候沒有人量到有一筆會 FAIL。**這是一個策展決定不是實作工作**——選項是把它降到已索引層（`data` 類精選會少一個）、放寬 300、或換一個精選候選。已開為 [`05` R-16](../../05-pending-rulings.md)。

**在那之前 `CONTENT-006` 不勾**，而它的其餘 14 筆已經審完、不必重做。
