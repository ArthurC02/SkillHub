...
>
> **2026-08-15 修訂**（依據 [m1-work-items-audit.md §8 第三梯](../m1/m1-work-items-audit.md)，CONTENT-007／008 已改列 M2）：勾選條件改為 **CONTENT-005／006 全數完成**。⑧「平台基準試跑」不再是 CONTENT-003 的勾選前提，改以引用處理——勾選當下 ⑧ 須明確記為「待 M2 基準試跑，見 CONTENT-007／008」，**不得記為 pass**。⑤⑦ 的綁定不變（原文保留於刪除線，供回溯）。
...
- **2026-08-16 回填**：四種平台行為（索引／展示／沙箱試跑／打包下載，另加 LLM 增強產出）的逐項風險評估與處置提案已寫成 [governance/anthropic-sa-license-memo.md](../governance/anthropic-sa-license-memo.md)。**該備忘同樣不是法律意見、不作終判**，只把選項與代價攤開；終判仍在負責人與法務。
...
### 6.0 白名單異動帳（`02:CONTENT-001` 第 7 條、`02:CONTENT-009` 第 4 條）

**2026-08-28 新增。** 這兩條允收逐字要求白名單異動（**新增、否決、下架**）記錄 **提名人、日期、原因與涵蓋類別**，而在此之前這份文件記了原因與類別、沒有記提名人與日期——**§6 的三張表答得出「為什麼不收」，答不出「誰在什麼時候決定的」**。`03:CONTENT-009` 因此一直不勾。

**這一節是帳，不是新的決定。** 每一列的原因與明細都在下面的既有小節裡，這裡只補齊那兩欄並指過去。

**兩件必須先講清楚，否則這張表會被讀成比它更強的東西**：

1. **「日期」是這個決定進入本文件的日期，不是它被做出的日期。** 來源是 git（逐列附 commit），因為平台不持有白名單——它是一份策展產物，不是一張資料表，**所以沒有任何 per-entry 時間戳可以取**。兩者通常同一天，但這張表只保證前者。
2. **這是一本流水帳，不是一份現況清單。** 它答得出「誰在何時准入或否決了什麼」，**答不出「現在的白名單到底有哪些」**——後者要一個可查詢的狀態，而 `02:CONTENT-001` 第 5 條那句「進入索引的來源必須在白名單內」**今天沒有任何程式在把關**（匯入只過 `admission/fetch.go` 的 host 白名單，那是 SSRF 防護，與收錄來源白名單是兩件事）。**這一節不解那個缺口，也不假裝解了。**
...
| **下架** | **0 筆** | — | — | 至今沒有任何白名單條目被下架 | — | **這一格是空的，而空的不等於沒有機制**：跨工作區下架於 2026-08-28 落地（`04` 丙-80），下架流程存在且會寫 audit event；只是還沒有用過 |

**M0 已記錄的 14 組不在本帳內**，它們屬於 [data-category-sourcing.md §4.3](../m0/data-category-sourcing.md) 的更早一批，提名人相同、日期以該文件為準。
