...
**這個結論會在什麼情況下改變**：如果展示需要**真正的併發語意**（例如現場示範兩個人同時搶同一把鎖），那要換的不是加一層代理，是換候選——`pgmock` 已實測給得出真的多 backend（ADR-060 決策 3），代價是 32-bit 的 pgvector 缺口。**加代理只會讓「看起來像多 session」這件事更難被發現。**
