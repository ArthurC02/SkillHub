...
1. ~~**Node 在該環境的白名單上**~~ **2026-08-29 已答：可以跑 Node。** 剩下的是 **Go**——B 半跑的是 Go 測試，而 Go 會把未簽章的執行檔丟進暫存目錄再執行（environment-probe 的 **Q2**）。**同一題現在也擋著 A 半的啟動指令碼**，因為它是在那台機器上現場建 `cmd/api` 與 `sandboxd`。
...
| 2 | 受限環境的白名單內容已知 | **Q1 已答（2026-08-29）：Node 可以跑，所以資料庫那一軸成立、A 半不必重新設計。**<br>**但擋著的那一題換人了**：今天的啟動指令碼還需要 `go`（`cmd/api` 與 `sandboxd` 是 Go 程式，指令碼在那台機器上現場建它們），而 **Go 會把未簽章的執行檔丟進暫存目錄再執行**——那正是 [environment-probe](environment-probe.md) 的 **Q2**。**Q2 現在是關鍵路徑，而它沒有繞法**：替代路是「事先建好的二進位帶上去」，那是同一道白名單問題的另一種問法。<br>~~未取得，而且它現在擋住的比原本記的多~~。資料庫那一軸的**兩條候選路都跑在 Node 裡**（PGlite 與 pgmock 都是 npm 套件），所以 Q1 是「否」的話 A 半也要重新設計。**已備妥可直接交出去的清單：[environment-probe.md](environment-probe.md)**——五個問題、十分鐘、全部唯讀、不安裝任何東西、不做任何繞過 |
