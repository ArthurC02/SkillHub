...
- 決策：**淨測試模式的核心決定（~~Proposed~~ **Accepted，2026-08-30**）**——淨測試模式是**同一套產品程式以旗標切換三個實作**；淨測試模式建立在 API 邊界後方真實 PostgreSQL 之上的先前決定已被它取代（決策 1／4／5 延續，量測全部仍有效）、**淨測試模式執行 Driver 誠實不僭稱沙箱的那份決定（~~Proposed~~ **Accepted，2026-08-30**）**（沙箱那一軸）<br>**2026-09-03 訂正**：本列寫成時兩份都還是 `Proposed`，兩者都在 **2026-08-30** 轉 `Accepted`（見各自的狀態行）。狀態以 ADR 檔案本身為準。
...
### ✅ 2026-08-29：環境的三個事實取得了，而它們縮小了這個里程碑

**負責人答覆 [`05` R-23](../../05-pending-rulings.md)**。**本節只登錄事實，不做設計**——要怎麼用這些事實是 `PORT-007`／`PORT-011` 規劃時的事，今天不決定。

| 問的是 | 答覆 | 它改變了什麼 |
| --- | --- | --- |
| **防火牆能不能多開一個網域**（把 Run 送回自己的 gVisor 節點，R-23 的選項 (b)） | **不能。(b) 否決。** | **這是本里程碑最短的退場路徑，它關了。** [淨測試模式](../../../adr/README.md#淨測試模式)的「退場條件」一節記著這件事，下一個人不必再問 |
| 套件取不取得到 | **前後端所需套件都取得得到** | **離線 bundle 的重要性下降。** 它解的是「拿不到套件」，而那個問題不存在——Node 半 9.1 MB、Go 半 149 MB、以及那個 **246 MB 的 Go 工具鏈模組**（`go.mod` 要的版本比一般機器裝的新，`go` 會自己去下載它，空 bundle 在編第一行程式之前就死於 `toolchain not available`）**都不再是關鍵路徑** |
| 模型出口通不通 | **OpenAI API 打得通** | **本機起一個 LiteLLM 閘道成為可能。** 這是規劃 `PORT-007`／`PORT-011` 時可以假設的一個事實，**而不是一個已經做出的設計** |
| 網頁搜尋 | **大部分可達**（部分站台不允許） | 現場的輔助動作（查文件、開連結）不必全部離線化 |

**上面「對外網路只保證得到模型供應商，其餘看白名單，而負責人明示不想賭」那一句仍然成立**——它講的是**保證**，而今天拿到的是**實測**。兩者的差別在 Demo 當天會不會變，沒有人知道，所以本節不把實測升格成保證。

**時程錨點（負責人同日一併裁定，`01` §10 有完整的裁定表）**：

- **Demo：2026 年 9 月中下旬。** 本 README 先前逐字寫著 Pitch 日期「未知」，並把那件事列為**最該被質疑的地方**——那一句到此為止。
- **M6 真正完成後凍結新功能。**「真正完成」＝ `03` §20 的十一項全部收束（勾選或撤回），不是「程式面收斂」。
- **M1 閘門 D 日 ＝ 2026-09-11**，也是本輪夯實工作的最後期限。**它與 Demo 搶同一個人**。

**這一批事實對 M6 的淨影響是「少做」不是「多做」**：(b) 原本是唯一一條「隔離與生產一模一樣、不必生第二套安全論述」的路；它關了之後，M6 剩下的價值集中在**讓 demo 在那台機器上跑得起來**，而不是**讓不受信任的內容在那台機器上安全地跑**——後者今天沒有解，也不需要有解（`02:PORT-010` 第 5 條的內容來源限制**在 `PORT-010b` 完成前沒有強制點**，見該條 2026-08-29 的補記）。
...
1. ~~**Node 在該環境的白名單上**~~ **2026-08-29 已答：Node 可以跑，Go 也可以執行（Q1、Q2）。這一條不再擋任何東西。**
2. **解掉「握一條連線、再開一條」的假設**——實測顯示 `lockTestSchema` 在單連線下死鎖，而其中有些測試（advisory lock 序列化、並行首次登入）**本質上就需要多於一條連線**。<br>**2026-08-29 對真的 PGlite 承載重測，這一條是 B 半今天唯一剩下的阻礙**：守衛擋在資料庫名稱（而 PGlite 連線時接受任何名稱，**所以用假名字就能過關——不該這樣過**）、連線池預設多條會把承載弄卡、單連線之後**死鎖十分鐘**。

~~**B 半刻意不給時程。** 在那個事實到手之前，任何時程都是猜的~~ **2026-08-29：那個事實到手了，而 B 半仍然不給時程——理由換成「它需要一次沒有被主張過的程式改動」**（`02` §4.10 明文寫著本節不主張 B 半）。要開它先要一次裁定。<br>——而這份 repo 在 2026-08-28 這一天已經因為未確認的環境前提改過兩次計畫。

## ⛔ 邊界：這個模式要自己說出它不是什麼

[淨測試模式](../../../adr/README.md#淨測試模式)決策 5（延續同一份決定先前版本的決策 4）。**這一條的形式與 `02:GEN-004` 對生成物的要求同源**，理由也一樣：一個看起來像產品的東西，如果不說自己是什麼，就會被當成產品——而這一次的觀眾是金融機構。

**三條軸各有自己要宣告的東西**：

| 軸 | 在淨測試模式下不是真的 | 宣告形式 |
| --- | --- | --- |
| **沙箱** | **沒有隔離**（不是「比較弱的隔離」） | `isolation.level = "clean"`，由白名單閘門強制（[淨測試模式](../../../adr/README.md#淨測試模式)；`PORT-010a` 已完成） |
...
**淨測試模式不取代原有系統的任何能力。** 它是一個**平行、不干擾**的模式，以旗標切換，**原模式隨時可以照常運行**；**旗標未設時行為與今天逐位元相同**，而那正是「不干擾」的可檢驗形式（`02:PORT-005` 要求有測試在守）。決策見[淨測試模式](../../../adr/README.md#淨測試模式)。
...
| **沙箱** | `sandbox.Driver`（11 個方法） | `dockerdrv`（gVisor） | 本機行程 Driver（`PORT-010b`、[淨測試模式](../../../adr/README.md#淨測試模式)） | ~~**第二個實作**與選擇點。`sandboxd` 目前寫死 `dockerdrv`~~ **2026-08-29 兩者都落地了**：`sandboxd` 依同一個旗標選 `localdrv`，並宣告 `isolation.level = "clean"`（把它改成謊稱 `gvisor` 的突變會紅）；旗標未設時走 `dockerdrv`，也有測試在守。**今天缺的換成了契約**：`ResourceEnforcement()` 與 `Reaping()` 兩個誠實的偵測結果在能力宣告裡沒有欄位可以說出來（`04` 丙-83 已於同日補上兩個欄位，**但平台側的消費端還沒接**，見該列） |
...
**仍然沒有量到的，逐條**：~~完整的 API server 沒有起來過（本次只跑了 queue 這一層）；派送到沙箱那一段沒有走過~~ **2026-08-29 晚間這兩項都走過了**（`tools/cleanmode/start.mjs` 起三個行程，淨測試模式第一次整套跑起來，`PORT-001`／`003`／`005` 隨之勾選；`sandboxd` 依旗標選 `localdrv` 並宣告 `clean`）；**仍然沒有量到的是**：沒有做長時間或多使用者的壓力；`pool_max_conns=1` 之下真實請求量的延遲未知；**而同日稽核在這一條上找到第四個受害者**——Outbox publisher 握著那唯一一條連線去要第二條（`04` 丙-88，[淨測試模式](../../../adr/README.md#淨測試模式)有補記）。**這一段證明的是「那個阻礙不存在」，不是「這條路已經可行」**，而那句話在有了第四個受害者之後更成立了：**「握一條、再開一條」的假設散在整個 repo，而今天沒有任何機器在對它做整體檢查。**

### 備援：多連線這個限制，`pgmock` 身上不存在

`pgmock`（v86 模擬 x86，裡面跑未修改的 postmaster）實測：**兩條獨立的 pgx 連線拿到不同的 backend（1600／1603）、`pg_try_advisory_lock` 一真一假、temp table 互不可見**；42 支 migration **37 過，5 支全部只敗在 pgvector，零個版本語法失敗**；**判準一過**（UPDATE 與 DELETE 都被擋，訊息帶[資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)）。逐項見 [report-inmemory-postgres.md](report-inmemory-postgres.md) §10。
...
**不採 `pgmock` 的理由不是它做不到，是它剩下的缺口大小未知**：沒有 pgvector，而 guest 裡沒有編譯器；補上它要在別的機器為 i686-musl 編一顆共享函式庫，**而 pgvector 在 32-bit 有 runtime 失敗前科**。**它記在這裡而不是被刪掉**，是因為若單行程形狀在實作中垮掉，它是唯一已知還站著的替代品（[淨測試模式](../../../adr/README.md#淨測試模式)決策 3）。
...
## 「要不要在 PGlite 前面加一個連線池」——2026-08-29 問過、量過、不加

**問題是對的**：一條連線在展示現場會不會出事。**答案是不加，而且理由不是潔癖，是三個量到的數字。**

**① 那個「連線池」就是 multiplexer，而它會偽造互斥。** `pglite-socket` 唯一能讓多個 client 進來的機制，是把它們共用同一個 Postgres session。實測：兩條連線**同時拿到同一把 `pg_try_advisory_lock`**、`pg_backend_pid()` 兩邊相同、temp table 互相看得見。**本專案有三處產品程式靠 advisory lock 互斥**（`creator/workspace`、outbox 的單一 publisher、`runs`），在那個組態下它們會**全綠而毫無保證**。`maxConnections > 1` 因此是禁用組態（[淨測試模式](../../../adr/README.md#淨測試模式)決策 2）。<br>
**而「真的多 session」在 PGlite 上不存在**：Postgres 每條連線 fork 一個 backend，WASM 沒有 `fork()`。任何宣稱做得到的東西都是多工，不是多 session。

**② 併發不是瓶頸，這是量的不是猜的。** 對跑起來的淨測試模式：

| 情境 | 結果 |
| --- | --- |
| 10 個併發請求 | **全部 200，每個約 2 ms** |
| 50 個併發請求 | 31 × 200、19 × 429——**而那 429 是速率限制器，不是資料庫**（burst 過後單一請求立刻回 200） |

**一條連線扛得住一個人在台上點畫面，而且差得很遠。**

**③ 真正咬過人的不是併發，是「卡死」，而它已經從兩個方向關上了。** 先前一次失敗的 TLS 探測讓承載之後每一條連線都逾時且不自行恢復。今天：連線字串內建 `sslmode=disable`（探測不會發生），而且**`maxConnections=1` 加上 API 長期握著那唯一一條，等於沒有第二個 client 進得來把它弄壞**——實測送一個裸 SSLRequest 再斷線，承載毫髮無傷，API 照樣回 200。**限制本身變成了保護。**

~~**這個結論會在什麼情況下改變**：如果展示需要真正的併發語意……~~ **2026-08-29 負責人裁定：展示現場不會有兩個人同時操作。** 那個觸發條件因此**不成立，這一條路關上**——不要再為併發重開連線池或換候選的討論。<br>
**`pgmock` 仍然留著，但它等的是另一件事**：若單行程形狀在實作中垮掉（[淨測試模式](../../../adr/README.md#淨測試模式)決策 3），它是唯一已知還站著的替代品。**併發不再是它的理由。**<br>
**而畫面上那句「資料庫只有一條連線、併發語意與生產不同」不撤**——它擋的不是展示現場的人數，是**日後有人拿「在淨測試模式下沒重現」當作併發缺陷不存在的證據**。那兩件事沒有關係。
...
| # | 條件 | 現況 |
| --- | --- | --- |
| 1 | 淨測試模式的兩項先前決定由 `Proposed` 轉 `Accepted` | **未定**。其中一項動的是測試可信度的定義與部署形狀，不只是工具選擇；另一項動的是派送閘門的語意 |
| 2 | 受限環境的白名單內容已知 | **✅ Q1 與 Q2 都已答（2026-08-29）：Node 可以跑，Go 也可以執行。**<br>**A 半因此在那台機器上成立**——PGlite 是 Node 套件，啟動指令碼現場建的兩個服務是 Go 程式，離線相依兩半都打包並驗過（`tools/cleanmode/bundle.mjs`，Node 9.1 MB ＋ Go 149 MB，其中 246 MB 解壓後是 Go 工具鏈本身）。<br>**B 半仍然不成立，但擋住它的東西換了**：不再是環境，是程式。同日對真的承載試過——測試守衛擋在資料庫名稱、連線池預設開多條會把承載弄卡、加上單連線之後**死鎖十分鐘不動**，而原因文件早就寫著（`lockTestSchema` 握一條再開一條）。**它等的是一次程式改動加一次裁定，不是一個答案。**。資料庫那一軸的**兩條候選路都跑在 Node 裡**（PGlite 與 pgmock 都是 npm 套件），所以 Q1 是「否」的話 A 半也要重新設計。**已備妥可直接交出去的清單：[environment-probe.md](environment-probe.md)**——五個問題、十分鐘、全部唯讀、不安裝任何東西、不做任何繞過 |
...
- **[淨測試模式](../../../adr/README.md#淨測試模式)的三項待決策**：①**合併入口是第三個二進位還是既有二進位的旗標**；②**淨測試模式的 Run 要不要進正式的 Run 歷史**（它們是真的 Run，但跑在沒有隔離的地方）；③**`objstore` 的選擇點放哪裡**（`FromEnv()` 內部分支最省，但那讓「生產永遠拿到真的 client」不再是型別層面的保證）。
- **[淨測試模式](../../../adr/README.md#淨測試模式)的三項待決策**：①工作負載的環境變數契約抽到共用套件還是複製一份（**ADR 的建議是抽出來**）；②與上一條的②同一個問題；③`Adopt()` 回空要不要在畫面上說（服務重啟會殺掉跑到一半的 Run）。
- **受限環境的白名單內容**（見啟動條件 2）。這是要取得的事實，不是要做的決定。
- ~~**Pitch 現場要不要能真的跑一次**（[`05` R-23](../../05-pending-rulings.md)）~~ **2026-08-29 部分回答**：選項 (b)（爭取多開網域）**否決**；(a)（播放真實錄下的 Run，或以本機 Driver 現跑策展素材）成為預設。**「要不要跑觀眾當場出的題目」這一題本身仍未回答**，但可選的答案少了一個，而剩下的都不需要新的行政前置。
- **`02:PORT-010` 第 5 條（不得承載不受信任的內容）要不要一個真的強制點**，還是明文由操作者承擔。今天三份文件互相指認對方是強制點而沒有一個是；補法是平台側的一行判準（`is_catalog` 或 `curation_tier`，欄位已存在），**不是 Driver 的事**。→ `04` 丙-85
...
| [report-judge-regression-2026-09-26.md](report-judge-regression-2026-09-26.md) | 現行 Judge 的 45 筆固定基準、重複性與證據缺漏驗證；附逐筆原始回應、成本與限制 |
