...
| 問的是 | 答覆 | 它改變了什麼 |
| --- | --- | --- |
| **防火牆能不能多開一個網域**（把 Run 送回自己的 gVisor 節點，R-23 的選項 (b)） | **不能。(b) 否決。** | **這是本里程碑最短的退場路徑，它關了。** ADR-059 的「退場條件」一節記著這件事，下一個人不必再問 |
...
**① 那個「連線池」就是 multiplexer，而它會偽造互斥。** `pglite-socket` 唯一能讓多個 client 進來的機制，是把它們共用同一個 Postgres session。實測：兩條連線**同時拿到同一把 `pg_try_advisory_lock`**、`pg_backend_pid()` 兩邊相同、temp table 互相看得見。**本專案有三處產品程式靠 advisory lock 互斥**（`creator/workspace`、outbox 的單一 publisher、`runs`），在那個組態下它們會**全綠而毫無保證**。`maxConnections > 1` 因此是禁用組態（ADR-060 決策 2）。<br>
...
**`pgmock` 仍然留著，但它等的是另一件事**：若單行程形狀在實作中垮掉（ADR-060 決策 3），它是唯一已知還站著的替代品。**併發不再是它的理由。**<br>
