...
- 狀態：**試算完成（v2，經複核修正；§6.2 已依模型供應商定案增補 v3 重估），資料所有權與核心基礎設施、Sandbox 隔離與執行安全定案待負責人確認**
- 日期：2026-08-13（v1）／2026-08-13 修訂（v2）／**2026-08-14 §6.2 增補 v3**（模型供應商定案為 OpenAI，見 §6.2.3 與 §9.4）
- 用途：解除 [資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)（「部署平台確認與成本試算」）與 [Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)（「部署平台支援驗證」）的定案前置條件；同時回應 [01-goals-and-plan.md](../../01-goals-and-plan.md) 第 12 節風險「Sandbox 成本不可持續」。
...
> ⚠️ **與核心基礎設施選型的取向衝突**：核心基礎設施選型（見 [資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)）當時（Proposed）選型原則 1 為「受管服務優先，不自架可以買到的東西」，決策表明列「受管 PostgreSQL」。本文件的方向與該原則相反（**物件儲存維持受管，故該列不衝突；衝突僅在 PostgreSQL 一列**）。定案時須由負責人決定：**(A)** 趁該決策仍為 Proposed 直接修訂其選型原則與決策表，或 **(B)** 另立決策取代並將原決策標為 Superseded。**本文件不修改任何 ADR。**

---

## 1. 候選平台如何產生

核心基礎設施選型與 Sandbox 隔離技術選型 **未列出具名候選平台**——前者只指定能力，後者只指定執行節點形態。候選集合由決策內文的硬性條件推導：

| ADR 條件 | 出處 | 對平台的篩選效果 |
| --- | --- | --- |
| PostgreSQL 須能跑 FTS + pgvector | 資料所有權與核心基礎設施決策表、意圖搜尋決策 | 容器化後不篩選平台（自帶 extension 的映像）；受管對照組需供應商支援 pgvector |
| S3 相容物件儲存（presigned URL） | 資料所有權與核心基礎設施決策表 | 需 S3 API 相容 |
| gVisor（runsc）**在一般雲端 VM 即可執行，不需裸機或巢狀虛擬化** | Sandbox 隔離與執行安全的選項 B | **不篩選任何平台**；僅要求可自訂 OS 映像的 IaaS VM |
| 執行節點為**獨立 VM 池**，不與應用工作負載混排 | Sandbox 隔離與執行安全決策 | **Sandbox 節點不進 Kubernetes**，E1／E2 皆維持獨立 VM 池 |
| Egress 全部經專用 Proxy，default-deny | Sandbox 隔離與執行安全 | 需自建 Proxy 節點；刻意不用受管 NAT Gateway |
| 控制平面採模組化單體 + 少量 Worker，依壓力拆分 | 系統情境、平面與部署路徑 | E1 的節點合併符合此原則；E2 只在壓力觸發後啟用 |
| LiteLLM Proxy 為獨立部署單元 | 模型閘道與可觀測性 | 容器化，與控制平面共節點 |
| Langfuse **Cloud**（自架 v3 需 ClickHouse+Redis+S3） | 模型閘道與可觀測性 | 固定 SaaS 訂閱，所有組合相同；**明確列為不容器化的例外** |
...
註：Fly.io、Render 等 PaaS 未列入——Sandbox 隔離與執行安全的決策要求 Sandbox 為自管的獨立 VM 池並可自訂 runsc runtime，PaaS 抽象層阻擋此需求。
...
| 單 Run 佔用時間 | **6 分鐘** = 4 分執行 + 1 分 provisioning/gVisor 冷啟 + 1 分清理 | Sandbox 隔離與執行安全「分鐘級」；介於 PDM-010 的中位 2 分鐘與 PDM-005 的 10 分鐘軟上限之間 |
| gVisor 效能損失 | 已內含於上述 6 分鐘（相對 runc 約 +20% wall time） | Sandbox 隔離與執行安全「性能損失需納入 Run 時間與成本估算」 |
...
| 節點冗餘 | **N+1**（兩個情境一致套用） | Sandbox 隔離與執行安全「節點以 IaC 建置、無狀態、定期重建」需滾動汰換餘裕 |
| 單 Run egress | 10 MB | 文字負載；模型閘道與可觀測性呼叫路徑 |
| 單 Run 產出物件 | 20 MB | 資料所有權與核心基礎設施「大型 Payload 進物件儲存」 |
| 物件保存期 | 90 天（Trace）／30 天（Artifact） | PDM-006 分級表 |
| 單 Run Langfuse units | 30 | 模型閘道與可觀測性埋點範圍 |
| 每月天數 | 30 | — |
...
情境 (b) 的部署形態仍在 [系統情境、平面與部署路徑](../../../adr/README.md#系統情境平面與部署路徑) 的「模組化單體 + 少量 Worker」範圍內，未觸發任何服務拆分條件。
...
- 網域、TLS 憑證（Let's Encrypt $0）、CI/CD 分鐘數、O11y 受管後端（資料所有權與核心基礎設施尚待決策，各平台皆有免費額度）。
...
> **配置修訂註記（2026-08-16，[Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全) 強制條件 6）——總額不變，原文不改寫。**
>
...
> - **修訂後**：**LiteLLM 移出資料節點，改跑在沙箱面專屬節點上**——即原本編列給 Egress Proxy 的那台 CX33（(b) 為 2 台）。Sandbox 隔離與執行安全 Q3 決定不部署 Squid／Envoy（沙箱層允許清單收斂為一項平台自有目的地），該預算行因此空出來，正好承接這個需求。
> - **為什麼必須移出**：Sandbox 隔離與執行安全 Q2 強制條件 6——沙箱層允許清單的目的地**不得是控制平面／資料節點的位址**。LiteLLM 若留在資料節點上，沙箱「唯一到得了的位址」就是那台跑著 PostgreSQL 的機器，系統情境、平面與部署路徑的平面隔離會被一條防火牆規則抵銷（威脅模型 TM-CTL-01）。
> - **金額影響**：**$0**。兩情境的月費合計、單 Run 平台成本與所有比較結論皆不變；改變的只是同一筆錢買到的東西從「Egress Proxy」變成「沙箱面 LiteLLM 節點」。
> - 驗證：Sandbox 隔離與執行安全 SEC-009 測項 **T5-7**（對允許清單 `pinned_ip` 的 5432／平台 API／9000 必須 DROP）是這項配置被違反時唯一會叫的檢查。
...
- **D. Hetzner+Neon**：Neon **4 CU** × 730h × $0.106 = $309.52 + 200 GB × $0.35 = $70 → $380（假設大型 Payload 已依資料所有權與核心基礎設施下沉物件儲存；若 300 GB 全留 Neon 則 +$35）。
...
| **Langfuse** | **例外：維持 Cloud** | 自架 v3 需 ClickHouse + Redis + S3 三個新資料產品，違反資料所有權與核心基礎設施選型原則 2；$29–57/月遠低於三套備份監控升級的成本 |
| **Sandbox 執行節點** | **例外：獨立 VM + gVisor，不進 K8s** | Sandbox 隔離與執行安全明訂執行平面為獨立 VM 池、不與應用工作負載混排；把 Sandbox 放進 K8s 會讓系統情境、平面與部署路徑的平面隔離退化為命名空間隔離 |
...
| 容器自架於節點內含 NVMe | 需 CX53 級節點多台 | 無跨節點耐久性，節點重建即資料遺失——與 Sandbox 隔離與執行安全「節點定期重建」直接衝突 |
...
| 資料遺失（節點重建、磁碟故障） | pgBackRest／WAL-G 持續 WAL 歸檔至物件儲存 | 同左 + standby | **還原演練必須排入 M0 收尾**，未演練過的備份等於沒有備份（系統情境、平面與部署路徑的 DR 要求） |
...
| --- | --- | --- | --- |
| **單 Run 佔用時間** | 6 分鐘 | 尖峰 slot 13 → 25，節點 5 → **8 台**。E1 Hetzner Sandbox $802 → **$1,283**（總計 **$1,448**）；C. DO $1,360 → **$2,176** | [Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)待決策；PDM-005 的軟上限是 **10 分鐘**，加 provisioning/cleanup 約 12 分鐘——**翻倍即等於「使用者普遍跑到上限」**，不是極端假設。**唯一應優先量測的數字**（NFR-004 已要求量測 Sandbox 建立時間） |
...
| 風險 | 嚴重度 | 說明與對策 |
| --- | --- | --- |
| **濫用政策**：Sandbox 執行不受信任程式碼，若被用於挖礦、掃描或濫用流量，Hetzner 處置偏向直接停機而非協商 | **高——選 Hetzner 必須付出的營運工作** | [Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)的 default-deny Egress Proxy 是主要對策；另需**事前與 Hetzner 溝通用途**、建立濫用偵測與自動封停 Run 的流程（對應 SEC 需求）。這一項與容器化無關，且是 Hetzner 相對 DO 的唯一實質劣勢 |
...
**MVP 採 E1**：docker compose 或 k3s 單機，控制平面全容器化。理由是[系統情境、平面與部署路徑](../../../adr/README.md#系統情境平面與部署路徑)的「依壓力拆分，不預先建設」原則同樣適用於編排層——E2 在 MVP 規模**不省錢**（§4.3 事實 3），只增加 K8s 升級稅。
...
3. **[系統情境、平面與部署路徑](../../../adr/README.md#系統情境平面與部署路徑)服務拆分觸發表已觸發任一列**（Run Orchestrator、Ingestion Worker、Catalog & Search、Evaluation Worker 需獨立擴展或獨立部署）。
...
3. **Sandbox 執行節點 → 獨立 VM 池 + gVisor，不進 Kubernetes**（[Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)不變）。這佔總平台成本的 **83%**，也是唯一真正值得優化的一項——但優化手段是**縮短單 Run 佔用時間**（§6.1）與**控制 Run 量**（PDM-010），不是換編排技術。
...
4. **ADR 怎麼處理？** 本方向與[資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)（Proposed）的「受管服務優先」原則衝突（僅 PostgreSQL 一列）。修訂仍為 Proposed 的該項決策，或另立新決策取代？

---

## 8. 下一步

本文件解除[資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)與[Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)定案條件中的**成本試算**部分。定案前仍待：

- **[資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)**：部署平台確認（＝ §7.4 決策問題 1）；**容器化方向的 ADR 處理方式**（＝決策問題 4）。
- **[Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)**：PDM-003／004 Runtime 確認、部署平台支援驗證（在選定平台上實跑 runsc）、逃逸測試通過（SEC-009、SBX-010）。
...
5. **Sandbox 執行節點維持獨立 VM 池、不進 K8s**（[Sandbox 隔離與執行安全](../../../adr/README.md#sandbox-隔離與執行安全)的執行平面隔離不變）。
6. 新增 §5.3 自架風險（**E1 為 Postgres 單實例，無自動 failover**）與 §5.4 每月營運小時估計。**§5.4 的誠實結論是：容器化在 MVP 規模不靠省錢回本，損益平衡費率僅 $18–69/h。**
7. **與[資料所有權與核心基礎設施](../../../adr/README.md#資料所有權與核心基礎設施)的衝突已於 §0 標明**，並列為 §7.4 決策問題 4：修訂仍為 Proposed 的該項決策，或另立新決策取代。**本文件未修改任何 ADR，亦未變更任何工作項目勾選狀態。**

### 9.4 v3 增補（2026-08-14）

| # | 位置 | v2 | v3 | 原因 |
| --- | --- | --- | --- | --- |
| 14 | §6.2（新增 §6.2.3）、§6.1 三變數表 | 模型費 **$0.30/Run**（`claude-sonnet-5`），佔總帳單 61%～97%；「AWS 與 Hetzner 差距壓縮到約 20%」 | 模型費 **$0.05/Run**（`gpt-5.4-mini`，含 87% 實效快取折扣），佔比降為 **21%～44%**（打滿上限時 82%）；**AWS 與 Hetzner 差距回到 63%** | **負責人 2026-08-14 定案模型供應商採 OpenAI API**（經 LiteLLM 閘道，[模型閘道與可觀測性](../../../adr/README.md#模型閘道與可觀測性)不變），試跑預設依實測定為 mini 級。**平台選型的槓桿因此回升，v2「基礎設施辯論是次要問題」的結論在中位用量下不再成立**；三大成本變數的順序改為模型分層 → Token 預算 → 免費額度 |
