...
- 殘餘風險：抓取是控制平面的直接出口，不走 Sandbox 的 egress proxy 與允許清單（ADR-022）；同意是「這個網址」，不是「這個網址會連到的每一個位址」。
...
- **2026-09-07 裁定**：`05` R-53 已簽——殘餘風險的緩解正式定為「目錄維持策展」（僅人工審核可進 `is_catalog` 工作區），寫入 ADR-013 定案調整 8；投毒量測列為放寬准入前與改檢索／增強規則時的常設紅線。對 `indexed` 層級加 Top-3 曝光上限或延遲策展信號未採用（現在不做）——它防的是投毒已進目錄之後的擴散，而進目錄先經過人；重啟條件是出現放寬准入的提案且重量後紅線仍未過，屆時再設計並回填 `03`。
...
45 是這份清單從 2026-08-15 到 2026-08-25 的數字，N-08 在後者加入時沒有跟著改，而它下面的分區表、§5 的閘門歸屬與 §5.6 的覆蓋表都還按 45 寫。**後果不是算錯一格**：ADR-022 第三部分的覆蓋核對按 N 區 7 項配測項，N-08 因此沒有任何測項指到它，而整體通過判準是「45 項全數 pass」——一個存在於基線、卻不會被評到的檢查。ADR-022 §2 的補記記錄了它的測項歸屬。
...
基線不是一次性文件，須以測試維持（對應 ADR-015「驗證方式」與 SBX-010／SEC-009）。下表覆蓋全部 46 項：
...
| ~~Q11~~ | ~~Sandbox 內注入 Virtual Key 的機制（環境變數 vs 設定檔）~~ → **已答（2026-08-26 回填，決定它的是程式不是本表）**：**環境變數**——`dockerdrv` 的 `env()` 以 `ANTHROPIC_AUTH_TOKEN` 傳遞，因為 Claude Agent SDK 原生讀它（PDM-003）；**本欄擔心的「對 Script 最易讀取」在這個選擇下是已知且被接受的**，補償控制是每 Run 專屬 ＋ TTL ＋ `max_budget` ＋ teardown 撤銷（ADR-017）。**但同日查出一個本表沒有的後果**：環境變數是容器的**啟動參數**，daemon 會保存到容器移除為止（實測 `docker stop` 後 `docker inspect` 仍讀得到），因此 `02:SEC-005` 第 3 條「不出現在啟動參數的持久記錄中」一度**對模型憑證不成立**。**2026-08-27 已收斂該條的字面範圍**：「持久」指**超出該 Run 生命週期**而仍存在，而容器在 teardown 被移除、Virtual Key 在同一次 teardown 被撤銷——**撤銷之後那個值不再是憑證**。本條仍然禁止任何 Run 結束後留著的形態，而「沒有被移除的容器」由 X-01～X-04 承擔。更硬的形態（tmpfs ＋ entrypoint 交接）未被否決，屬部署期硬化 | ADR-017、PDM-003、`02:SEC-005` |
