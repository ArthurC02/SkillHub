...
| ~~Q11~~ | ~~Sandbox 內注入 Virtual Key 的機制（環境變數 vs 設定檔）~~ → **已答（2026-08-26 回填，決定它的是程式不是本表）**：**環境變數**——`dockerdrv` 的 `env()` 以 `ANTHROPIC_AUTH_TOKEN` 傳遞，因為 Claude Agent SDK 原生讀它（PDM-003）；**本欄擔心的「對 Script 最易讀取」在這個選擇下是已知且被接受的**，補償控制是每 Run 專屬 ＋ TTL ＋ `max_budget` ＋ teardown 撤銷（ADR-017）。**但同日查出一個本表沒有的後果**：環境變數是容器的**啟動參數**，daemon 會保存到容器移除為止（實測 `docker stop` 後 `docker inspect` 仍讀得到），因此 `02:SEC-005` 第 3 條「不出現在啟動參數的持久記錄中」**對模型憑證不成立** → [`04` 丙-74](../../04-backlog-and-handoffs.md) | ADR-017、PDM-003、`04` 丙-74 |
