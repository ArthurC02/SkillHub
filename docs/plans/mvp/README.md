...
- MVP 的第一入口是探索既有 Skill，而不是從零撰寫。（**2026-08-23 補記**：[從描述生成 Skill](../../adr/README.md#從描述生成-skill) 開了 M5 的生成路徑，**這一條不因此改變**——決策 7 逐字把生成的入口放在「沒有夠接近的 Skill」那一格，首頁不提供與搜尋對等的生成入口。「第一入口」仍然是探索。）
...
11. [M5 執行計畫與產出](m5/README.md)（從任務描述生成 Skill）——**全面開工，程式面已收斂**（2026-08-23）：三項啟動條件由[從描述生成 Skill](../../adr/README.md#從描述生成-skill)暫時放行、投入上限由同一決策解除，**而解除它的是授權不是證據**（[ask-5.md](m5/ask-5.md) 一次都沒執行）。◐ 的是 `GEN-008` 與 `GEN-009`；**勾選數以 [`03` §19](../03-work-items.md) 的 checkbox 為準，此處不複述**。**目錄本身早於開工而存在**，因為 `GEN-009` 的前三分之一先跑掉了（[生成品質基線（前置）](m5/report-generate-spike.md)）。**一條仍然生效的邊界：開工不等於曝光**——生成入口不得對封測使用者出現，直到漏斗第一段有讀數為止

跨里程碑仍在被引用的主題目錄（依[Repo 結構、CI 與驗證層](../../adr/README.md#repo-結構ci-與驗證層)；不屬於任何 `mX/`）：[`content/`](content/)（策展資料與 writing rubric）、[`governance/`](governance/)（授權備忘與上游詢問信草稿）、[`gate-test/`](gate-test/)（M1 驗證閘門材料，**另含閘門與封測共用的[受測者同意書與資料保存政策](gate-test/consent-and-data-policy.md)——法務已於 2026-08-23 確認（[`05` R-4](../05-pending-rulings.md)），但仍未生效：§9 的 ⬜ 佔位未填完（報酬金額待簽）、受測者尚未簽署**）。
