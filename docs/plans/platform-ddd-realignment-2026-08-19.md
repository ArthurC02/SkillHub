...
| DDD-007 | decision | `testlab` 公開契約面明確化 | 二擇一：snapshot API 抽子包，或以套件文件正式標註公開面；ADR-032 待決策回填。 |
...
| DDD-012 | decision | 事件目錄 | 決定 outbox 事件 schema 落點（`contracts/` vs Go 型別＋文件）；每個事件有 producer context、consumer context、冪等鍵說明;ADR-032 待決策回填。 |
...
3. Phase 2 在 Phase 1 收斂後啟動；DDD-011 依賴 DDD-003 的 NewApp。
