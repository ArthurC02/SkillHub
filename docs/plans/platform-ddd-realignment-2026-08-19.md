...
### Phase 7 — read 存量開始清除（2026-08-21 更晚）

DDD-031 收掉的 B 組是正確性問題；剩下六組是整潔與 schema 耦合。本 Phase 收最便宜的兩組（owner 的 import 方向本就合法、Service 門面也都有），並把兩批改動在別處留下的過期宣稱一次對帳。

| ID | 狀態 | 項目 | 完成條件 |
| --- | --- | --- | --- |
| DDD-033 | fixed | read 存量的 C 組與 G 組：owner 開讀取面 | **2026-08-21 完成**：七條跨 context read 全部改走 owner 匯出的讀取面，形狀與 DDD-019／025／026 一致——收呼叫端的 `*gen.Queries`、不自開交易。testlab 新增 `ReadSnapshot`／`ReadDataset`／`CasesForSkill`／`CaseDatasets`／`WorkspaceObjectKeys`，trace 新增 `(*Service).LiveEvents` 與 `MaskingActivity`（前者是 Service method 因為 eval 已注入 `Trace *trace.Service`，後者是 package function 因為 run 只持有 `trace.Signer`）。**視窗大小留在 `run/halt.go`**：trace 回答 masker 做了什麼，run 決定什麼算證據。**`ReadSnapshot` 回傳列而非解碼後的視圖**——三個呼叫端要三個不同子集，而 `Decode*` 已經把那些欄位定義過一次。**推翻了 ADR-035 對 `ListWorkspaceObjectKeys` 的清除方向**：該 ADR 說它方向與 `PurgeWorkspace` 一致、應併進那支注入，但物件刪除發生在**交易之前**（物件儲存沒有 rollback），跑在交易內的步驟回答得太晚——故改為獨立的 `identity.WorkspaceObjectKeys` 注入欄位，未注入即拒絕整批（與五個 purge step 同一個 fail-closed 論證）。**`identity → testlab` 經查確實是 depguard deny**，所以這一條本來就不能走 import。命名陷阱全數避開（無任何 wrapper 與其包裝的 query 同名）。十四個破壞驗證，其中兩個第一次是綠的——**因此補掉兩個既有的測試盲區**：`grantsFor` 的 dataset 分支從來沒有測試（所有 fixture 都跑沒有檔案的 case），`ExcludedUserUploadedDataset` 這個較具體的打包拒絕理由也從未被斷言；另補一個「被引用的 trace 證據在事件被丟棄後不再宣稱可解析」的測試，那件事**整個 repo 先前沒有任何地方斷言過**。`read_allow:` 33 條目六組 → **26 條目四組**。 |
| DDD-034 | fixed | DDD-031／032 落地後的過期宣稱對帳 | **2026-08-21 完成**：兩批改動各自都有 agent 點名了自己 allowlist 外的過期敘述，本項逐條查證後分類處理。**更正**：ADR-032 附錄 A 的 2026-08-20 註記（結論不變，但取鎖的人換了）、`db/queries/test_lab.sql` 的 `LockTestCase` 註記（原本只說它序列化 dataset 上傳，實際還擋 criteria 編輯、rubric 寫入與快照凍結，且唯一的外部呼叫端已消失）、`db/queries/governance.sql`、`catalog/restriction.go` 檔頭、`m4/release-checklist.md` 的三處（環境變數表缺 `TRACE_RETENTION`、H-5 驗收欄、以及 §2.6 把「預先建立未來兩個月」講得太保守——真正會撞 `23514` 的是**當月**）。**推翻一個前提**：`governance.sql` 的註解並沒有錯，它從來沒指名呼叫端，只是對這次改動沉默——補了一則具名註記而非更正。**量化出一個先前沒人注意的歧義**：「條」在同一份文件裡是兩個單位——節標題是 yaml 條目數、分組標題是 (query, caller) 組數，兩者差 12；ADR-035、`db/query-owners.yaml` 與本 ledger 三處已一次講明。`db/queries/*.sql` 的註記由主 Agent 跑 `task gen:sql` 重生（**確認零行非註解變更**，故 generated 簽章不變）。**刻意不做**：不為 `0004`／`0019`／`0029` 三則過期的 migration 註解補一支只改註解的 migration——ledger 已逐則記下，等它們從「過期」變成「會害人做錯事」再說。 |
...
**Phase 7（DDD-033～034）於同日更晚追加並結案**：read 存量從 33 條目降到 26 條目，剩下的四組沒有一組是正確性問題。同批把 DDD-031／032 在別處留下的過期宣稱對帳完畢，並修掉一個讓所有數字都可能被誤讀的單位歧義。
...
- `read_allow:` 現為 **26 個條目、四組**（＝37 組 (query, caller)）。導入時是 35 個條目、七組（47 組）；**B 組由 DDD-031 收掉、C 與 G 兩組由 DDD-033 收掉**。⚠️ **這裡有兩個單位**：條目數是「一個 query 一行」，而分組標題與 ADR-035 表格裡的「條」是 (query, caller) 組數，兩者不可相加——這個歧義在 DDD-033 那批被指出並在三份文件裡一次講明。**剩下的 A／D／E／F 四組都是整潔與 schema 耦合問題，不是正確性問題**：A（11 條目）要 registry 開一支讀取面，D 要走注入（import 會成環），E 是 objreconcile 的讀取半邊沒跟著 DDD-020 反轉，F 是六個各有成因的單點。逐組判讀見 `db/query-owners.yaml` 的分組註解與 [ADR-035](../adr/ADR-035-read-ownership-enforcement-and-context-map-completeness.md)「存量」節。
...
- `testlab.WorkspaceObjectKeys` 回答的不只是 testlab 的列：它背後的 `ListWorkspaceObjectKeys` 是 `datasets UNION artifacts`，所以那支函式也替 run 與 packaging 的列作答。`db/query-owners.yaml` 把該 query 判給 testlab，函式因此只能住在那裡；要按 owner 拆就得先拆 query（DDD-033 行內，已在程式與 ADR-035 具名）。
- `run/grants.go` 的「dataset 在快照凍結後才被刪除」分支仍無測試（要造出那個競態太麻煩），DDD-033 只補到 happy path。
