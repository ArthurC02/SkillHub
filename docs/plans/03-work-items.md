...
> 3. **殘留的缺口必須有具名落點，不得因為本節收斂而消失。** 兩處：`DESIGN-007` 的 preflight 版本選擇器 → ；`DESIGN-010`／`011` 承接的[評估判定與 Judge 信任邊界](../adr/README.md#評估判定與-judge-信任邊界)待決策 → 實質已由既有畫面回答，剩餘的呈現缺口是 。**[評估判定與 Judge 信任邊界](../adr/README.md#評估判定與-judge-信任邊界)的待決策已就地回填此裁定**。
>
...
- [ ] DESIGN-007 設計 Test Case、Dataset、MCP 與工具設定流程。（**不再追蹤**——落點：`TEST-012` 的 `apps/web/src/features/lab/test-cases/` 與 `/lab/datasets` 上傳頁。**兩項範圍註記**：①MCP 與工具設定屬後 MVP，本項那一半不會有落點；②**preflight 頁仍然沒有 Skill Version 選擇器**——`version` 只能從 URL query 進來，`EVAL-011` 與  都把它記在本項名下。本項收斂後該殘留**改由  具名承接**，不隨本節一起消失）
...
> **2026-08-16 §9 允收範圍裁定（ 結案，取 `m2` 對帳 §3.2 的 (a) 但只退回一項）**：本節的允收準則**含 UI**，但**含不含 UI 是逐條判定，不是逐節判定**——判準取自 `02` 各條允收準則自己的主詞與動詞：
>
> | 準則的主詞／動詞 | 要在哪個平面滿足 | 例 |
> | --- | --- | --- |
...
> **`PACK-010` 這個編號從未存在（2026-08-29 查證後記，避免下一個人再找一次）。** 本節是 `PACK-001`～`009`，§19 有 `PACK-011`／`PACK-012`，**中間跳過一號**。稽核指出「一個消失的工作項與一個從未存在的號碼在文件上長得一模一樣」，而本專案有過「一個工作項寫在別的檔案裡所以沒人找得到」的前例。**本次結論是跳號**：全 repo 對 `PACK-010` 零命中，`02` 的 `PACK-*` 允收也沒有對應條目；`PACK-011`／`012` 是 2026-08-23 才新增的，編號從 011 起跳而沒有回填 010。**不補號、不改既有編號**——編號在這份文件裡是歷史，改它會讓所有既有引用失準；這一句就是那個號碼的墓碑。
...
>
> **共同的三個阻擋**（不逐項重複）：①**甲類四項未到期**（`SEC-009` **46 項**全 pass 0 unknown〔2026-09-03 訂正：原寫 45，基線 2026-08-25 起為 46〕、`SBX-010`、`SBX-005`／`007` 生產網路面、`SBX-002` 閘門 A 探針）——依 [m4/README.md §4](mvp/m4/README.md) 的裁定它們在 M4 是封測的阻擋項，拍板見 ；②**六項 PDM 未逐項追認**（`03` §1）；③**M1 驗證閘門的 D 日未宣告**——[m4/README.md §5.3](mvp/m4/README.md) 已寫明「**在一個未結案的閘門上宣告 MVP 完成，是把兩個相反的結論同時寫進同一份文件**」。
...
- [ ] RELEASE-004 Sandbox 與 Secrets 安全檢查完成（MCP、Local Runner 隨後 MVP 啟動時補檢）。（**不勾**：**2026-08-27 訂正：`SEC-003` 已勾（隨 `05` R-13），`SEC-001` 亦已勾（隨 R-14）；仍未勾的是 `SEC-002`（要節點）**——（第 3 條的字面範圍收斂， 結案），`SEC-009` 的十個測項沒有一個被記為通過（甲-1）。原文寫「十個測項一個都沒跑」，而 T1、T2 與 T8 的映像半都跑過了，證據在 [`m4/sec-009-acceptance/`](mvp/m4/sec-009-acceptance/)——**「跑過」與「記為通過」不是同一件事**，那份判定表逐列寫的是有沒有證據（17／4／25），不是 pass／fail。**無例外流程**——`02:SEC-009` 明文基線全數 pass 且 0 unknown 才放行，任一 fail 或 unknown 即不得開放外部使用者提交 Skill 執行；**基線自 2026-08-25 起是 46 項**（原文寫 45，同日更正，見[Sandbox 隔離與執行安全](../adr/README.md#sandbox-隔離與執行安全) §2 補記）)
...
- [x] RELEASE-007 使用者可理解執行權限、評估依據與相容性限制。（**不勾**：權限（`TEST-005`／`008`／`009`／`011`）與相容性（[打包、授權溯源與散布](../adr/README.md#打包授權溯源與散布)三層、`0022` 的量測列）兩項已落地，**評估依據那一項有 `QA-006` 記的破口**——`artifact` 型引用的引文從未回驗（ G7），使用者會看到一個看起來像依據的東西。**拍板前這一格不能勾**）
