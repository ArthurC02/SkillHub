...
- [x] OPS-011 漏斗趨勢圖（[`05` R-78](05-pending-rulings.md)）：`GET /admin/trends/funnel`（`RequireOperator`、列入 authz 矩陣），搜尋／看詳情／下載取 `analytics` 的查詢、開始試跑取 `run` 的查詢，計數粒度在 Go 決定；`/admin/trends` 多一組圖與四段的粒度說明。依帳號排行不做。（對應 `02:OPS-008`）
