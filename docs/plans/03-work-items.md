...
- [ ] INGEST-019 plugin manifest 的驗證與來源事實：宣告別份規格者為 info 並改走整棵樹、宣告公規卻 `name` 不合才是阻擋錯誤並指名是哪一項、`extensions` 連值都不驗、其餘未知欄位為 info；每個 Skill Version 記下 plugin `name`／`version`／`repository` 與相對路徑；一個 Plugin 只存一份套件物件（整包），各 Skill Version 共用它、內容雜湊取各自子目錄；單次匯入超過 50 個 Skill 即整批拒絕並說出兩個數字（`02:SKILL-006`）。
...
- [x] DISC-011 實作意圖分析與查詢改寫，並給中文查詢在 FTS 腿上的下界一個明文決定。（允收：`02:DISC-005` **全部七項**）<br>**七項各有具名測試**（對照見[需求 ID × 測試對照表](requirement-test-matrix.md)）：五欄結構化意圖、關鍵詞與合法篩選、可解釋且可修正並保留原句、分析失敗仍以原句走向量腿（具名反證測試把改寫器換成永遠失敗，斷言向量腿仍走得到且結果非空）、中文詞法的明文決定與實測下界、每個數字的強制點與 `one-number` 對帳、一次 Gateway 呼叫加匿名速率限制（限流擋掉的是第二次花費，不只是第二個請求）。<br>**中文的下界裁定為「只走向量腿」**：量過的每個詞法候選要嘛不及 24／30，要嘛把投毒進 Top-3 的題數推高幾倍。歷史的中文 20% 是既有 golden BM25 的 Top-1 基準，不是當前 PostgreSQL FTS 的實測值；日後重開選型不得用較弱基準或 Top-5 取代 Top-1。<br>**這一項不主張意圖分析的品質**：沒有帶標註的任務描述，golden query set 量的是召回（`02:DISC-005`「未涵蓋」那一段）。量測範圍與它證明不到的東西見[意圖搜尋驗證](intent-search-validation.md)。
...
- [ ] SEC-006 建立 Dataset、Trace 與 Artifact 保存及刪除政策。（允收：`02` §6）<br>**已成立**：①使用者可主動刪除 Run 輸入與 Artifact 且刪除有可追蹤狀態（`CORE-007` 已勾，`Me.deletion_scope` 是契約必填欄位）；②`cleanup_status` 是 Run 列表的一等欄位，不以最終回應成功取代清理確認；③上傳授權限定目的地、大小與有效期（`SBX-008` 已勾）。**允收沒有要求一份獨立的政策文件**，對外揭露由 `GET /policy/data-retention` 與 [consent-and-data-policy.md](mvp/gate-test/consent-and-data-policy.md) 承擔。<br>**仍不勾的兩個理由（都不是缺實作，同 `02` §6）**：①**Run 列本身沒有保存期限值**——`.env.example` 有 `DOWNLOAD_ARTIFACT_RETENTION`／`ANALYTICS_RETENTION`／`AUDIT_RETENTION`／`FEEDBACK_RETENTION`／`TRACE_RETENTION`，沒有 Run 的；②**模型觀測平台今天沒有標的**，這個 repo 沒有任何一條路徑接上它（`creation.py` 唯一碰到的地方是把追蹤關掉），見 [`05` R-24](05-pending-rulings.md)。
...
- [ ] BETA-004 記錄所有阻斷首次成功旅程的問題。（[beta-design.md §5](mvp/m4/beta-design.md) 的判定是「使用者在該段停下且沒有自行繞過」，不能從一次造訪或沒有後續事件推定。）`session_started` 與 `sh_analytics` cookie 已存在，保留為此用途的預備資料，**目前沒有自動消費端**；回報也沒有分析 session 欄位，不能自動串接或以相近時間猜測同一人。是否新增回報關聯仍由丙-78 承接隱私決策，不能用本項的需求代替授權。此事件不參與第七段 Workspace 留存計算；用途文件對齊不代表真人問題已收齊，本項保持未勾選。
...
- [ ] RELEASE-001 所有 MVP 必要需求都有對應測試與結果。（**不勾**：交付物[需求 ID × 測試對照表](requirement-test-matrix.md)已存在並由 `requirement-test-matrix` 逐列對帳，93 個 MVP 必要 ID 全數在表上；但本項要的是「**都**有對應測試」，而表上 `DISC-005` 與 `SKILL-006` 是**未實作**、`NFR-004` 與 `SEC-009` 待真機、`SEC-001` 與 `CONTENT-005` 待真人，另有十四項只有部分準則被證明。**現在缺的不再是那份表，而是表上那些列**）
- [ ] RELEASE-002 完整核心旅程通過端到端驗證。（**不勾**：**只剩真人那一半**。走完全程的那支測試存在並會過——`TestTheCoreJourneyRunsFromIntentSearchToADownloadedPackage` 用同一個 client 依序打過搜尋、詳情、Fork、Test Case 與驗收條件、preflight 確認、Run 到終態、評估、打包 preview 與 create、下載內容與紀錄，最後開啟下載到的套件驗 manifest 溯源回目錄那一筆（`QA-001` 已據此勾選）。**唯一沒被 HTTP 走過的接縫**是「Run 的真實 trace 自動產生評語」：測試等到自動評估發布後，另外寫入最終輸出並直接呼叫評估服務，斷言的判定來自後者。剩下的是真機部署上由**一位真人**走一次（`02` §7 DoD 第 1 條的主詞是「一位新使用者」），那屬封測）
...
- [ ] RELEASE-006 Provider 故障、Run 逾時與清理失敗有可操作處理方式。（**不勾**：機制與測試面已相當完整（`QA-003` 已勾、`SEC-012` 的停派送開關已落地、`O11Y-002`／`003` 已勾），缺的是**「可操作」的那一半**——`SEC-010` 的 runbook（`docs/runbooks/p1-dispatch-halt.md`）與一鍵停用流程本體（`0030_dispatch_halts.sql`、`GET /admin/dispatch`、`PUT`／`DELETE /admin/dispatch/halt`）都已產出，**Alertmanager 的設定與通知路由也已在 repo**：`infra/deploy/control-plane/alertmanager.yml.tmpl`（`critical` 每小時重寄、其餘四小時、`send_resolved`）、`bin/skillhub-alert` 讓 systemd 計時器把失敗推成告警、`tools/ci/deploy-config-check.sh` 以釘選 digest 的 `amtool check-config` 驗過。**缺的是值與一台部署好的控制平面**——`SKILLHUB_ALERT_EMAIL` 與四個 SMTP 設定不在 repo（見[控制平面 runbook](../runbooks/control-plane.md)），而單人團隊裡送不到人的最高級告警等於沒有告警。silence 政策與 Grafana dashboard 仍未做）
- [x] RELEASE-007 使用者可理解執行權限、評估依據與相容性限制。（權限（`TEST-008`／`009`／`011`；`TEST-005` 是後 MVP，不是本項條件）與相容性（[打包、授權溯源與散布](../adr/README.md#打包授權溯源與散布)三層、`0022` 的量測列）已落地。**評估依據那一項的破口已補**：`artifact` 型引用只拿得到 `not_checked`，而 `verifiedQuote` 不接受它，所以帶 `evidence_required` 的條目會被降成 `undetermined` 並帶 `evidence_unverifiable:` 前綴；引用本身仍留著，畫面標「未回驗引文」並說明它只證明檔案存在。引文比對接受正規化，但短於下限的引文只接受逐字）
- [x] RELEASE-008 下載套件可安裝且不包含受保護資料。（後半由 `PACK-004` 成立並對匯出位元組斷言。前半三個條件都到齊：每個 target 打出來的套件平台自己收得回去、同一份輸入打兩次得同一個內容雜湊、授權與作者與溯源三個檔在每個 target 的套件裡都在，三者各有具名測試；`claude-code` 的一次本機安裝由 H-9 完成）
...
- [x] CRED-005 讓 `catalog`（搜尋 embedding、索引增強）、`eval`（評審／建議）、`ingest`（單次生成對照）三個既有 context 各自呼叫 `credit.RecordCost`（context map新增四列 Customer–Supplier 依賴）；MVP 期間這三類只寫 `cost_events` 餵統計，不對使用者扣點（待決策見[Credit 計量與扣款](../adr/README.md#credit-計量與扣款)）。（對應 `02:CRED-005`；依 CRED-001）
