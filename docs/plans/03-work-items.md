...
- [ ] DISC-011 實作意圖分析與查詢改寫，並給中文查詢在 FTS 腿上的下界一個明文決定。（允收：`02:DISC-005` **全部七項**）<br>**工作樹已有實作，尚未完整允收**：五欄分析、關鍵詞與合法篩選、保留原句及使用者修正、分析失敗後仍嘗試向量檢索，已有局部測試與突變反證；跨程序及瀏覽器替身測試的證明範圍見[意圖搜尋驗證](intent-search-validation.md)。<br>**仍須一起完成**：中文詞法策略的正式決定及規定下界、投毒紅線與既有搜尋非回歸、真實前後端整合、每項新增數字的強制／測試與 `one-number` 對帳，以及版本化 Gateway 呼叫、匿名速率與成本控制。歷史中文 20% 是既有 golden BM25 Top-1 基準，不是當前 PostgreSQL FTS 的實測值；選型不得用較弱基準或 Top-5 取代 Top-1。<br>**分析失敗不得繞過向量腿**：以永遠失敗的改寫器證明仍嘗試原句向量且結果非空；向量也不可用時須誠實揭露降級，不能以這個例外抹去前一項要求。完整缺口由 `04` 丙-76 承接；文件同步、Commit／Push 與完整 SHA 的所有遠端 CI 成功前不勾選。
