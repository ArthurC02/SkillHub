...
>
> **共同的三個阻擋**（不逐項重複）：①**甲類四項未到期**（`SEC-009` **46 項**全 pass 0 unknown、`SBX-010`、`SBX-005`／`007` 生產網路面、`SBX-002` 閘門 A 探針）——依 [m4/README.md §4](mvp/m4/README.md) 的裁定它們在 M4 是封測的阻擋項，拍板見 ；②**六項 PDM 未逐項追認**（`03` §1）；③**M1 驗證閘門的 D 日未宣告**——[m4/README.md §5.3](mvp/m4/README.md) 已寫明「**在一個未結案的閘門上宣告 MVP 完成，是把兩個相反的結論同時寫進同一份文件**」。
