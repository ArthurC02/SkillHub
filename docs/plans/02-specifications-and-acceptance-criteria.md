...
**這一節是負責人 2026-09-03 的裁定**，形狀與 `PORT-010c` 同一種：使用者實地看過畫面之後說「首頁應該要有 skill catalog 讓大家選擇，而不是馬上就詢問一堆問題」。**它落在 2026-08-30 起生效的新功能凍結裡，因此它是凍結之後第二項、也是由負責人本人放行的第二項**（第一項是 `PORT-010c`）。
...
- **⑥篩選在目錄上也生效**：`DISC-002` 的四個可用維度同樣作用於目錄。**理由是 §2.2 而不是方便**：一個只在搜尋之後才生效的篩選器，在目錄狀態下就是一排不強制任何事的控制項。
- **⑦不需要登入、不需要模型**：與 `DISC-001` 同樣對匿名開放，且**這條路上不呼叫任何模型**——「這裡有什麼」不該擋在模型閘道的可用性後面。範圍固定為目錄工作區，且沒有任何參數能放寬它（`CORE-006`、[ADR-011](../adr/ADR-011-workspace-scope-and-tenancy.md)）。
