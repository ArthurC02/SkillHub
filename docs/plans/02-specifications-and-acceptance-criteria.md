...
| 是否包含 Script | **M1** | — |
| 驗證狀態 | **M1** | — |
| 類別 | ~~**依 CONTENT-003 策展資料啟用**~~ → **已啟用（2026-09-03，migration `0053`）** | ~~類別目前只存在於策展判斷，無任何欄位保存；需 CONTENT-003 先決定類別是否成為平台欄位，以及使用者自行匯入的 Skill 如何取得類別~~ **前半句答了，後半句沒有**：`skills.category` 為 nullable 欄位，值來自策展清單（`tools/content/backfill-category.sql`，45 筆＝文件 10／寫作 10／資料 25），Fork 繼承；**使用者自行匯入的 Skill 一律 `unassigned`，畫面上是 `尚未定值`（設計 §2.9），不推定、不歸入任何一類**——那正是 [`05` R-19](05-pending-rulings.md) 還沒簽的那半句，本次依負責人「分類不明確」的指示先落 R-19 的 (a) 當第一版，(b)（索引期由模型判定並標示）仍是登記的升級路徑。篩選值域三個（`documents`／`writing`／`data`），`?category=` 逐字比對，不再回 400。<br>**這一項也落在 2026-08-30 起生效的新功能凍結裡**（同 `DISC-006`，本格原本沒提）：它隨同日稍晚的**第四批放行**落地（commit `69efad4`，五項 UI／UX 指示，放行者負責人本人）。**凍結例外的累計計數不在本文件維護**，逐項與範圍見 [`01` §10 的 2026-08-29 裁定表第 4 列](01-goals-and-plan.md) |
