...
**2026-09-06 晚間補充（負責人定的產品形狀）**：創作是**每一輪試跑、給建議、直到使用者可接受**的迴圈，不求一稿滿分。試跑結果帶回會話後，模型先診斷每條沒過的條件修在哪（body、驗收條件或範例輸入），body 由獨立的一次重寫產生（回的草稿不得與試跑那份相同，Go 會退回），修在條件或範例輸入的重提確認；訊息告訴人哪些沒過、改了什麼、可接受或說要改哪裡；會話頁一鍵把該 Test Case 最新已結束的 Run 帶回，不要人貼 Run ID。改稿 materialize 成新版本再試跑（GEN-010 的不可變不變）。下一步的工具意圖與定向問人見 `04` 丙-177、`05` R-47。
