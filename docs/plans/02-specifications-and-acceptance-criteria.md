...
**本節的小節編號不是遞增的**：`DISC-006`（目錄瀏覽）排在 `DISC-005`（意圖分析與查詢改寫）之前，因為兩者分別於 2026-09-03 與 2026-08-29 補寫，各自插在當時讀起來最順的位置。**不搬動整節**——需求 ID 是被外部逐條引用的識別字，重排帶來的收益小於任何一次引用漂移的代價。**要找某一個 ID 請直接搜字串，不要依賴順序。**
...
- 使用者可依類別、來源層級、Agent、是否包含 Script、是否需要 MCP 與驗證狀態篩選。（**修訂**：原文保留，六個維度改為分階段允收，見下方「篩選維度的允收階段」表。修訂依據：[m1/m1-work-items-audit.md §5.2／§5.3](mvp/m1/m1-work-items-audit.md)——其中 Agent 相容與 MCP 兩維度在 M1 內結構性不可能收斂。）
...
**篩選維度的允收階段**（依 [m1/m1-work-items-audit.md §5.3](mvp/m1/m1-work-items-audit.md) 新增；此表只切分允收時程，不改變上方任何一條準則的內容）：
...
**引用訂正：本節的 `DISC-006` 與 `03` 的 `DISC-006`（實作 Skill 一般詳情頁，M1 內已勾）不是同一件事**，兩者只是共用一個號。本節的承接工作項是 **`03:DISC-012`**，不是 `03:DISC-006`。同型訂正 `02:GEN-004`（`DISC-005` 撞號）已一次，這是第二處。

**這一節是負責人的裁定**，形狀與 `PORT-010c` 同一種：使用者實地看過畫面之後說「首頁應該要有 skill catalog 讓大家選擇，而不是馬上就詢問一堆問題」。**它落在 2026-08-30 起生效的新功能凍結裡，因此它是凍結之後第二項、也是由負責人本人放行的第二項**（第一項是 `PORT-010c`）。
...
- **⑥篩選在目錄上也生效**：`DISC-002` 的**五個**可用維度同樣作用於目錄（**訂正數字**：`script`／`validation`／`agent`／`tier`／`category` 五個，逐條列在 `contracts/openapi/public.yaml` 的 `GET /api/skills/catalog` 參數上；`category` 是同日隨 migration `0053` 啟用的第五個，本條與 `01` §10 M1 節都還停在啟用之前的四個）。**理由是 §2.2 而不是方便**：一個只在搜尋之後才生效的篩選器，在目錄狀態下就是一排不強制任何事的控制項。
...
**圖示訂正（兩行拆成兩軸，語意不變）**：原圖把 `cleaning_up` 接在終止狀態後面的同一條鏈上，讀起來像 Run 會離開 `succeeded` 再進入第九個狀態。實際上它是 `runs.cleanup_status` 這一欄的值，與 `runs.status` 是**兩個獨立的軸**——Run 列表把兩者並列顯示（`03:WS-004`），而「`succeeded` 但 `cleanup_status` 不是 `cleaned`」是一個會真的出現、且必須看得見的組合。
...
上一條允收準則自此就沒有數值可驗，值其實已於 PDM-005 定案。以下為該組值，作為本需求的可判定形式：
...
**「作者」的落點（新增；修訂依據：[m4/audit.md §2.1](mvp/m4/audit.md) 對 `PACK-003` 的裁定與 [m4/release-checklist.md §1.9](mvp/m4/release-checklist.md) P-2）**
...
**「可散布」的判準（新增，依 [打包、授權溯源與散布](../adr/README.md#打包授權溯源與散布)；修訂依據：[m4/README.md §7 差-2](mvp/m4/README.md)）**
...
- 生成的入口出現在搜尋的無結果狀態（**`03:DISC-005`** 既有的「沒有夠接近的 Skill」；**引用訂正**：那是 `03` 的工作項編號，照原字面在本文件內查不到——而 `02` 自此另有一個 `DISC-005`（§4.1「意圖分析與查詢改寫」），**兩者不是同一件事**。同型訂正 `02:GEN-002` 已一次，這一處是那批漏掉的）與工作區的 Skill 列表；**首頁不提供與搜尋對等的生成入口**（先搜尋、搜不到再生成，這個順序本身是產品意見）。
...
（**訂正**：本段原本還列了「重試次數上限」與「Fork 來源」兩項並指向生成規則草案當時的待決策章節。**兩項都已在本節上方有準則**——重試見 `GEN-003`、Fork 見 `GEN-002`——那個待決策章節後來已被全部回答。一份文件同時說「可以」與「待決策」，是本批稽核抓到最嚴重的一處自相矛盾。）
...
- **圖旁邊的短標題不是「太短的任務描述」**（深化批補充）：`GEN-001` 的八字下限只在文字是唯一輸入時適用；有流程圖時，任何非空白的文字都原樣交給模型當脈絡，圖才是任務。`apps/llm` 那一側的下限只擋空字串與純空白——它是備援不是規則（鐵律 6）；落地當天它寫的是「至少 8 字」，比它備援的規則更嚴，四個字的標題配一張圖會在那裡被拒成 422、到使用者手上變成「模型服務沒有給出可用的結果」。已訂正並有兩側各一支測試。
...
- 附超過 3 個 `reference_skill_ids`，或其中任一個 Skill **讀不到**（既有 scope 規則：非呼叫者自己的工作區、也不在目錄）、**已下架**、**受 `access_restriction` 限制**、或 **`redistribution = blocked`**，整批請求以**單一錯誤形狀 422** 拒絕；不得部分放行、部分丟棄。**拒絕訊息不指名是哪一個 id、也不說是四種原因裡的哪一種**（稍晚訂正，落地時的判斷：說出「那個 id 是別人工作區的私有 Skill」本身就是在描述別人的私有內容，`NFR-002` 與鐵律 3 都不允許；使用者能做的事——換一個——四種原因下都一樣）。
...
- **截斷後的長度含標記仍在上限內**（深化批訂正）：「超過 20,000 runes 截斷並標記」的正確讀法是**內容加標記合計不超過 20,000**——落地當天是先切到 20,000 再補 12 個字的標記，`apps/llm` 對同一個常數的上限檢查把它拒成 422，於是**任何超過上限的參考都讓整次生成失敗，而失敗的正是截斷規則存在的那個輸入**。已訂正，兩側各一支測試釘住「剛好等於上限」與「超過上限仍成功」。
...
**里程碑**：`PORT-001`～`PORT-010` 為 **M6**（`PORT-002` 與 `PORT-006` 已）；**不計入 MVP 完成度**（同 M5 先例，`01` §7.3），惟該判定本身待裁定（`05`）。
...
**撤回理由**：[淨測試模式](../adr/README.md#淨測試模式)改為真的本機後端這項決策。跑的是真的後端，**端點就是端點**，不存在「清單外」這個概念。<br>**但本條的第二句仍然有效，已移交 `PORT-003`**：**不受信任內容的執行**仍然不行，由派送閘門強制（`02:PORT-010`）。<br>**訂正**：上一句的「由派送閘門強制」在寫下時**不成立**——隔離白名單沒有任何分支看得到內容來源。真正的強制點自此是 `PORT-010` 第 5 條所指的 `requireCuratedContent()`。
...
- 需要 Go 側商業邏輯才能成立的路徑（打包下載、任何寫入後的狀態轉移）**一律屬於清單外**。<br>（**範圍訂正**：本條初稿把「開始一次 Run」也列為清單外。[淨測試模式](../adr/README.md#淨測試模式)之後改為**「不受信任內容的執行」屬清單外**——策展過的展示素材可以在 `PORT-010` 的 Driver 上真的跑完一次。**這個區別是整條規則的重點**：擋的是內容的來源，不是動作本身。）
...
- **不得**為本模式手寫任何替代資料層——具體形式是：不得出現第二個實作 `db/gen` 那批查詢方法的型別，也不得引入第二套 schema 定義。<br>（**措辭訂正**：本條初稿點名了一個 sqlc 在開啟 `emit_interface` 時才會生成的介面型別，而**這個 repo 沒有開那個選項，那個型別不存在**。`devctl automation-check` 的 `doc-identifier` 在提交前抓到了它——一份規格點名一個不存在的識別字，正是本專案反覆記載的那種缺陷。）
...
- **改寫（[`05` R-76](05-pending-rulings.md)、[帳號清除與 Credit](../adr/README.md#帳號清除與-credit)補記）**：Credit 紀錄永久保存，沒有保存期限，也沒有任何清除路徑。機器守著：`cost_events`／`credit_entries` 是不可變表，`db/query-owners.yaml` 的 `immutable_allow` 不列任何刪除它們的 query，新增一條會被 query-owners 檢查擋下。
...
本新增（負責人指示「需要圖表的查詢和展示」，[營運後台](../adr/README.md#營運後台)決策）。
...
本新增（[`05` R-84](05-pending-rulings.md) 裁定）。背景：六個模型端點的逾時秒數寫死在 `apps/llm` 的模組常數裡，而契約在三個端點明寫逾時政策屬於 Go——縮短其中一個要改程式並重新部署。
...
關閉威脅模型的開放問題 **Q19**（`02` 沒有 SEC-* 需求 ID 與允收準則，`03` 第 16 節的十個工作項因此無正式判準）。位置依 Q19 的建議置於非功能需求之後。以下準則**取自 [m0/threat-model-and-sandbox-baseline.md](mvp/m0/threat-model-and-sandbox-baseline.md)（v2）已落地的 32 條威脅與 **46 項基線檢查**（**訂正**：N-08 於 2026-08-25 加入，本文件 `SEC-002`／`SEC-009` 另外三處早已寫 46，只有本句與下方 `SEC-009` 的通過判準停在 45）**，不新增安全要求；該文件未定的門檻值一律標為未涵蓋，不在此處代為定值。
...
- **勾選前提**：Q15（匯入 SSRF 在 MVP 期間的歸屬）與 Q16（後 MVP 威脅不排入 M0–M4 是否為認可決定）已有負責人答覆。
 ...
 - **已答**：負責人追認「後 MVP 的 8 條威脅不排入 M0–M4」，威脅模型 Q16 已回填。**兩個勾選前提到此都有答覆，`03:SEC-001` 同日勾選。**
...
> **標題訂正**：本小節原本的標題逐字是「`SEC-010` 事件嚴重度分級與回應」，於是 `02` 出現**兩個 `SEC-010` 標題**。語意上沒有歧義（後者是前者的子節），但需求 ID 的唯一性在這份文件裡一直沒有機器在守，而 `devctl automation-check` 的 `requirement-refs` 把「`02` 的標題 ID 不得重複」列為 FAIL 條件。**改的是子節標題；需求 ID 與內容一個字都沒動。設計前提：團隊是一個人。** 沒有輪班、沒有交接、沒有 follow-the-sun。因此分級的用途不是「派給誰」而是「該不該現在把服務停掉」，且 **P1 的第一個動作必須是自動的**——等人接的最高等級告警，在單人團隊等於沒有 SLA。
