...
- **②排序說得出來，而且說得出強制它的那一行**：預設排序為**精選在前，其餘依版本建立時間由新到舊**，並在畫面上敘明。**不得使用下載次數、星數或任何人氣訊號排序**（[ADR-041](../adr/ADR-041-trust-signal-vocabulary-typed-absence-and-rule-precedence.md)、設計 §2.11(b)）——這一條的強制點是那條 SQL 的 `ORDER BY`，不是一句承諾。
...
- **⑦不需要登入、不需要模型**：與 `DISC-001` 同樣對匿名開放，且**這條路上不呼叫任何模型**——「這裡有什麼」不該擋在模型閘道的可用性後面。範圍固定為目錄工作區，且沒有任何參數能放寬它（`CORE-006`、[ADR-011](../adr/ADR-011-workspace-tenancy-policy-and-usage.md)）。
...
- 生成前顯示預估成本與本次將消耗的額度，並套用 [ADR-028](../adr/ADR-028-beta-admission-and-quota-enforcement-points.md) 的既有配額強制點；額度不足時**在呼叫模型之前**拒絕，不得先花錢再說。<br>**2026-08-29 條件化（形式同 `RUN-003` 對 `TokenBudget` 欄位的既有處理）**：**「本次將消耗的額度」這一半，只在 `GENERATE_QUOTA` 為強制時適用。** 出貨值依 [ADR-056](../adr/ADR-056-the-generation-allowance-is-its-own-switch-and-it-is-off.md) 是 `off`，此時畫面上**刻意不顯示任何額度數字**——`04` 乙-2 的裁定是「顯示但不強制是兩者中最壞的一種」，畫一個沒被強制的數字就是那個缺陷。**開關一旦轉為強制，這一格要跟著出現**，落點是 `GenerateSkill.tsx` 的同一個 `<dl>`。成本那一半無條件適用且已落地（`03:GEN-008`）。
...
- **生成物頁面列出參考 Skill 的名稱**（生成當時的名稱，純文字）。要不要連到那些 Skill 是 [ADR-066](../adr/ADR-066-generation-takes-a-diagram-or-reference-skills-as-input.md) 待決策 2，本項不預設答案。<br>**2026-09-05 稍晚補充**：待決策 2 已答——每個名稱是一個連到該 Skill 詳情頁的連結；讀者現在還能不能看那一份，由詳情頁自己的 scope／下架／暫扣判定回答，本頁不替它決定、也不因此多說一個字。
...
**互動創作增量（2026-09-05）：以下 GEN-007～012 為已同意的允收；程式已開始落地，完整證據與尚待真實模型／人類量測見 [開發手冊](../development/interactive-creation.md)。** GEN-001～006 保留單次生成基礎的適用範圍；新會話以 [ADR-067](../adr/ADR-067-interactive-skill-creation-with-langgraph.md) 為準，不把舊路徑完成證據當成本節驗收。
...
本節於 **2026-09-08 新增**。修訂依據：[ADR-068](../adr/ADR-068-credit-is-the-only-unit-of-account.md)，回應負責人本輪五點裁定（逐字引用見該 ADR 背景）：Credit 是唯一計價單位且只在本系統內有意義；每個帳號記 Credit Balance，創作依 LLM 用量乘上加成換算扣除額，可能低於 0；負債下限 −50；餘額低於門檻可在開始前擋下；創作與搜尋的實際費用都要記錄，並在特定時機與固定時間統計以推導門檻。**本節不重複既有機制**：LiteLLM 閘道的 Virtual Key／`max_budget`／`tpm_limit`（`SEC-005`、ADR-017）、互動創作既有的 `max_cost_usd`／`raise_budget`（ADR-067）、`RUN_QUOTA`／`GENERATE_QUOTA` 次數配額（ADR-028、ADR-056）對它一體適用，本節只新增「使用者的 Credit 餘額」這一層，且與次數配額是平行機制、互不取代。
...
- 物化的餘額欄位與分錄之和的同步為交易內同步（同 ADR-068 決策 4）。有一條測試證明：分錄寫入與物化欄位更新在同一個資料庫交易內；交易中止時兩者都不生效，不出現「分錄寫了、餘額沒動」或反過來的中間狀態。
...
- 既有的會話單場成本上限 `max_cost_usd` 與 `raise_budget`（ADR-067、`05` R-45）不因本節新增而改變行為。既有涵蓋這兩者的測試在本節新增後不需修改仍然通過。
...
- **未涵蓋（待決策）**：面額、加成、保守常數與滾動窗長度的最終值（[ADR-068](../adr/ADR-068-credit-is-the-only-unit-of-account.md) 待決策）。
...
- `cost_events` 不得含使用者的搜尋查詢字、任務描述原文、對話內容或任何自由輸入文字——只含用途、模型、prompt 版本、token 數、`usd_micros`、`workspace_id`／`user_id`（可為 null）、來源 ref 與冪等鍵（同 [ADR-029](../adr/ADR-029-product-analytics-events-and-audit-trace-boundaries.md)「分析事件不留查詢內容」的既有紀律，本節對成本事件套用同一條）。有一條反證測試：以含有可識別字串（例如一段搜尋查詢文字）的呼叫觸發成本事件寫入，斷言該字串不出現在寫入列的任何欄位。
...
- 事件驅動：一場互動創作會話終結（保存、放棄、逾時，ADR-067 既有終態）時，該場的成本收斂為一列摘要，成為統計輸入的一個樣本點。有一條測試證明：模擬一場會話走到任一終態，斷言產生恰好一筆該會話的成本摘要，不因會話有多步而產生多筆。
...
- **實作形式是允收的一部分**：算繪器直接產生 React 節點、不使用 `dangerouslySetInnerHTML`、也不接受任何原始 HTML；`<a>` 與 `<img>` 不是被過濾掉的，是**沒有任何程式路徑產生得出來**（`apps/web/src/components/ModelMarkdown.tsx`）。
...
- **給人看的一律標出來、不准剝**。Skill 本文是人要採用的東西，而[鐵律 4](../../AGENTS.md) 說 Skill Version 不可變：顯示時改位元組等於請人簽一份他沒看過的東西。對人的攻擊（Trojan Source，CVE-2021-42574）靠的正是「看起來是一回事、存起來是另一回事」，拿掉那個字元是把騙術藏起來。**「給人看的」是全站，不只互動創作**：匯入套件的 SKILL.md、版本差異、建議差異都算——判準是「這個人正在讀這段別人寫的字，然後做一個決定」。允收由一份名冊掃描守（`apps/web/src/untrusted-text.test.ts`）而不是逐頁測試，因為這條規則的失效方式是**下一個人再開一處**，不是現有某一處寫錯。
