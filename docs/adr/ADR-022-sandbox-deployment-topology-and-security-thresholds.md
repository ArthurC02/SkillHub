...
| # | 強制條件 | 依據／落點 |
| --- | --- | --- |
| 1 | 每個 Run 一個獨立 gVisor sandbox，`/work`／`/out` 為該容器私有 **tmpfs**（行程結束即由核心回收，不落盤）；不共用任何可寫層、Volume 或快取 | C-01、C-06（M2 已落地並驗證）。**2026-09-10 對帳（`05` R-17a）**：`C-01` 由兩處判定——**宣告面在 T8**（閘門 A：runtime 已註冊、無共用可寫 host 掛載、每 Run tmpfs 設定存在），**執行期在 `SBX-005`**（兩個並行 Run 不共用可寫路徑）。閘門 A 拍的是一台閒置節點，那裡沒有第二個 Run 可看 |
...
**補記（2026-09-10）——R-17 已裁定，上面那三格的判定形式定案於此。** 仍然是對帳：三條全部照 `05` R-17 的建議，而三條改的都是「怎麼判」，沒有一條動到「要求什麼」。

- **`C-01` 拆成宣告面與執行期，兩邊都有承接者（R-17a）**。**閘門 A 只判宣告面**：sandbox runtime 已註冊、沒有共用的可寫 host 掛載、每 Run tmpfs 的設定存在——這三件在一台閒置節點上看得到。**後半句「不與其他 Run 共用可寫路徑」由 `SBX-005` 的整合測試承接**，那裡真的有兩個 Run。**後半句不刪**：刪掉它是把一條真的允收準則換成一個比較好過的閘門，而發現這件事的整個理由就是不要那個。§2 覆蓋表第 1 列的 `C-01` 因此讀作「T8（宣告面）＋ SBX-005（執行期）」，**§3 的 fail-closed 一個字都不改**——變的是 T8 對 `C-01` 回報什麼，不是 `unknown` 還算不算 fail。
- **`T5-5` 的節點 loopback 立一個具名例外（R-17b）**。該子項的判準改為：**被拒絕，且沒有任何 nftables 計數器動過**（即拒絕發生在核心路由層），評分等級 `kernel`。**這不是把標準放鬆**——被拒絕仍然是必要條件，改的是「怎麼證明它被拒絕」，因為封包連 nftables 都沒進到，`iifname`／`daddr` 那條 drop 規則對這個方向永遠不會觸發。**而它是一個可以被推翻的斷言**：哪一天有計數器動了，那一列就會紅，而那時候要看的是為什麼封包開始離開 namespace。T5 其餘七個子項的「每一次嘗試都要留下一筆 drop」**維持原文**。
- **節點輸入契約定案：`/etc/skillhub/node.json` 五欄全部必填（R-17c）**——`node_id`、`role`、`node_created_at`、`iac_commit`、**`build_phase`**。`role` 的字面值定為 **`sandbox-exec`**，追認探針已經在用的字串（「這個字串沒有人裁定過」這個缺陷，用追認關掉和用改名關掉一樣有效，而改名會製造一次沒有理由的不相容）。**`build_phase` 是新的第五欄**：cloud-init 在建置階段寫 `provision`，節點正式服役後由開機腳本改寫為 `serving`。**它取代探針目前判「時戳是不是節點自報的」所用的 2 秒容差啟發式**——啟發式會在一台慢節點上誤判，而沒有人會知道；這是輸入契約要多一個欄位，不是探針推得出來的東西。五欄連同 `role` 的字面值進 `DEPLOY-IAC-001` 的需求清單（[release-checklist §2.8](../plans/mvp/m4/release-checklist.md)），節點供應商與規格見 `05` R-43。
