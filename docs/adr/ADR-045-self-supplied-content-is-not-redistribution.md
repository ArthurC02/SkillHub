...
- ~~**`redistribution` 仍然沒有寫入端點與 audit**（`04` 乙-17 的另一半）。本 ADR 只回答了「使用者自己的內容怎麼辦」；「誰可以把**別人的**內容標成可散布、要什麼證據」仍未決，仍不在 MVP。~~ **2026-08-23 補端點、2026-08-27 由 ADR-057 補判準**（見下方待決策的回填）。**「仍不在 MVP」那半句沒有被推翻，是被繞過**：進 MVP 的不是一條讓使用者放行內容的產品路徑，而是一條 operator 要付出具名證據才走得完的治理路徑。
...
- ~~**`redistribution` 的寫入端點與 audit**（承 ADR-027 待決策、`04` 乙-17）：誰能改別人的內容、改成 `allowed` 要什麼證據。ADR-021 §5.3 已裁定放寬的門檻不能只是一個按鈕。~~
  → **已決，分兩次**：端點與 audit 於 2026-08-23 補上（[`05` R-3c](../plans/05-pending-rulings.md)）；**誰能改（只有 operator）與要什麼證據（具名且必須與凍結快照相符的 `license_expression` ＋來源層級）**於 2026-08-27 由 [ADR-057](./ADR-057-releasing-content-takes-named-evidence-not-a-button.md) 裁定。**本項最後那句話是後來那份 ADR 的整個依據**——「不能只是一個按鈕」變成了「必須是平台能當場否定的東西」。
