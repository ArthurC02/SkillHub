...
| [ADR-083](./ADR-083-aggregates-own-their-rules-in-go-types.md) | Skill（含版本）、Run（含 attempt）、Evaluation 各有一個 Go 型別當 aggregate root：狀態不匯出、方法是純的並擁有規則的唯一定義與不連資料庫的測試、載入吃呼叫端的交易並以列鎖讀出、存回只有一處呼叫 sqlc；不引入 repository interface，SQL 的原子性守衛全部保留，aggregate 不跨 context。取代 ADR-032 §4 第一條末句「補文件與測試即可，不重寫」，§4 其餘三條不變 | Accepted |
