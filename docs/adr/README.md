...
| [ADR-002](./ADR-002-domain-boundaries-and-ownership.md) | 領域邊界與模組責任 | Accepted（package 對應與 Product Analytics 邊界經 ADR-037 修訂） |
...
| [ADR-015](./ADR-015-sandbox-isolation-technology.md) | Sandbox 隔離技術（gVisor 基線） | Accepted（定案紀錄「語意界線」一句的適用範圍經 ADR-050 縮小） |
| [ADR-016](./ADR-016-language-and-framework-selection.md) | 語言與框架（TS／Go／Python） | Accepted |
| [ADR-017](./ADR-017-model-gateway-and-llm-observability.md) | 模型閘道與 LLM 可觀測性（LiteLLM＋Langfuse）——**觀測那一半已由 [ADR-070](./ADR-070-llm-observability-without-an-external-service.md) 縮限為「不外接第三方服務」（2026-09-10）；閘道那一半不動** | Accepted（觀測半邊經 ADR-070 縮限） |
| [ADR-018](./ADR-018-containerized-core-infrastructure.md) | 核心基礎設施容器化自架（E1 起步，取代 ADR-014） | Accepted |
| [ADR-019](./ADR-019-monorepo-structure-and-cicd.md) | Monorepo 目錄結構與 CI/CD | Accepted（§1 目錄結構經 ADR-031 修訂） |
...
| [ADR-026](./ADR-026-evaluation-reassessment-evidence-lifetime-and-judge-trust-boundary.md) | Evaluation 的重評（append-only）、證據壽命與 LLM Judge 信任邊界（併入原規劃的 ADR-027） | Accepted（defence 3 經 ADR-043、ADR-049 修訂） |
...
| [ADR-028](./ADR-028-beta-admission-and-quota-enforcement-points.md) | 封測准入與配額的強制點（允許清單疊在 OAuth 之上；配額強制點是平台計數器，不是閘道） | Accepted（決策 2 的部署設定經 ADR-055 修訂） |
...
| [ADR-032](./ADR-032-ddd-bounded-context-governance-for-platform.md) | Platform 的 DDD Bounded Context 治理與機械強制（補充 ADR-002；depguard 白名單＝context map 的 CI 表述） | Accepted（package mapping 經 ADR-037 修訂；§4 第一條末句經 ADR-083 取代、由 ADR-084 承接） |
| [ADR-033](./ADR-033-sqlc-query-ownership-and-cross-context-write-enforcement.md) | sqlc Query Ownership 與跨 Context 寫入強制（補充 ADR-032；`db/query-owners.yaml` ＋ `automation-check`，先鎖 write） | Accepted（清除路徑第 1、3 條經 ADR-034 修訂） |
| [ADR-034](./ADR-034-cross-context-writes-close-by-inversion-not-by-events.md) | 剩餘跨 Context 寫入以依賴反轉收斂，不事件化（修訂 ADR-033 清除路徑 1、3；交易保證不換最終一致） | Accepted（aggregate 之間的寫入經 ADR-084 改用領域事件） |
| [ADR-035](./ADR-035-read-ownership-enforcement-and-context-map-completeness.md) | Read Ownership 開始強制，Context 對照表補上完整性檢查（回答 ADR-033／034 待決策；`read_allow:` 棘輪＋ AGENTS.md 第 11 條的機械強制） | Accepted（package identity 與 coverage 判定經 ADR-037 修訂） |
...
| [ADR-039](./ADR-039-frontend-design-system-and-ui-evaluation-criteria.md) | 前端設計系統與 UI/UX 評估準則（把 `index.css` 註解裡的隱性決策成文化，否決第三方設計系統；四層＝義務／原則／系統／強制對照表，含誠實的「沒有人把關」空格；手冊在 [docs/design/system.md](../design/system.md)） | Accepted（決策 4 對照表的「`index.css` 是唯一的樣式表」一列經 ADR-085 修訂） |
...
| [ADR-046](./ADR-046-generating-a-skill-from-a-task-description.md) | 從任務描述生成 Skill——**生成物是工作區私有，不是目錄的第四層**：`tier` 三態全是「經過多少人工檢視」的階梯，生成物一格都不符合，於是不進目錄也不進搜尋索引；新增獨立的 `/v1/generate-skill` 而**不放寬改善迴圈的 prompt**（引文回驗在「從零」上整條落空）；`redistribution` 多一個 `generated`（判準同 ADR-045，但與 `self_supplied` 分開，因為「能不能發佈到目錄」的答案不會一樣）；**生成器不得寫 `license` 欄位**——模型編出來的宣告不得佔用「已宣告」這個狀態 | Accepted（決策 1 的啟動時點經 ADR-052 修訂；決策 3 的輸入範圍經 ADR-066 縮限） |
| [ADR-047](./ADR-047-generation-path-rulings-retry-truncation-and-quota.md) | 生成路徑的五個裁定（回答 ADR-046 的全部待決策）：**單次重試且平台不修改模型交出的位元組**（來源紀錄必須重現得出那份套件，否則一個事實有兩份定義）、`max_tokens` 提到 16000 且**截斷是另一個失敗類別不重試**、**額度按「一次生成」計不按呼叫計且失敗不扣**、生成物維持不進目錄但把它換成三個可檢核的前置、**生成物可以當 Fork 的來源因為阻擋它才要寫程式** | Accepted（決策 1 的適用範圍經 ADR-048 縮小；決策 5 的預設模型經 ADR-051 修訂） |
...
| [ADR-052](./ADR-052-m5-starts-in-parallel-with-an-unfinished-mvp.md) | M5 與未完成的 MVP 並行——三個啟動條件全部暫時放行（與 ADR-050 同型：**一個被明示接受的風險，不是一個被解決的問題**）。被移除的是「阻擋」這個效力，三項的事實一件都沒改變。**唯一的硬邊界是「開工不等於曝光」**：生成入口不得對封測使用者出現，否則 `01` §11.2 第一段量到的就不再是搜尋好不好，而那個數字只有一次機會、封測只有 12 個人。ADR 原本自承旗標靠預設值與紀律、沒有任何機制會告訴我們它被誤開過——**該項已於 2026-08-23 在 ADR-052 內回填答案：有，而且已經在跑**（`audit.ActionFeatureFlags` / `feature_flags.roster`，每次 API 啟動寫一次，everything-off 也寫）。**紀律那一半沒有變**：稽核事件記的是「這個部署的設定是什麼」，不擋任何人把旗標打開 | Accepted（經 ADR-053 加上一條投入上限） |
| [ADR-053](./ADR-053-the-assumption-under-m5-gets-its-own-five-people.md) | ADR-046 底下那個零證據的假設（搜不到的人會想要一個被做出來）**脫離 D 日單獨問 5 個人**，並成為 M5 的投入上限：ADR-052 放行了開工，而曝光旗標擋得住「使用者看得到」、**擋不住投入**——這是路線圖上唯一一個「越晚知道越貴」的風險。訊號前只做丟得掉的部分（契約 schema、`redistribution` 值域、量測），管線與 UI 暫停；三段門檻（0／1～2／3+）**寫在問之前** | Accepted（決策 2 的投入上限經 ADR-054 解除） |
...
| [ADR-068](./ADR-068-credit-is-the-only-unit-of-account.md) | Credit 是平台唯一的計價單位，且只在系統內有意義、不可兌出——面額 1 credit = US$0.001，加成預設 1.3（存 basis points，逐筆記當時值）；**兩本帳分開**：`cost_events` 記平台真實支出（不存查詢字，同 ADR-029）、`credit_entries` 記使用者餘額異動，每筆扣點指向其來源 `cost_events`；餘額＝分錄之和，物化欄位僅為讀取捷徑；扣點接在既有 `settleCost` 之後、與快照同交易，冪等鍵 `(session, revision)`，讀不到成本按預留額扣並標記 `estimated`，絕不扣 0；換算一律無條件進位；**三道閘各自獨立**——開始前餘額低於統計推導的門檻即拒絕、每步扣款前餘額減本步預估低於 **−50** 即停、既有 `max_cost_usd`／`raise_budget` 不動；門檻＝最近滾動窗 p95 × 加成（樣本 < 20 退回保守常數並標示估計值），統計由會話終結事件與 River 每日排程雙軌重算；MVP 不接金流，充值＝operator 授予一筆分錄（封測報酬即此機制）；新表併入既有保存清冊與 `cmd/maintenance` 的 `skillhub_purge` 路徑。新增套件 `creator/credit`（ADR-032 §1）。面額與加成最終值、搜尋與試跑是否收點、Credit 過期政策留待負責人裁定 | Accepted（決策 11 由 [ADR-073](./ADR-073-account-deletion-keeps-the-credit-ledger.md) 取代：帳號刪除那一半 2026-09-11、保存期限那一半 2026-09-12 補記） |
| [ADR-069](./ADR-069-the-creation-conversation-streams-steps-not-tokens.md) | 互動創作的等待畫面串流「步驟事件」而不是模型 token：模型輸出在 Go 接受前只是提案（`proposal()` 會整份退回，或以 nudge 丟掉並重問），一次 review 有三次模型呼叫且第三次的 body 會被第二次取代，回應本身是結構化物件而非文字——三者各自否定 token 串流。改串 Go 已寫進 Postgres 的 append-only 狀態事件，傳輸用 SSE（`Last-Event-ID` 續傳直接查事件序號，**不需要 Redis**，因為串的東西已經有家），Worker 只寫、API 只讀，跨行程通知用 `LISTEN`／`NOTIFY` 且可退化成輪事件表。前置工作只有一項：先量 TTFT——沒量之前不知道收益是零點幾秒還是十幾秒。token 串流不關死，但重開需先滿足三個前置（守門對串流內容的裁定、Python 標註哪一次呼叫可串、取消稅進 `cost_events`）。要不要做、SSE 端點如何進 `public.yaml`、量測的付費授權，三件於 2026-09-09 全數簽署並當日落地 | Accepted |
| [ADR-070](./ADR-070-llm-observability-without-an-external-service.md) | LLM 可觀測性不外接第三方服務，縮限 ADR-017 的觀測半邊（閘道半邊完全不動）：Langfuse SDK 埋點、LiteLLM `success_callback` 回呼與 Prompt Management 三項皆不做。**否決回呼那一半的理由是鐵律 11 不是成本**——它要的兩個環境變數裡有一把 Langfuse 金鑰，為了一個封測期間沒有人會看的儀表板在閘道之外多開一個金鑰存放點；同日 `05` R-72 以完全相同的判斷否決把閘道金鑰複製進 CI secret。事實來源改指系統裡已經有的三樣：`cost_events`（花多少、打哪個模型）、Trace 分割表（發生了什麼、順序）、`tools/eval-regression`（品質有沒有退步——ADR-017 指給 Langfuse Datasets/Evals 的兩件事今天實際上就是它在跑）。**放棄的是互動式探索介面**，接受的條件是規模（12 人、45 個 Skill 撐不起 Dataset/Evals 的樣本數）。`03` 刻意不新增承接工作項——承接「不做」的工作項就是永遠不會被勾的空格子 | Accepted |
...
| [ADR-078](./ADR-078-dependency-governance-admission-updates-install-guards-licenses-and-pins.md) | 依賴治理：Dependabot 每週／每月開群組 PR、新版本冷卻 7 天（toolchain 綁住的版本不自動升）；npm 一律 `ignore-scripts`、uv `exclude-newer = "7 days"`；`dep-audit` 加上出貨依賴的授權閘門（允許清單＋SPDX 判讀，兩筆具名接受）與 zizmor 的 workflow 稽核；automation-check `dependency-policy` 擋沒釘 digest／SHA 的映像與 Action；新增依賴先答五個准入問題 | Accepted（node／python／golang／uv 映像與 uv-build 的忽略規則經 ADR-079 取代） |
| [ADR-079](./ADR-079-toolchain-versions-follow-upstream-and-move-together.md) | 工具鏈版本跟著上游升級：Node 24 LTS、Python 3.14、Go 1.27.1、uv 0.12.13、Actions 升到最新主版本，npm 開 `min-release-age=7`；取代 ADR-078 對 node／python／golang／uv 映像與 uv-build 的忽略，改由 `dependency-policy` 要求同一個工具在每個位置版本一致，Dependabot 只改到一處的 PR 會紅並列出其餘位置 | Accepted |
| [ADR-080](./ADR-080-image-vulnerabilities-block-the-images-we-build.md) | 映像漏洞只擋會被部署的那幾顆：`image-scan.yml` 以 syft ＋ grype 掃 compose 拉下來的四顆與我們建的四顆，清單直接從 compose 解析而不另抄一份；閘門分三層——會被部署的 `platform`／`web`／`llm` 有可修的 Critical／High 就紅，上游四顆只報（只有每週排程那一跑設 `FAIL_ON_UPSTREAM=1` 才紅），開發容器 `devtools` 永不紅（不部署，且發現住在第三方工具的編譯產物裡）。掃描器 pin 進 `toolchain_versions.go` 的名冊（沿用 ADR-022 的 `--only-fixed`，延伸 ADR-077 的判準） | Accepted |
| [ADR-081](./ADR-081-frontend-components-in-three-layers-and-server-state-lives-in-api.md) | 前端分三層組件：`pages/`（有位址的畫面）→ `components/`（可重用 UI、共用詞彙與沒有位址的區塊）→ `api/`（伺服器狀態），只能往下 import、頁面不 import 另一頁；react-query 只在 `api/` 使用，每個寫入是一個自己宣告失效的 hook，畫面反應留在呼叫端；快取鍵一律由 `api/queryKeys.ts` 產生；伺服器回應不複製進 `useState`，換參數用 `key` 重掛；`retry: false` 只寫在 `queryClient.ts`。`architecture.test.ts` 守前述四條中可機器判斷的部分 | Superseded（決策 1 與決策 6 的路徑由 [ADR-082](./ADR-082-frontend-features-own-their-pages-sub-components-and-services.md) 取代；決策 2–5 延續） |
| [ADR-082](./ADR-082-frontend-features-own-their-pages-sub-components-and-services.md) | 前端以 feature 收納：`app/`（組裝與外殼）、`features/<name>/`（八段旅程各自的頁面、子元件、service、model）、`shared/`（跨功能 UI）、`core/`（http、快取鍵、登入身分），import 只往內或往下，feature 之間只經對方的 `index.ts`；角色看檔名——`*.page.tsx` 只由路由掛載且每一頁都要有位址、`<頁面>/components/` 是只屬於那一頁的子元件（可直接呼叫自己 feature 的 service）、`*.service.ts` 是唯一用 react-query 的地方且不含畫面、`*.model.ts` 是型別與純函式；ADR-081 的伺服器狀態規則延續，只換位置；測試放在被測物的 feature，量全 app 的尺在 `guards/`。`guards/architecture.test.ts` 守可機器判斷的部分 | Accepted |
| [ADR-083](./ADR-083-aggregates-own-their-rules-in-go-types.md) | Skill（含版本）、Run（含 attempt）、Evaluation 各有一個 Go 型別當 aggregate root：狀態不匯出、方法是純的並擁有規則的唯一定義與不連資料庫的測試、載入吃呼叫端的交易並以列鎖讀出、存回只有一處呼叫 sqlc；不引入 repository interface，SQL 的原子性守衛全部保留，aggregate 不跨 context。取代 ADR-032 §4 第一條末句「補文件與測試即可，不重寫」，§4 其餘三條不變 | Superseded（由 [ADR-084](./ADR-084-aggregates-speak-in-domain-events.md) 全份取代） |
| [ADR-084](./ADR-084-aggregates-speak-in-domain-events.md) | Aggregate 以領域事件說話：root 的狀態不匯出、只有唯讀存取；公開命令不回傳值、不做 I/O，成立就改狀態並記下領域事件，不成立就只記一則帶理由的拒絕事件；載入吃呼叫端交易並以列鎖讀出，存回同交易寫狀態並把事件寫進 outbox，拒絕不存回；aggregate 之間只用事件——outbox → Dispatcher 訂閱 → 訂閱者 context 的 Mailbox（River 佇列，以事件識別去重）→ 消化方法，最終一致；測試只看唯讀狀態與事件。取代 ADR-083 全份（承接其對 ADR-032 §4 第一條末句的取代）；修訂 ADR-034：aggregate 之間的寫入改用事件，搜尋投影與帳號刪除 purge 照舊（purge 送 `05` R-83）；不引入 repository interface，SQL 的原子性守衛全部保留 | Accepted |
| [ADR-085](./ADR-085-styles-split-into-project-layers-and-component-sheets.md) | 樣式分兩層：專案統一風格是 `styles/` 的四層（`tokens` → `base` → `layout` → `patterns`，由 `main.tsx` 依序載入），元件特化放在元件旁同名的 `.css`，只由那個 `.tsx` import；`:root` 與色彩字面值只在 `tokens.css`；元件樣式表的每個選擇器都要含一個只有該資料夾在用的 class，不得外洩成全域規則；只有一個資料夾在用的 class 不進全域層，例外登記並只能變短。不用 CSS Modules、不加依賴。修訂 `system.md` §6「`index.css` 是唯一的樣式表」一列。`guards/design-system.test.ts` 守這四條 | Accepted |
| [ADR-086](./ADR-086-a-citation-names-the-adr-that-stands-now.md) | ADR 的引用指向現行的那一份：ADR 仍然只增不改，但被取代的 ADR 被引用時，同一行要寫出取代它的那一份；新 ADR 在檔頭寫了 `取代`／`修訂` 哪一份，那一份的索引狀態就寫上新編號；程式、設定、契約與測試寫規則本身，非 markdown 檔裡的 ADR 編號只准變少（上線時 852 處）。`automation-check` 的 `adr-citations` 守這三條；寫 ADR 的門檻、合併型 ADR 與搬移、契約裡的編號、各區域的現行決策入口留待負責人裁定 | Accepted |
