...
## 後續（2026-08-27）：四個值已追認，而本 ADR 的決策一個字都沒有變

**PDM-010 §8.1 的四個值於 2026-08-27 追認**，照提案值：首窗 **20**（`min(20,30)`，這正是提案自己點名「請負責人明確擇一，不要留給實作推斷」的那一次擇一）／每窗 **30**／每日 **5**／窗長 **30 天**。紀錄在 [m0/pdm-proposals.md §9.1](../plans/mvp/m0/pdm-proposals.md) 與 [`05` R-1b](../plans/05-pending-rulings.md)。

**追認補的是簽名不是行為**：`entitlements/quota.go` 的四個常數、`DefaultQuotaLimits()` 與它們的測試**一個值都沒有改**，只有註記從 `待追認` 換成追認日期與出處。

**本 ADR 的決策不受影響，而這個分別要講清楚**：

| | 被追認的 | 被本 ADR 關掉的 |
| --- | --- | --- |
| 是什麼 | 四個數字**是多少** | 這個部署**要不要套用**它們 |
| 機制 | PDM 簽名 | `RUN_QUOTA=off` |
| 2026-08-27 之後 | 有簽名了 | **仍然 off** |

所以「上方待決策」那一節的第一句仍然成立——**公開 Beta 是否恢復額度，本 ADR 沒有回答**；改變的只是它恢復時用的那組數字**不再需要另一次簽名**。

**同時解除的一條禁令**：[ADR-028](./ADR-028-beta-admission-and-quota-enforcement-points.md) 決策 4 的「未追認的數字不得出現在畫面上」對這四個數不再成立。**但畫面上今天仍然沒有額度可顯示**，理由換成本 ADR 的開關——顯示跟著強制（`04` 乙-2），而強制是關的。**解除的是禁令，不是開關。** 上方「這個裁定沒有授權的事」那一節因此**逐字仍然成立**：受測者不得被告知有額度。

**本節不修改上方任何一段。** 上方保留追認前的狀態描述，包括「四個值仍是 `待追認`」——那是它被寫下來時的事實，也是本 ADR 主張「不限制是一個動作」的證據之一。
