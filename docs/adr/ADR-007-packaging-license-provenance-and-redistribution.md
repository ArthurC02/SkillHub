...
- **第二種形狀的判定只看兩件事**：根目錄的 `plugin.json` 帶著規格指定的 canonical schema 識別字，且 `name` 合規格（1–64 字元、小寫英數與 `-`／`.`、首尾須英數、不得有連續 `-` 或 `.`）。**兩者其一不成立就是阻擋錯誤**，訊息說出是哪一個——不是默默退到第三種形狀。一個放了 `plugin.json` 的來源已經宣告了自己是什麼，把那個宣告降級處理等於把一個可修的錯誤藏起來。這與「逐個 Skill 獨立判定」不衝突：那條管的是個別 Skill 的驗證，這條是來源層級的事實，與大小上限同一層。
...
- 只讀公規 manifest 的代價落在今天的生態系身上：宿主專屬形狀的來源退到第三種形狀，於是**匯得進來、但沒有 Plugin 層的事實**（沒有 plugin 名稱、版本與 repository，只有各自的相對路徑）。那些來源今天是多數，所以這個代價是現在就在付，不是將來才付。
