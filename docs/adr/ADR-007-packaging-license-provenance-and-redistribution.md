...
### 決策 9：只收兩份公開規格的產物，Agent Plugin 是匯入形狀不是一等公民

平台收的是兩份**公開、跨供應商**規格的產物：**Agent Skill**（一個目錄，根有 `SKILL.md`）與 **Agent Plugin**（一個目錄，根有 `plugin.json`，其下 `skills/` 綑綁 Agent Skill）。兩者是容器與內容物的關係，不是兩個並列的領域概念。

- **各家 Agent 自有的 Plugin 形狀一律不讀。** 這是這個決策的邊界，也是它存在的理由：Skill 那一層早就只有一份規格，Plugin 那一層卻長出過數種宿主專屬形狀，各自的 manifest 路徑、欄位與元件種類都不同。收了其中一種就得回答「為什麼不收另一種」，而那個問題沒有終點。**判準是規格本身是否公開且跨供應商**，不是哪一家的使用者多。宿主專屬 manifest 因此不被當成 manifest 讀——它所在的來源退到第三種形狀，照樣匯入得了裡面的 Skill，只是沒有 Plugin 層的事實。
- **匯入接受三種來源形狀**，三種走同一條管線、同一組驗證：①單一 Agent Skill；②Agent Plugin；③沒有合規 manifest、但目錄樹裡有一個以上 `SKILL.md` 的來源。**第三種是 GitHub repo URL 匯入的常態**——抓回來的是整個 repo 的位元組，而 repo 極少剛好在根目錄放一份 `SKILL.md`；它同時是宿主專屬形狀的退路。
- **第二種形狀的判定只看兩件事**：根目錄的 `plugin.json` 帶著規格指定的 canonical schema 識別字，且 `name` 合規格（1–64 字元、小寫英數與 `-`／`.`、首尾須英數、不得有連續 `-` 或 `.`）。**判準是 `$schema` 而不是檔案路徑**，因為根目錄那個路徑本身已經被別的供應商佔用：量測樣本裡就有一個 repo 在根 `plugin.json` 寫著另一家的 schema 識別字。只看「根目錄有沒有 `plugin.json`」會把它讀成公規產物。
- **宣告別份規格的 manifest 既不讀也不擋。** 它得到一則資訊層級的揭露，來源退到第三種形狀，裡面的 Skill 照樣匯得進來。**擋下它會是另一種供應商偏袒**——平台不讀別家的 manifest，不代表要把別家的內容拒於門外。唯一的阻擋錯誤是**宣告了公規卻不符合它**：`$schema` 對上了而 `name` 不合規，那是一個來源沒通過它自己指名的規格，訊息要說出是哪一項。

- **Skill 的探索位置固定為 `skills/` 的直接子目錄，不往更深處遞迴**，規格明文如此。第三種形狀沒有這個約束，它本來就是走訪整棵樹。
- **展開後的單位仍然是 Skill Version，但套件只有一份**：一個 Plugin 存成**一個**套件物件——整包 Plugin 的位元組，不切成 N 個 zip。N 個 Skill 各自建立獨立版本、共用那一份物件，而**內容雜湊取的是各自子目錄的內容**，不是整包的位元組；`SKILL-001` 的重複判定與授權事實因此仍然逐個成立。切成 N 個 zip 會製造 N 份重疊的位元組，而那些位元組在來源裡本來就是一份。**不引入 Plugin 實體，也不引入「多 Skill 版本」的聚合**——容器格式不足以成為領域概念，而 Plugin 層的事實由每個 Skill Version 的來源欄位承載（plugin `name`、`version`、`repository`，以及該 Skill 在 Plugin 內的相對路徑）。**這三個欄位除 `name` 外都是規格的選填欄位，記它們才有意義是量出來的**：同一份樣本裡的合規 manifest，`repository` 全數具備、`version` 僅一份缺漏。日後真的需要「以 Plugin 為單位下架」再談，那時要的是一個聚合，不是今天這個標籤。
- **共用一份套件物件的代價是每個 Skill Version 必須知道自己的根在哪裡**，否則「一份套件」會讓每一個讀取者都在 Plugin 的根目錄找 `SKILL.md`——那裡只有 `plugin.json`。因此 Version 記下它在套件內的根目錄（本身即整包時為空），而物件鍵與根目錄**成對一起傳**，讀取者拿不到只有鍵的那一半。適用於可攜套件建置、詳情頁掃描、版本差異、試跑前置、改善迴圈與索引增強回填。可攜套件因此從該 Skill 自己的根建起，內容是那一個 Skill，不是整包 Plugin。
- **同一個來源裡兩個目錄宣告同一個 `name` 時，第二個起被拒絕**並說出先占用它的路徑。否則第二個會被當成第一個的新版本，把兩個不同作者宣告的 Skill 併成一條版本鏈——那是不可變版本規則下不可回復的錯誤合併。
- **跨來源也一樣：名字已經屬於另一個來源的 Skill 時，匯入拒絕那一個**，原因碼 `name-held-by-another-source`，訊息說出占用它的來源，並指出兩條出路（到那個 Skill 頁面「上傳新版本」，或改掉這一份的 `name`）。來源的身分是 Plugin 名稱，沒有 Plugin 時是來源網址；**兩邊都有身分且不同才拒絕**——同一個 Plugin 或同一個 repo 重新匯入仍是新版本，單獨上傳的 zip 沒有來源身分，照舊掛成新版本。**Skill 身分仍然只是工作區內的 `name`**，不把 Plugin 併進身分：Plugin 是打包工具，不是結構上的上一層，把它變成身分的一部分就是把它變成結構。
- **找不到任何 `SKILL.md` 時，失敗訊息要說出它找過哪些位置。** 只說缺少 `SKILL.md`，對一個指著整個 repo 的人不構成任何可行動的資訊——他不知道系統要的是根目錄、是 `skills/`、還是他根本指錯 repo。
- **Skill 以外的元件一律不匯入、只揭露、永不執行。** 規格的標準元件只有兩種，Skill 是其一，另一種是 `mcp.json` 宣告的 MCP server；其餘一切（slash command、subagent、hook…）規格自己就劃為宿主專屬，並要求放在反向網域命名空間的頂層目錄之下。兩類都進匯入回應的排除揭露，原因碼 `plugin-component`。**它不是決策 8 的 `excluded_files`**：那一份說的是打包器從一個正在建置的可攜套件裡拿掉了什麼，這一份說的是匯入沒有把來源裡的什麼變成 Skill——時機與主體都不同，而可攜套件從該 Skill 自己的根建起，`mcp.json` 與其他 Skill 根本不在它的建置範圍內。理由不是它們沒有價值，是 MCP server 與 hook 的定義**本身就是執行指令**（`command` 加 `args`）；把它們收進一個之後會被複製進沙箱的套件，等於讓匯入替使用者決定要執行什麼。manifest 的 `extensions` 同理：規格要求未實作的命名空間**連值都不要驗**，平台一個命名空間都不實作，所以全部忽略。**「不匯入、不執行」在沙箱裡的落實方式是「只安裝那一個目錄」**：Provider 請求帶著該 Version 在套件內的根目錄，執行期只把那個目錄裝成 Skill，並在裝完移除解開的暫存副本——`mcp.json` 與其他 Skill 既不被安裝，也不留在沙箱的檔案系統裡。**但它們的位元組仍然跟著那一份套件進入沙箱**，因為套件只有一份（本決策前面那一條）：要讓它們連進去都不進去，就得為試跑另存一份只含該目錄的物件，那與「一個 Plugin 一份套件」直接衝突。這是那個決定的代價，不是尚未完成的修法。
- **manifest 的未知欄位是 info，不是 error**——與決策 8 對 `SKILL.md` frontmatter 的處置相反，而那個相反是有理由的：frontmatter 收緊是因為**那是我們會再發出去的位元組**，寬鬆會讓使用者拿到別處才發現不合規；plugin manifest 不進我們產出的套件。規格散文要求用戶端「回報並忽略」未知欄位後繼續載入，而它發佈的 JSON Schema 在根層寫 `additionalProperties: false`——**兩者衝突，平台照散文**，與決策 8 在散文與參考實作衝突時的處置同一個方向：照可執行／較明確的那一邊，並把衝突記下來。
- **manifest 的 `license` 不成為新的授權來源層級。** 它宣告的是 Plugin 的授權，而打包器不會把 manifest 搬進 Skill 套件，所以拿到套件的人無法重驗它——那牴觸決策 2 的排序準則（證據可稽核的程度）。沒有這一層，這類套件退到 `repo-license-file`，而那一層**本來就會揭露「涵蓋 repository，不必然涵蓋本套件內容」**：方向保守，沒有安全損失，少一個要維護的層級。
- **逐個 Skill 獨立判定，不整批連坐**：五個 Skill 裡四個通過一個被擋，就建立四個並逐個報出第五個被擋的原因；全部被擋才算匯入失敗。規格自己也是這個處置（不合規的 Skill 跳過、其餘繼續）。整批連坐會讓一個 repo 裡任何一個壞 Skill 擋住其餘全部，而那些是不同的位元組、不同的作者宣告。
- **單次匯入建立的 Skill 數量上限為 50**，超過即整批拒絕並說出上限與實際數量。它與 `MaxZipBytes` 守的不是同一件事：一個兩 MB 的 repo 可以合法地含有數百個 Skill，而那一次匯入會一口氣寫進目錄、一口氣吃掉索引增強的模型額度。**這條不是防禦假想敵**：量測樣本裡 52 個 repo 的 `SKILL.md` 數中位數是 16，最大的一個是 7,695，六個超過 100。50 蓋過中位數一大截，擋住的是聚合型 repo；放寬比收緊容易，所以先保守。
- **單一 Skill 的產出仍然只有標準 Agent Skill 套件。** 匯入認得 Plugin，不代表下載會還原成那一包 Plugin——決策 1 的打包形狀一個字不變。把多個 Skill 組成一個 Plugin 是一個發佈功能，不是匯入的對稱操作：作者自己挑選版本組成的 Bundle 以 Agent Plugin 形狀匯出，規則在[發佈、發佈者與組合散布](./ADR-026-publication-publishers-and-bundled-distribution.md)決策 6～8。

### 決策 10：來源連續七天抓不到才算失效，失效只標示、不下架

每日檢查第一次抓不到時蓋上 `unavailable_since`，抓得到就清掉。從那個時間起**未滿七天是「暫時無法取得」，滿七天是「來源已失效」**，判定在平台做，每個畫面畫同一條線。兩者都只標示，**不自動下架**：一次上游機房故障不該把策展內容掃掉，而下架仍是營運者逐筆的判斷。七天的理由是檢查每日一次、告警在最久沒檢查的來源超過兩天時響——七天足以把「上游故障」與「來源消失」分開，又不讓一個永久消失的來源在目錄上長期看起來與正常來源無異。沒有網址的來源（上傳、生成）沒有上游可查，不給這個判定。
...
- 只讀公規 manifest 的代價落在今天的生態系身上：宿主專屬形狀的來源退到第三種形狀，於是**匯得進來、但沒有 Plugin 層的事實**（沒有 plugin 名稱、版本與 repository，只有各自的相對路徑）。**這個代價現在就在付**——以五個 GitHub 主題取回並去重的 182 個 repo 為樣本，帶宿主專屬 manifest 的有 64 個、帶合規根 manifest 的只有 14 個，另有 9 個兩種都放（`tools/contracts/survey_plugin_shapes.py` 可重跑，數字會隨生態系變動）。
- Skill 以外的元件被揭露但不匯入，使用者拿到的是一個**比他指的那個 repo 少東西**的結果。反過來，他也拿不回整套：**平台從不交出那一份已存套件物件**——試跑只安裝該 Skill 自己的目錄，唯一的下載是單一 Skill 的可攜套件。畫面要說出這一點，否則使用者會以為「匯入了一個 Plugin」等於「平台手上有一個 Plugin 可以還給他」。揭露說得出少了什麼，但說不出那些元件在他原本的環境裡做了什麼——落差要由他自己補。
...
- Agent Plugins 規格的 manifest 要不要比照 Agent Skills 規格釘選 commit 與 blob SHA；它有版本字串與 JSON Schema，與 Agent Skills 規格「無版本可釘」的處境不同，所以不是照抄一份設定就成立。
- plugin manifest 要不要在來源根目錄以外的位置也認得。規格定義的 plugin 是**一個資料夾**，沒有說那個資料夾必須是 repo 的根；而量測顯示 repo 常常是一批 plugin 的集合而不是一個 plugin——帶合規根 manifest 的 14 個 repo 裡，仍有四成三的 `SKILL.md` 落在 `skills/` 以外，其中最大一群是 `plugins/<名稱>/skills/<skill>/SKILL.md`。維持「只認根」的結果是**絕大多數來源都拿不到 Plugin 層事實**，決策 9 記那三個欄位的那一半因此幾乎不會被觸發。觸發訊號是第一個「我的 plugin 在 repo 的子目錄裡，為什麼沒有來源資訊」的真實回報。
