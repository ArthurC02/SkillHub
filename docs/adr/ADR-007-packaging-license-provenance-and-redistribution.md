...
- Skill 以外的元件被揭露但不匯入，使用者拿到的是一個**比他指的那個 repo 少東西**的結果。反過來，下載 Plugin 裡的某一個 Skill 拿到的是**整包 Plugin**——套件只有一份，畫面要說清楚這件事，否則使用者會以為自己拿到的只是那一個 Skill。揭露說得出少了什麼，但說不出那些元件在他原本的環境裡做了什麼——落差要由他自己補。
