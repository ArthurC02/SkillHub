...
- **Skill 以外的元件一律不匯入、只揭露、永不執行。** 規格的標準元件只有兩種，Skill 是其一，另一種是 `mcp.json` 宣告的 MCP server；其餘一切（slash command、subagent、hook…）規格自己就劃為宿主專屬，並要求放在反向網域命名空間的頂層目錄之下。兩類都進既有的 `excluded_files` 揭露（決策 8），原因碼 `plugin_component`。理由不是它們沒有價值，是 MCP server 與 hook 的定義**本身就是執行指令**（`command` 加 `args`）；把它們收進一個之後會被複製進沙箱的套件，等於讓匯入替使用者決定要執行什麼。manifest 的 `extensions` 同理：規格要求未實作的命名空間**連值都不要驗**，平台一個命名空間都不實作，所以全部忽略。
