...
- **第二種形狀的判定只看兩件事**：根目錄的 `plugin.json` 帶著規格指定的 canonical schema 識別字，且 `name` 合規格（1–64 字元、小寫英數與 `-`／`.`、首尾須英數、不得有連續 `-` 或 `.`）。**判準是 `$schema` 而不是檔案路徑**，因為根目錄那個路徑本身已經被別的供應商佔用：量測樣本裡就有一個 repo 在根 `plugin.json` 寫著另一家的 schema 識別字。只看「根目錄有沒有 `plugin.json`」會把它讀成公規產物。**兩者其一不成立就是阻擋錯誤**，訊息說出是哪一個——不是默默退到第三種形狀。一個放了 `plugin.json` 的來源已經宣告了自己是什麼，把那個宣告降級處理等於把一個可修的錯誤藏起來。這與「逐個 Skill 獨立判定」不衝突：那條管的是個別 Skill 的驗證，這條是來源層級的事實，與大小上限同一層。
- **Skill 的探索位置固定為 `skills/` 的直接子目錄，不往更深處遞迴**，規格明文如此。第三種形狀沒有這個約束，它本來就是走訪整棵樹。
- **展開後的單位仍然是 Skill Version**：一個來源展開出 N 個 Skill，就建立 N 個獨立版本，各自帶自己的內容雜湊、授權事實與驗證報告。**不引入 Plugin 實體，也不引入「多 Skill 版本」的聚合**——容器格式不足以成為領域概念，而 Plugin 層的事實由每個 Skill Version 的來源欄位承載（plugin `name`、`version`、`repository`，以及該 Skill 在 Plugin 內的相對路徑）。**這三個欄位除 `name` 外都是規格的選填欄位，記它們才有意義是量出來的**：同一份樣本裡的合規 manifest，`repository` 全數具備、`version` 僅一份缺漏。日後真的需要「以 Plugin 為單位下架」再談，那時要的是一個聚合，不是今天這個標籤。
...
- **單次匯入建立的 Skill 數量有上限**，超過即整批拒絕並說出上限與實際數量。它與 `MaxZipBytes` 守的不是同一件事：一個兩 MB 的 repo 可以合法地含有數百個 Skill，而那一次匯入會一口氣寫進目錄、一口氣吃掉索引增強的模型額度。**這條不是防禦假想敵**：量測樣本裡 52 個 repo 的 `SKILL.md` 數中位數是 16，最大的一個是 7,695，六個超過 100。值放部署設定，不在本 ADR 定（同本 ADR 對其他上限數字的既有處置），但**它是這個決策實際的容量閘門**，不是補充條款。
...
- Skill 以外的元件被揭露但不匯入，使用者拿到的是一個**比他指的那個 repo 少東西**的結果。揭露說得出少了什麼，但說不出那些元件在他原本的環境裡做了什麼——落差要由他自己補。
