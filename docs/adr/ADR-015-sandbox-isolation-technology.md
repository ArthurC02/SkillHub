...
- ~~節點編排：Kubernetes（RuntimeClass=runsc）或純 VM＋輕量 Agent。~~ → **已回填（2026-08-14）**：採**純 VM 池＋輕量 Agent，Sandbox 節點不進 Kubernetes**。理由是把 Sandbox 放進與應用共用的編排層，會讓 ADR-001 的平面隔離退化為命名空間隔離。此為 [ADR-018](./ADR-018-containerized-core-infrastructure.md) 明列的容器化例外項；見 [cost-estimation.md](../plans/mvp/m0/cost-estimation.md) §5.1、§7.3。→ 具體形式（compose-per-VM，一節點一個 `sandboxd` 服務，不裝叢集排程器）與節點租戶模型（節點單租戶於「執行平面」，同節點多 Run 併存）見 [ADR-022](./ADR-022-sandbox-deployment-topology-and-security-thresholds.md) 第一部分 Q1、Q2。
- Egress Proxy 具體實作與允許清單管理流程。→ **部分回填（2026-08-14）**：MVP 允許清單維持三項目的地——LiteLLM 閘道、物件儲存短效授權端點、Trace ingestion 端點；首批三個 Skill 類別（`documents`／`writing`／`data`）皆為「上傳檔案進、產出檔案出」，**不需為任何類別開放額外目的地**。見 [pdm-proposals.md](../plans/mvp/m0/pdm-proposals.md) §1、§5.2。~~**具體實作與清單變更流程仍未決，移交 M2（SBX-007）。**~~ → **已回填（2026-08-16）**：[ADR-022](./ADR-022-sandbox-deployment-topology-and-security-thresholds.md) 第一部分 Q3——沙箱層採 nftables default-deny ＋節點固定 DNS 解析器（不部署 Squid／Envoy），允許清單存於 `infra/egress/allowlist.yaml` 並定義變更、複審與目的地記錄流程。同時記錄 M2 實作把沙箱層的允許清單收斂為**一項**（LiteLLM 閘道）——物件儲存與 Trace 端點改由 sandboxd 代搬，沙箱到不了。
- **SEC-002 六項門檻定值與 SEC-009／SBX-010 驗收程序** → [ADR-022](./ADR-022-sandbox-deployment-topology-and-security-thresholds.md) 第二、三部分（2026-08-16 定案）。上方「定案紀錄」列為實作期驗收關卡的兩項，其判準與證據要求由該 ADR 給出；**通過與否仍未改變**——ADR-022 定的是判準，不是結果。
