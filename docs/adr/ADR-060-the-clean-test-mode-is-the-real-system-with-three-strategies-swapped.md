...
- 狀態：**Accepted**（2026-08-29 Proposed → **2026-08-30 Accepted**）

> **落地與驗證（2026-08-30 補記，不改動以下任何決策內容）。** 本 ADR 的形狀由 M6 全數收束證實（`03` §20）：淨測試模式跑的是同一套產品程式、以 `SKILLHUB_CLEAN_MODE` 切換三個實作，旗標未設時行為與生產完全相同。實測補上的兩件事：
>
> 1. **決策 2（`maxConnections=1`）比預期更有牙齒，而代價出現在意料之外的地方。** 它先是讓 outbox publisher 死鎖（publisher 握著唯一連線跨越整個 delivery，而 consumer 的第一個動作是跟同一個 pool 要一條），2026-08-29 修正為「在 advisory lock 下領一批、放掉連線、再投遞」；接著它讓 `PORT-007` 的展示資料**無法從 seeder 那一側寫入**，因為 API 一起來就握死那條連線——唯一可寫的窗口是載體 ready 之後、API spawn 之前，啟動器現在就在那時候建立目錄層 workspace。**兩件事都不是這個決策的錯，是它的正確性真的被強制了。**
> 2. **決策 5（三條軸各自宣告自己不是什麼）在畫面上落地**（`PORT-003`），並在 2026-08-30 由 `PORT-009` 補上第二軸的另一半：那個承載不驗 presigned URL 這件事，現在不只寫在畫面與註解裡，而是**同一組斷言對真 S3 全過、對承載全垮**的量得到的差別。
- 日期：2026-08-29
- 決策者：產品負責人、架構規劃
