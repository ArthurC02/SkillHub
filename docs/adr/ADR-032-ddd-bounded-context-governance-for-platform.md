...
| `cmd/worker` | `entrypoint/worker.BuildWorkers` | `run.Service`（含 gateway 與可工作的 queue client）、`eval.Service`、outbox dispatcher 與全部 River worker／periodic job |
...
| 全部 context → `entrypoint/worker` | **禁止**，與 `apiserver` 同一條規則：**沒有任何 context 可以 import 一個 composition root**（12 條 deny 已入 `.golangci.yml`） | — |
...
2026-09-10（CRED-005 實作註記）：**索引增強的成本事件由 `ingest` 寫，不是 `catalog`。** 上表 `catalog` → `credit` 那一列寫的是「搜尋 embedding、索引增強寫成本事件」，兩件事實際上分屬兩個 context——搜尋 embedding 在 `catalog`（`discovery/service.go` 的 `embedQuery`），索引增強的兩次付費呼叫（`EnrichSkill` 與其後的 `Embed`）在 `ingest`（`admission/enrich.go` 的 `enrichPackage`），因為增強跑在匯入路徑上，而匯入路徑是 `ingest` 的。判定與處置都不變：`ingest` → `credit` 那一列本來就在表上（單次生成的成本事件與扣點分錄），成本事件是同一張表的同一種寫入，不需要新的許可，depguard 也不需要改。本註記只更正**誰付這一筆**。同批新增的第五種付費呼叫沒有出現在這裡的任何一列，因為它還沒有可寫的 kind：`catalog` 的 match-reasons 是一次真的模型呼叫，而 [ADR-068](./ADR-068-credit-is-the-only-unit-of-account.md) 決策 3 的六個 kind 沒有它，migration 0060 的 CHECK 也就擋著——見 `04` 丙-232。
