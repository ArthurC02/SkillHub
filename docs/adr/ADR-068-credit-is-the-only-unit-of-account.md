...
## 2026-09-11 補記：Credit 邏輯審查修掉的九處，以及負債下限是軟的

這一節不推翻任何決策。對 Credit 相關邏輯做了一輪審查，找到十一處：一處負責人裁定只寫文件（負債下限），一處裁定不做（帳號清除時連 `credit_accounts` 一起刪——「清帳號，不要碰 Credit 資料」，維持現狀），其餘九處照原決策修掉，每一處都有一條還原後會變紅的測試。

**負債下限是軟的，負責人同日接受（「接受軟下限」）。** 決策 7 的閘 ② 在花錢前用「餘額 − 本步保留額 ≥ −50」試算，但保留額不從帳上扣住。同一個帳號同時有幾件事在跑時，每一件都對著同一個餘額試算、都會通過，然後各自花掉自己的保留額。所以 −50 保證的是「單獨一件事不會讓帳跌破 −50」，不是「帳永遠不低於 −50」。超出多少有上界：

- **超出量 ≤ 同時在跑的保留額總和 − 其中最大的一筆。** 跌破之後，下一件事的試算一定不過，所以超出不會累積。
- **試跑**：一次 Run 的保留額是閘道上界 $0.50 × 1.3 ＝ 650 點，每個工作區最多同時 2 次（`MaxConcurrentRunsPerWorkspace`），最壞到 −700 點。
- **互動創作**：每一步保留 `max_call_cost_usd`（出貨 $0.10）＝ 130 點；同時開幾場會話沒有上限，每多一場同時在跑就再多 130 點。M5 生成入口不對封測使用者出現（`01` §10 ⛔），所以封測期間這一項是 0。
- **不改成硬下限的理由**：硬下限要在開始時真的扣住保留額、結算時退回，Run 與創作兩條結算路徑都要改；而超出量有界，封測期間不超過一次 Run 的上界。[`05` R-73](../plans/05-pending-rulings.md) 的每人暴險數字據此補記更正。

**畫面與閘門改用同一個定義。** `/me/credits` 的 `can_start` 讀「一步」的統計（`creation_step`），閘 ① 讀「一場」（`creation_session`，見前面「閘 ① 一直讀錯了統計」那節）。樣本夠多之後，畫面會說可以開始、按下去卻被 422 擋。兩者現在都讀 `creation_session`；`debt_floor_credits` 也改讀 `CREDIT_DEBT_FLOOR` 的設定值，不再是寫死的 −50。

**統計排除估計值。** 決策 8 的門檻從實付推導，但滾動窗把 `estimated` 的成本事件也算進去——讀不到閘道回報時記的那一列會把分位數往下拉。會話摘要同理：有任何一步是估計值的會話不算樣本。兩條彙總 query 都只取實付。

**空的滾動窗不再讓每日重算失敗。** 視窗內沒有任何成本事件時，`percentile_cont` 與 `max` 回 NULL，掃進整數就錯，整個重算中止。加上 `coalesce(…, 0)`：樣本數 0 會讓閘 ① 退回保守常數，與樣本不足同一條路。

**統計有保存期限：48 小時。** 閘 ① 讀的是 `cost_statistics` 最新一列，不管它多舊；每日重算一停，門檻就停在最後一次算出來的值。超過 48 小時（錯過兩次排程）視同沒有統計，退回保守常數。

**淨測試模式又兩處單連線死鎖**，與 R-74 那節和 `03:CRED-008` 同一個形狀：創作每步結算在 `finish()` 的交易裡查工作區擁有者，以及花費為 0 的那一步查餘額，兩處都向連線池要第二條連線。改成走呼叫者的交易。

**operator 更正與找不到的工作區。** 契約允許以負數授予做更正，但一律以 `grant` 入帳，而 `grant` 分錄不收負數，所以更正以 500 失敗；負數現在記成 `adjustment`。授予給不存在的工作區原本回 500，改回契約列出的 404。重送會授予兩次維持原樣——契約寫明 MVP 刻意不冪等。
