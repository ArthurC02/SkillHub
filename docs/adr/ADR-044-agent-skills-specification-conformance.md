...
- **規格沒有版本，所以釘選是脆的。** 上游改一行，我們的 blob SHA 就過期。本 ADR 首發時**沒有任何東西會自動告訴我們**；同日的補批補上了排程偵測（見「後續」），但那是通知，不是保護——**規格漂移了，已經發出去的套件仍然是照舊版驗的**。
...
## 後續（2026-08-22 稍晚，同日補批）

本 ADR 的兩個待決策在同一天被關掉。**兩個都不是靠再拍一次板關掉的**——一個是量出來的，一個是套用了 repo 裡已經有的同型答案。

### 決策 4 的解鎖條件已量測：殺傷範圍是零，未知欄位升級為 error

決策 4 自己寫的解鎖條件是「對 45 個 pin commit 套件跑一次驗證，數出帶未知欄位的比例與欄位名分布……量完之後若殺傷範圍為零，升級為 error 不需要再開一次 ADR」。量完了：

| 語料 | 套件數 | 帶規格六欄之外欄位的 | 欄位名分布 |
| --- | --- | --- | --- |
| 45 個選定 seed 套件 | 45 | **0** | — |
| 11 個 pin commit repo 裡的**全部** SKILL.md | 106 | **0** | — |

**第二列才是真正回答問題的那一列，而它是本批補的。** 決策 4 的選項 (a) 那格寫的成本是「為 Claude Code 寫的 Skill 合法地帶著 `argument-hint`、`when_to_use` 等欄位」——**那句話擔心的族群，45 個選定套件按定義一個都不含**（那 45 個是我們自己挑的）。只量它等於用一個沒有取樣到風險族群的量測去回答風險問題。所以 `import_seed.py` 加了 `--all-skills`，把同樣那 11 個已下載的壓縮檔裡**每一個** SKILL.md 都重打包一次，106 個，一樣是零。

順帶一提，**`argument-hint` 與 `when_to_use` 根本不是 Skill 的欄位**，它們是 slash command 的 frontmatter。決策 4 那格的成本估計本身建立在一個錯的前提上——這件事只有量了才會知道。

量測本身留在 repo 裡（`TestSpecFrontmatterCensus`，`SEED_CORPUS` 環境變數閘控，形式比照 QA-002 語料）。**它不對分布做任何斷言**：一個在數字變動時就讓 build 失敗的普查是一條政策，而政策是本 ADR 的，不是那個測試的。它唯一斷言的是每個套件都真的解析成功——一個語料默默載入失敗的普查，那個零沒有意義。

**於是升級**：`frontmatter-unknown-field` 從 warning 變成 error，訊息同時說出怎麼修（搬進 `metadata`，或刪掉）。`INSTALL.md` 那段也跟著改寫——原本說「這裡是警告、別處是硬錯」，現在說的是**你從 Skill Hub 下載的套件不會帶著這種欄位**。

### 待決策 4（漂移偵測）：排程跑、開 issue，不進 merge gate

判準不是新的，repo 裡已經有一個同型的答案：`gvisor-baseline.yml` 也是「上游是別人的 repo、我們需要知道它動了、但它不該有權擋我們合併」。照抄那個形狀：

- **不進 merge gate。** gate 判的是我們的程式碼能不能合併；別人的 repo 今天早上改了一行不是關於我們程式碼的事實。把它接進 gate 等於把「能不能合併」的鑰匙交給第三方的 main branch，包含半夜、包含他們修錯字的那種 commit。
- **但看不見比耦合更糟。** 所以：每週排程、漂移時開 issue、並讓那一次 run 失敗，這樣排程自己的歷史就是證據。
- **「連不上」不等於「沒漂移」。** 抓不到來源是 exit 1，不是綠燈——這正是這個檢查存在的理由。
- **只有一個例外會在 PR 上擋人**：改到 `contracts/spec/SOURCE.json` 或檢查器本身的 PR。那時「釘選對不對」是關於我們程式碼的事實。

實作是 `tools/contracts/check_spec_drift.py`（本機也跑得動）＋ `.github/workflows/spec-drift.yml`。腳本分得出兩件事：**pinned commit 服務的位元組與我們記的 blob SHA 不合**（exit 3，釘選本身壞了，必須先解釋）與**追蹤分支已經離開釘選版本**（exit 2，普通漂移，要人去讀 diff）。首次執行：兩者皆無。
...
- ~~**未知欄位升級為 error 的殺傷範圍**（決策 4）~~ → **已量測並升級**，見上方「後續」。
- ~~**規格漂移偵測**要不要進 CI，以及失敗時是 fail 還是提醒~~ → **已決定：排程跑、開 issue、不進 merge gate**，見上方「後續」。
- **升級為 error 的量測有一個它答不到的邊界**：106 個套件全部來自 11 個 pin commit repo，而使用者**自己寫**的 Skill 不在裡面。封測開始後應該回頭數一次真實上傳的拒絕率；如果那個數字不是零，要重看的是本決策，不是使用者。
