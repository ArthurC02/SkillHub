...
| 趨勢圖 | 成本、點數異動、Run、operator 動作、漏斗五組每日圖，各自歸事實 owner，見決策 9 | `creator/credit`、`run`、`audit`（Generic）、`analytics` |
...
- 五組每日圖，各自查詢歸事實 owner，不回 user id、workspace id 或 email，因此不算個資讀取、不寫 audit：
  - 每日成本（依種類）——`credit`
  - 每日點數異動（依分錄種類的淨額）＋全平台目前餘額總和——`credit`
  - 每日建立的 Run（依目前狀態）——`run`
  - 每日 operator 動作（依動作，動作清單與 operator 動作紀錄同一份）——`audit`
  - 漏斗各段每天到達的數量——搜尋與看詳情取 `analytics`（每個瀏覽工作階段一天算一次），開始試跑取 `run`（每個工作區一天算一次），按下下載取 `analytics`（每個工作區一天算一次）。各段單位不同，不相除成轉換率；每一段附一句伺服器寫的話，說它的一個計數代表什麼，分母系統性偏高的那一段寫明只能讀量級。
- Run 依「目前」狀態分組，是一張「那天建立的 Run 現在怎麼樣」的快照，不是狀態轉移的歷史；今天結束的 Run 會讓昨天那一格的狀態跟著變。
- 不做依帳號或工作區的排行，也不從圖下鑽到個別帳號：那會是新的個資讀取面。要找某個帳號，從帳號與點數頁以 email 查，那條路逐次寫 audit。
