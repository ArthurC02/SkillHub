...
~~**真正的 R5 違規只有一個**（`/lab/datasets`），而它的問題不是用語是**詞性**。~~ **2026-08-24：那一個也修掉了**（`上傳 Dataset` → `Dataset`），**R5 的現行違規清單為空**。原本被記成違規的 `Run 詳情` 已於 2026-08-23 改為 `Run 結果`。<br>**R5 沒有機器**（見 §0.1），所以這張盤點表與程式的一致性靠人；本節這一格在 08-24 那一批就漏更新過一次，是同一輪對抗審查抓到的。
