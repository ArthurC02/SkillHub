...
逐項附證據。**2026-08-23 全面覆核了一次**：八項裡**兩項已解決、兩項是我自己講錯的**（**08-24 再結四項**：IA-2 由 R2 回答、IA-5 的旗標關閉半邊補了匯入出口、IA-7 三頁裁定為 R3 的「具名」那一支、IA-8 的 h1 改名詞——**現在只剩 IA-6（登出狀態的資訊架構，要設計決策）一項開著**——**2026-08-25 再訂正：兩項。§2.3 一直記著 `/workspace/import` 欠第二條頁內入邊，那是一條 R3 的現行違規，而它在本節、§8 與 `04` 三處都沒有編號，所以沒有人數得到它；補為 IA-9**）（IA-3 把 R5 的問題寫成 R1、IA-4 把兩種立場數成三種），訂正就寫在該項底下而不是改掉原文。剩下的逐項標明**誰能決定它**——凡是需要產品負責人的，本檔一律不代簽。
...
### IA-9 `/workspace/import` 欠第二條頁內入邊（R3 的現行違規）
...
**方向**：R3 是 §0 的規則，規則走在程式前面——**要改的是程式，不是這條規則**。本檔不提修法（那是 `04` 的事），只負責讓它從此數得到。
...
| 375px 不橫向溢出 | [`e2e/rendered.spec.ts`](../../apps/web/e2e/rendered.spec.ts) | 18 個位址，三引擎。**清單是手寫的，沒有棘輪**（`e2e/routes.ts`）——新增一條路由而沒補進去，不會有任何東西 FAIL，這一列因此只保證「表上這 18 個過了」，不保證「全部路由都過了」（2026-08-25 補記；對照組是同表的 `a11y.test.tsx`，它比對 `router.routesById` 所以少一條就 FAIL） |
...
| **IA-6** 沒有登出狀態的資訊架構 | ⏳ **開著**，未查核 | **先查核再排**。已確認的只有 router 層沒有守衛（`beforeLoad` 與 `redirect` 各 0 處，2026-08-25 複驗）；各頁在 401 時畫什麼還沒看。查核完要一個設計決定 |
| **IA-9** `/workspace/import` 欠第二條頁內入邊 | ⏳ **開著**（2026-08-25 補號） | 技術題，不需要產品負責人。R3 是 §0 的規則，所以要改的是程式 |

**九項裡七項已關**（IA-1／IA-2／IA-3／IA-4／IA-5／IA-7／IA-8，其中兩項是本檔自己的訂正）**，開著的是 IA-6 與 IA-9，兩項都不需要產品負責人簽名。**

**先後仍依 system.md §0 的優先序，不依工作量**：排最前面的是 **IA-9**——R3 是可及性，而且它是唯一一項「規則在程式前面、程式現在不服從」的；其次是 **IA-6 的查核**，因為未查核的東西不能排序，查完才輪得到那個設計決定。§0.2 偏離帳裡 `/lab/run` 那一列標的「待修，成本低」不在這張表上，它是帳不是問題。
