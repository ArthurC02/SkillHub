...
機器：**沒有**。現行違規：IA-8（`/lab/datasets`）。**IA-3 已於 2026-08-23 修正**（`Run 詳情` → `Run 結果`）。
...
逐項附證據。**2026-08-23 全面覆核了一次**：八項裡**兩項已解決、兩項是我自己講錯的**（IA-3 把 R5 的問題寫成 R1、IA-4 把兩種立場數成三種），訂正就寫在該項底下而不是改掉原文。剩下的逐項標明**誰能決定它**——凡是需要產品負責人的，本檔一律不代簽。
...
| **IA-1** 探索不在導覽列 | ⏳ 未修 | **需要產品決定**：導覽列要不要有第六項、或第一項該不該是搜尋。它同時牽動 IA-5 與 M5 的曝光邊界，不是純技術題 |
...
**先後仍依 system.md §0 的優先序，不依工作量。** 現在排在最前面的是 **IA-5 的 `filtered_out` 那一格**——它是「快到第一個判斷」的直接違反，而且不需要任何人決定；其次是 **IA-6 的查核**，因為未查核的東西不能排序。**IA-1 排在它們後面不是因為不重要，是因為它是這張表上唯一需要產品負責人的一項。**
