...
> **訂正：本項初版寫「沒有出口」，那在寫下的當時為真，22 分鐘後不再為真。** M5 的生成入口就掛在 `no_results` 這一格（§2.4）。以下是修正後的敘述。
