...
| **1** | `/compare`、`/lab/datasets`、`/runs/$runId/compare`、`/workspace/import` | ❌ 三個舊的按上一頁之後回不去（IA-7）；`/workspace/import` 是剛從 0 升上來的，第二條入邊仍欠 |
...
逐項附證據。**2026-08-23 全面覆核了一次**：八項裡**兩項已解決、兩項是我自己講錯的**（**08-24 再結三項**：IA-2 由 R2 回答、IA-5 的旗標關閉半邊補了匯入出口、IA-8 的 h1 改名詞——**現在只剩 IA-6（登出狀態的資訊架構，要設計決策）與 IA-7（單入口頁）兩項開著**）（IA-3 把 R5 的問題寫成 R1、IA-4 把兩種立場數成三種），訂正就寫在該項底下而不是改掉原文。剩下的逐項標明**誰能決定它**——凡是需要產品負責人的，本檔一律不代簽。
