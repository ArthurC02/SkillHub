...
| `seed-skills.json` 裡 `tier: "curated"` 的 15 筆（九項 `checks` 全 `pass`）＋ [`content/curated-skill-list.md`](../../docs/plans/mvp/content/curated-skill-list.md) §2 的逐項判斷 | [`backfill-curation-tier.sql`](backfill-curation-tier.sql)（可重跑；依名稱比對目錄工作區，**Fork 刻意不含**——授權事實會隨副本走，人工審閱的結論不會） | **無檔案**——寫入 migration `0042` 的 `skills.curation_tier` 與 `curated_version_id`；**需先套用 `0042`**。不跑＝ PDM-002 的策展判定沒有落到任何一列，15 筆策展 Skill 與使用者自匯的在資料上無從分辨 |
...
**改了 `skills` 的分類、策展層級或 `skill_runtime_compatibility` 之後，接著跑一次 `go run ./cmd/reindex`（在 `apps/platform`）。** 目錄列表讀的是 `search_documents` 上的投影，手跑的 SQL 應用程式看不到；不跑的話列表會一直顯示回填前的值（[Query 與寫入所有權](../../docs/adr/README.md#query-與寫入所有權)）。
