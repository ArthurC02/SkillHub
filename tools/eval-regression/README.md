...
| [`test_judge_regression.py`](test_judge_regression.py) | `judge_regression.py` 鏡像 [ADR-043](../../docs/adr/ADR-043-evidence-citation-is-verified-by-content-not-by-its-claimed-source.md) 那四項行為的測試：引文內容回驗與改判、NFC ＋空白摺疊 ＋十二字元門檻、`exact`／`normalized`／`not_checked` 三態、`artifact` 引用不得滿足 `evidence_required`。**不呼叫模型、不讀 DB**：`python tools/eval-regression/test_judge_regression.py` 即可跑，也可被 pytest 收集。四項各自被還原過一次並確認變紅（AGENTS.md 規則 9）。**2026-08-29 起進 CI**（`ci.yml` 的 `contracts-drift` job）與 `task test`——它原本跟著下面那句「不進 CI」，但那句話的理由（要 DB、要 S3、要真的付錢）只對 `judge_regression.py` 本體成立；把一支不到一秒的純函式測試綁在它所守護的昂貴東西的規則上，結果是 ADR-043 的四條行為沒有任何機器在守 |
