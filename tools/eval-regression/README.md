...
| [`test_judge_regression.py`](test_judge_regression.py) | `judge_regression.py` 鏡像 [ADR-043](../../docs/adr/ADR-043-evidence-citation-is-verified-by-content-not-by-its-claimed-source.md) 那四項行為的測試：引文內容回驗與改判、NFC ＋空白摺疊 ＋十二字元門檻、`exact`／`normalized`／`not_checked` 三態、`artifact` 引用不得滿足 `evidence_required`。**不進 CI、不呼叫模型、不讀 DB**：`python tools/eval-regression/test_judge_regression.py` 即可跑，也可被 pytest 收集。四項各自被還原過一次並確認變紅（AGENTS.md 規則 9） |
