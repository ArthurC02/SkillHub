...
- 對應工作項目 ID：**SEC-001**（威脅模型）、**SEC-002**（Sandbox 最低安全基線與阻擋條件），見 `03-work-items.md` 第 16 節。`02-specifications-and-acceptance-criteria.md` 目前**沒有 SEC-* 需求 ID 與允收準則**，因此本文件的覆蓋度只能對照工作項目的一行敘述判定；補齊規格級允收準則列為開放問題 Q19。
...
- 殘餘風險：**ADR-015 狀態仍是 Proposed，定案條件之一正是逃逸測試通過**——在 SEC-009／SBX-010 完成前，隔離強度未經證實。gVisor 自身漏洞需持續跟隨上游修補（ADR-015 明列為持續責任）。同節點多 Run 併存時的橫向風險取決於節點是否單租戶，此點未決。
...
- 殘餘風險：需要網路政策與身分政策雙重落實，且需持續回歸測試（節點編排方案未定，政策實作形式跟著未定）。
- M0 之後工作：SBX-005、SEC-009；新增工作：把「Sandbox → DB 連線嘗試被阻擋」做成常駐監控探針而非一次性測試。
...
| P-03 | 節點以 IaC 建置、無狀態，且在既定週期內重建過 | 節點 | 告警 | ADR-015 |
| P-04 | 節點的 gVisor 版本不低於目前安全基準版本 | 節點 | 阻擋 | ADR-015 |
...
| I-04 | 漏洞掃描**已執行**，且掃描結果在有效期內（有效期值待定，Q18） | 發佈時 | 阻擋 | ADR-005「只允許預先建置、版本化與掃描的 Runtime Image」 |
| I-05 | Image 更新不改變歷史 Run 記錄中的 Runtime Version | 發佈時 | 阻擋 | ADR-005 |
| I-06 | 掃描發現的漏洞等級超過政策門檻（門檻與例外流程待定，Q18） | 發佈時 | 告警 | ADR-005 |
...
| X-02 | Reconciler 定期掃描超過期限的 Sandbox、Volume、Network Rule 與短效憑證（掃描頻率待定，Q18） | 持續 | 阻擋 | ADR-005 |
| X-03 | 遺留資源超過門檻時告警（門檻待定，Q18） | 持續 | 告警 | ADR-005、O11Y-003 |
| X-04 | 遺留資源超過上限門檻時暫停新 Run 排入該節點池（上限門檻待定，Q18） | 持續 | 阻擋 | ADR-005 |
...
- I-04 未通過（未掃描或掃描過期）→ 該 Runtime Image **不得發佈**，亦不得被新 Run 引用；I-06 僅告警，由值班依 Q18 的等級政策判斷是否例外放行。
...
- **SEC-002**：待 Q1～Q3（節點編排、單租戶與否、Egress Proxy 及允許清單流程）有答案；Q18 的門檻值可與實作並行補齊，但驗收前必須有值。
- **SEC-001**：待 Q15～Q16 的範圍確認（匯入 SSRF 的 MVP 歸屬、後 MVP 威脅不排入 M0–M4 是否為認可決定）。Q7～Q14 是後續里程碑的收斂項，不阻擋 SEC-001 勾選，但需各自有承接的工作項目。
- **Q19**（`02` 補 SEC 允收準則）未處理前，兩項的「完全符合允收準則」無正式判準。
...
| ID | 問題 | 為何阻擋 | 來源／建議歸屬 |
| --- | --- | --- | --- |
| Q1 | 節點編排採 Kubernetes（RuntimeClass=runsc）或純 VM＋輕量 Agent？ | 決定第 4 節多數檢查的**實作與驗證形式**（Pod Security Standard vs 自管 OCI 設定），也決定 P-02 網路政策的落實手段 | ADR-015 待決策 |
| Q2 | 節點是否單租戶（一個節點同時只跑一個 Run）？ | 直接決定 TM-EXE-01／TM-EXE-05 的殘餘風險大小；若多 Run 共節點，需要額外的節點內隔離檢查 | 本文件新提出，建議併入 Q1 決策 |
| Q3 | Egress Proxy 的具體實作與允許清單管理流程（誰能加、如何審核、多久複審）？ | N-01～N-07 全部依賴此決策；允許清單是 Top 2 風險的核心控制 | ADR-005、ADR-015 皆列為待決策 |
...
| Q18 | 六項基線檢查的門檻值與維護流程：P-03 節點重建週期、P-04 gVisor「安全基準版本」由誰維護及更新 SLA、I-04 掃描結果有效期、I-06 漏洞等級門檻與例外放行流程、X-02 Reconciler 掃描頻率、X-03／X-04 遺留資源的告警與暫停門檻 | 這六項目前是無值語句（「既定週期」「目前基準版本」「有效期內」「超過門檻」），**無法自動化判定**，等於檢查存在但不可驗收 | 本文件新提出；P-04 建議併入 SEC-010（含 CVE 應變 SLA），其餘建議隨 Q1 節點編排方案一併定值 |
...
| Q19 | `02-specifications-and-acceptance-criteria.md` 目前沒有 SEC-* 需求 ID 與允收準則，SEC-001／SEC-002 只有 `03` 的一行工作項目敘述。依 AGENTS.md「規格新功能先補需求 ID 與允收準則」，應在 `02` 補上 SEC-001／SEC-002 的允收準則（建議置於第 5 節非功能需求之後），否則兩項工作項目缺乏可判定的完成標準。本文件不代改 `02`。 |
