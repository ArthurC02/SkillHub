...
>
> M2 殘項清單的「乙、待負責人決策」另有一份互補的視角（那份記的是阻擋實作的決策，本節記的是產品參數的決策），見 [m2/README.md](m2/README.md) 末節。
...
- [ ] CONTENT-012 同步 [m1/curated-skill-list.md](m1/curated-skill-list.md) §2 三張表的「依賴」欄與 `tools/content/seed-skills.json`。（2026-08-16 的 `deps` 修正只落在 `seed-skills.json`；該人類可讀的 companion 的依賴欄仍是修正前的值，例如 `data-cleanliness-scan` 記為「prompt-only（`pandas`／`numpy`）✅」而實際宣告為 `pandas`／`openpyxl`／`pyarrow`／`chardet`／`python-dateutil`。**以 `seed-skills.json` 為準**。該檔是 M1 的逐筆策展紀錄且含九項檢查判定，改動涉及 13 列跨三張表，故獨立成項而非夾在映像批裡。<br>**本項含一個需要負責人裁定的判定問題，不由映像批代決**：精選檢查 **⑥「不需外網／MCP、依賴在 Runtime Image 內」** 目前對 7 個 Skill 記 `pass`，而修正後的 `deps` 顯示它們各有依賴被映像拒收——`data-cleanliness-scan`（**tier=curated**）、`add-iso3166`、`add-data-dictionary`、`data-comparability`（`pyarrow`）、`pdf`（渲染／OCR 棧＋Node）、`pii-flag`（`presidio-*`）、`document-format-skills`（`pywin32`）。兩種讀法都說得通：**嚴格讀**（宣告的依賴須全在映像內）→ 七筆改 `fail`；**功能讀**（該 Skill 在映像上跑得動）→ 維持 `pass`，因為七筆在 `2026.08-2` 基準上全部判「符合」（走可用子集）。**後果是有界的**：唯一受影響的精選是 `data-cleanliness-scan`，若降級則 `data` 類精選 6 → 5，仍在 PDM-002 的 [4, 6] 目標內，不會跌破下限；已索引數量不受影響。裁定後須同步 `seed-skills.json` 的 `checks.6`、本檔與 CONTENT-003 的達標敘述）
