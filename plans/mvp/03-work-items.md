...
- [ ] CONTENT-012 同步 [m1/curated-skill-list.md](m1/curated-skill-list.md) §2 三張表的「依賴」欄與 `tools/content/seed-skills.json`。（2026-08-16 的 `deps` 修正只落在 `seed-skills.json`；該人類可讀的 companion 的依賴欄仍是修正前的值，例如 `data-cleanliness-scan` 記為「prompt-only（`pandas`／`numpy`）✅」而實際宣告為 `pandas`／`openpyxl`／`pyarrow`／`chardet`／`python-dateutil`。**以 `seed-skills.json` 為準**。該檔是 M1 的逐筆策展紀錄且含九項檢查判定，改動涉及 13 列跨三張表，故獨立成項而非夾在映像批裡；檢查 ⑥「依賴在 Runtime Image 內」的判定亦須依新的准入／拒收結果重新逐筆確認）
