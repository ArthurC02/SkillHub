...
- [ ] SBX-011 把 Runtime Image 發佈流水線接上 **GHCR**，並以 OCI referrer（`cosign attach`／`oras attach`）把 SBOM 與掃描結果作為 attestation 隨 image digest 保存。（**部署批**。[ADR-022](../../adr/ADR-022-sandbox-deployment-topology-and-security-thresholds.md)「補充決策：Container registry」與 ADR-019 待決策 1 已定案 GHCR；本項是它的實作。完成後 I-03 的 SBOM 與 I-04 的 `scanned_at` 才在閘門 A 查得到，**SBX-002 的最後一個未勾原因才解除**；另可接 `schedule:` 重掃 registry 裡的 digest，補上 `infra/images/README.md` 記錄為「目前做不到」的定期重掃）
- [ ] SBX-012 實作 Reconciler 的 **in-flight orphan 表**（`provider_run_id → first_seen_round`）與**分項失敗計數器**（`gateway_revoke_failed`／`sandbox_destroy_failed`）。（[ADR-022](../../adr/ADR-022-sandbox-deployment-topology-and-security-thresholds.md) 第二部分 X-03／X-04 的量測前提：現有 `skillhub_orphan_sandbox_total` 是累加計數器，`increase(...) > 0` 說得出「這段時間銷毀過東西」，說不出「同一筆還在」；而 `cleanup_status` 是整個 attempt 的狀態，區分不出「金鑰撤不掉」與「沙箱殺不掉」，兩者的正確動作相反。完成前 `infra/observability/alerts.yml` 的對應規則是過渡形式）
...
- [ ] SEC-009 進行 Sandbox 逃逸、資源濫用與權限提升測試。（允收：`02` §6）**驗收程序已定案、測試未執行**：[ADR-022](../../adr/ADR-022-sandbox-deployment-topology-and-security-thresholds.md) 第三部分給出 9 個測項（逃逸 PoC 集、syscall 煙霧 fuzz、資源耗盡、Runtime 相容性、網路外洩含東西向與節點面、憑證範圍、清理失敗、宣告式供應鏈稽核、Provider 契約）、與 45 項基線的覆蓋核對、通過判準（**全數 pass 且 0 unknown**，任一 fail 即不得開放外部使用者提交 Skill 執行）、執行者與時機、證據存放（`plans/mvp/m2/sec-009-acceptance/<日期>-<節點>/`，保存 ≥ 1 年）。**該 ADR 另修正一個既有前提：gVisor 的 `systrap` 平台不需巢狀虛擬化**，多數測項可在一般 Linux runner 執行（待部署批第一台節點實測確認），ADR-019 待決策 3 的範圍因此縮小。
