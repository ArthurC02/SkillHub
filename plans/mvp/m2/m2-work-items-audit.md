...
>
> **2026-08-16 第二次回填**：§3.2 留給負責人的「TEST-001／002／003 是否同尺退回」已裁定，見**新增的 [§12](#12-追加節2026-08-16test-001002003-的同尺審查裁定)**。結果為 **TEST-003 退回、TEST-001／002 維持**，因此 §1 總表的「本次退回」自 1 項變為 2 項、「維持勾選」自 33 變為 32——**§1～§11 的內文一字未改**，該處記的是對帳當下（基準 commit `512add1`）的帳，本次裁定是其後的事件。
...
## 12. 追加節（2026-08-16）：TEST-001／002／003 的同尺審查裁定

> **本節為追加，不改寫 §1～§11 的任何判定。** §3.2 把「TEST-001／002／003 是否同尺退回」列為需負責人裁定的懸案（[README.md 乙-7](README.md)），本節執行該裁定並記錄逐條依據。
>
> **裁定結果：TEST-003 退回 `[x]` → `[ ]`；TEST-001／002 維持勾選。** 判定不是「三項一起」，因為它們對應的允收準則本來就不是同一條。

### 12.1 先把尺講清楚

§3.2 說 TEST-004 與其餘三項的差別是「有沒有只能由 UI 滿足的動詞」，並把「使用者可」列為**需要先裁定** `02` §4.3 在描述系統能力還是端到端可用性。**這一步其實不需要裁定——條文自己用主詞回答了。** 一致的判準是：

| 準則的主詞與動詞 | 要在哪個平面滿足 | 為什麼 |
| --- | --- | --- |
| 「系統必須…」「每個 Run 必須…」「系統依…自動建議」 | 系統平面（API ＋ 契約 ＋ 強制點） | 條文描述的是系統的義務，沒有任何一個字要求它被看見 |
| 「**使用者可**…」「**顯示**…」 | 使用者碰得到的平面 | 條文的主詞是使用者；沒有介面時該句是假的，而不是「以另一種形式成立」 |

「使用者可」與「顯示」落在同一格，不是因為它們同義，而是因為**兩者的主詞都不是系統**。差別只在「顯示」連補救方式都被指定了（要有東西被看見），「使用者可」還允許介面形式自由。

這把尺的另一半同樣要說：**它不會把 §9 整節掃掉**。TEST-008／009／010／011 的準則主詞全是系統（`02:TEST-005` 的「開始 Run 前顯示…摘要」有顯示動詞，而 `RunPreflight.tsx` 正是它的顯示面，已存在），RUN／SBX／TRACE 各節亦然。逐條判定的結果是**只有一項落地**，這正是逐條判定與逐節判定的差別。

### 12.2 逐條判定

`02:TEST-001` 共四條，四條分別由不同工作項承接：

| `02:TEST-001` 條文 | 主詞／動詞 | 承接工作項 | 判定 | 依據 |
| --- | --- | --- | --- | --- |
| 1. 每個 Run 必須包含非空白 User Prompt | 「每個 Run 必須」＝系統 | **TEST-001** | **已滿足** | `testlab.go:174 validateDraft`（`CreateTestCase:122`／`UpdateTestCase:160` 呼叫）＋ `0004` 的 `CHECK (btrim(user_prompt) <> '')` ＋ `0017` 的 `test_cases_name_not_blank`。條文沒有一個字要求顯示或使用者操作 |
| 2 前半. **使用者可**純手動建立驗收條件 | 使用者 | **TEST-003**（新增動作） | **缺顯示面** | 端點存在（`testlab.go:249`），`apps/web` 無消費端 |
| 2 後半. 系統依 Prompt 與 Skill 自動建議驗收條件為**可選強化** | 系統，且明文可選 | **TEST-002** | **已滿足** | `suggest.go:63` → `llmclient` → `POST /suggest-criteria`；LLM 未設定回 503 並保留手動路徑。條文自稱「可選強化」，其達成與否不隨必要路徑的介面缺口改變 |
| 3. **使用者可**新增、編輯、刪除及確認驗收條件 | 使用者 ×4 個動詞 | **TEST-003** | **缺顯示面（四個動詞全缺）** | 見 §12.3 |
| 4. 系統必須保存實際執行時使用的 Prompt 與驗收條件快照 | 系統 | **TEST-010** | **已滿足** | `snapshot.go:61 CreateSnapshot`，`0005` trigger 保證不可變（§3 已查證） |
...
| 面向 | 證據 |
| --- | --- |
| 新增／修改／刪除 | `testlab/testlab.go:249`／`:267`／`:301` |
| 並行安全 | `mutateCriteria:313` 的 `LockTestCase` 行鎖 |
| 確認語意 | 明示同意欄位；`:279-286` 在 `newText != list[i].Text` 時清 `ConfirmedAt` 並把 `Source` 轉回 `SourceUser`——**寫回相同文字不撤銷**，這是對的 |
| 測試 | `TestAcceptanceCriteriaLifecycle` |

**缺的是四個動詞的全部使用者面。** 實查（`apps/web/src/`）：

- `pages/` 為 `Compare`／`DatasetUpload`／`Home`／`RunPreflight`／`RunTrace`／`SkillDetail`／`SkillFiles` 七頁，**沒有任何驗收條件的列表、編輯器或確認控制項**。
- `router.tsx` 的路由樹七條，Lab 只有 `/lab/run` 與 `/lab/datasets`。
- `api/lab.ts` 匯出五個函式：`getDatasetLimits`／`uploadDataset`／`getPreflight`／`confirmPreflight`／`startRun`——**沒有任何 criteria 端點的呼叫端**，連 suggest 都沒有接。

**一個容易混淆的地方要先擋掉**：`RunPreflight` 上確實有一個「確認」，但那是**執行前權限摘要**的確認（`02:TEST-005`、`03:TEST-009`、`run_permission_confirmations` 表），與驗收條件的 `confirmed_at` 是兩個不同的物件與兩個不同的動作。用前者頂替後者會讓 `02:TEST-001` 第 3 條的「確認」變成一個沒有人做過的動作卻被記為完成。

**與 TEST-004 完全同型**：五條允收缺一條、缺的那條只差消費端、端點與資料來源都已存在且有斷言。TEST-004 補上 `/lab/datasets` 後依允收重新勾選；TEST-003 補上驗收條件的介面後同樣可重新勾選。實作由新增的 **`03` TEST-012** 承接。

### 12.4 為什麼不是「三項一起退回」，也不是「三項一起維持」

- **不一起退回**：TEST-001 的準則（第 1 條）主詞是 Run、TEST-002 的準則（第 2 條後半）主詞是系統且明文為可選強化。把它們一起退回等於用另一條準則的缺口去否定它們自己那條——那不是同一把尺，那是換了一把更大的尺。
- **不一起維持**：第 3 條的主詞是使用者、四個動詞一個都沒有介面。維持勾選需要主張「使用者可 X」能由 API 滿足，而 MVP 的 Persona 明確是**非技術使用者**（`01` §2.1「MVP 預設介面以非技術使用者可理解的任務語言呈現」）。那個主張撐不住。
- **也不是「準則混了 M4 範圍」**：Test Lab 的介面不是 M4——`03:DESIGN-007`（Test Case、Dataset、MCP 與工具設定流程）落在 §3「資訊架構與體驗設計（M0–M3）」，本來就在 MVP 內、本來就未勾。**準則沒有寫錯，是承接的工作項少了一個**，所以處置是補工作項而不是改準則。三份文件因此只有 `03` 與本文件變動，`02` 一字未改。

### 12.5 §3.2 建議的收斂狀態

§3.2 提的兩個選項中取 **(a)**（承認 §4.3 的準則含 UI、退回並補實作工作項），但**退回的範圍縮小為逐條判定的結果**：

| §3.2 的原始建議 | 本次執行 |
| --- | --- |
| (a) TEST-001／002／003 一併退回 ＋ 新增 Test Lab UI 工作項 | **TEST-003 退回**（TEST-001／002 依逐條判定維持）＋ 新增 `03` **TEST-012** |
| (b) 明文把 §9 界定在 API＋契約層 | **不採**——那會為了讓帳好看而降低既有的產品承諾 |
| 「目前的狀態是兩者都沒說」 | **已說**：判準寫在 `03` §9 開頭，逐條依據寫在本節 |

**§10 第二梯第 5 項（§3.2 的 UI 範圍裁定）自本節起結案**；其下游只剩 TEST-012 的實作。
