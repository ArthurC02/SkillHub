...
- 狀態：**審核已完成（自動化審校，2026-08-16）——42 筆通過、3 筆建議下架待修。** 45 筆審核欄位全數填畢，判定與理由見 [content-review-report.md](content-review-report.md)。
...
> **2026-08-16 執行方式變更（負責人授權）**：本節規定的人工審核未執行，改由 [`tools/content/review_summaries.py`](../../../tools/content/review_summaries.py) 以 **6 項 KPI ＋ 獨立 Judge 模型（`gpt-5.6-terra`）** 對 **45 筆全量**審校，判定寫在各筆表格，方法與逐筆理由見 [content-review-report.md](content-review-report.md)。下表的判準（主判準、三條否決條件、狀態值）**未變**，變的是誰執行與覆蓋率（全量取代抽審）。
...
> 2. 「**非決定性上限**：審核判定只對**已入庫的該版文字**生效；任何重跑增強後，須重新確認入庫文字與審核過的版本一致，否則該筆退回 `待審`」——**這條決定了審核要對著什麼看**，見 §2.4 與 [`catalog-rebuild-report.md` §5](catalog-rebuild-report.md)。**2026-08-16 更新**：45 筆已完成自動化審校，且審校一律讀線上文字，因此不存在「審過的版本與入庫版本不一致」的退回問題；重跑過的 10 筆是先重跑、再對重跑後的線上文字重審。

| 依據 | 要求 | 本工序的對應 | 判定 |
| --- | --- | --- | --- |
| **`02` §4.7 CONTENT-005**（2026-08-16 修訂版） | 見上（生產路徑產生、白名單、限制有值、審核人分離、主判準、否決條件、可追溯、非決定性上限） | 全部欄位與工序均已備齊；審核已由自動化審校完成（45/45） | ⚠️ **審核已執行，但精選 13/15 通過**——2 筆精選建議下架待修（[審校報告 §7](content-review-report.md)） |
| `03-work-items.md` CONTENT-005 | 對首批 Skill 產生一般使用者可理解的摘要 | 45/45 筆皆有繁中白話摘要與 3–5 則雙語任務範例句 | ✅ 已產生；**可理解性主判準 45/45 通過**（審校 KPI2） |
| PDM-002 檢查 ⑦ | 非技術使用者讀得懂它做什麼、需要什麼輸入 | 已審校：主判準 45/45 通過，但精選 2 筆因忠實性／白名單被判需修改 | ⏳ **⑦ 仍為 `pending`**——全 pass 才可改記 `pass` |
...
> **2026-08-16**：正式判定已由自動化審校產生（各筆表格）。本節預判保留供對照——**預判 14 通過／1 待裁定，實際審校結果為精選 13 通過／2 建議下架**；差異出在預判只做目檢與關鍵詞掃描，沒有逐條事實宣稱比對原文（`internal-comms` 的「清楚／精簡」與 `excel-format` 的「預設值講成必填」都是逐條比對才抓得到）。
...
| 審核狀態 | 審核人 | 審核日期 | 備註 |
| --- | --- | --- | --- |
| **建議下架** | automated-review v1（gpt-5.6-terra judge；負責人 2026-08-16 授權） | 2026-08-16 | 重跑 3 次仍重現：KPI1 忠實性：The user must provide both the style to modify and the application scope.。**建議自目錄下架待修，本工序不執行下架**；技術上更宜改 `enrich-skill/v4` 後重跑，見 [審校報告 §7](content-review-report.md) |
...
| 審核狀態 | 審核人 | 審核日期 | 備註 |
| --- | --- | --- | --- |
| **建議下架** | automated-review v1（gpt-5.6-terra judge；負責人 2026-08-16 授權） | 2026-08-16 | 重跑 3 次仍重現：KPI1 忠實性：技能可產出清楚的 FAQ 回覆。；KPI1 忠實性：技能可產出精簡的主管更新。；KPI4 白名單：「清楚的 FAQ 回覆 / clear FAQ responses」屬對 Skill 的評價；KPI4 白名單：「精簡的主管更新 / concise leadership update」屬對 Skill 的評價。**建議自目錄下架待修，本工序不執行下架**；技術上更宜改 `enrich-skill/v4` 後重跑，見 [審校報告 §7](content-review-report.md) |
...
| 審核狀態 | 審核人 | 審核日期 | 備註 |
| --- | --- | --- | --- |
| **建議下架** | automated-review v1（gpt-5.6-terra judge；負責人 2026-08-16 授權） | 2026-08-16 | 重跑 3 次仍重現：KPI1 忠實性：The Skill can extract speaker notes from a presentation and summarize each slide.；KPI1 忠實性：The Skill can remove duplicate slides when combining decks.。**建議自目錄下架待修，本工序不執行下架**；技術上更宜改 `enrich-skill/v4` 後重跑，見 [審校報告 §7](content-review-report.md) |
...
| # | 事項 | 狀態 | 阻擋誰 | 負責 |
| --- | --- | --- | --- | --- |
| 1 | ~~**45 筆的人工審核**（精選 15 必審、已索引 30 抽審 ≥1/3）~~ **改為自動化審校（45 筆全量）** | ✅ **已完成**（2026-08-16，42 通過／3 建議下架），見 [content-review-report.md](content-review-report.md) | — | — |
