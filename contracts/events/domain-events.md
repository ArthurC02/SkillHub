...
| `run.provisioning` `run.preparing` `run.running` `run.evaluating` | `TransitionRun`（非終態） | `to_status`、`from_status`、`reason?`（空值時缺席） | `causation_id`＝attempt ID；開出第一個 attempt 之前的轉移為 NULL（§2 例外 1） |
