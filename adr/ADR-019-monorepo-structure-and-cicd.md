...
1. **Container registry**：GHCR（與 Actions 整合最省事）、部署平台自帶的 registry，或其他？與 ADR-018 的部署平台選擇一起定。
2. **Branch 與 review 策略**：trunk-based（`main` ＋ 短命 feature branch）是預設提案；單人開發期間是否要求 PR 至少一個 approval，或允許自行合併但保留 required checks？
3. **CI runner**：GitHub-hosted runner 足以跑 job 1～5，但 M2 的 gVisor／`runsc` 隔離測試（SBX-010、SEC-009）需要巢狀虛擬化，可能得改用 self-hosted runner。要在 M2 前確認，或接受該類測試只在部署平台的節點上手動執行？
