...
| `docs/adr/` | 57 份架構決策紀錄（ADR-000～056；014 已由 018 取代，024 已由 031 取代，019 §1 現由 031 修訂；042 回答 041 的六個待決策，043 修訂 026 的 defence 3 判準，044 釘選 Agent Skills 規格並定義「符合規格」的範圍；045 讓使用者第一次下載得了自己的 Skill——自己帶進來的位元組交還給自己不是再散布；**046 開 M5：從任務描述生成 Skill，生成物只在個人工作區、不進目錄也不進搜尋索引；047 把 046 的五個待決策一次裁完——單次重試且平台不改模型的位元組、截斷不重試、額度按「一次生成」計且失敗不扣、與 Run 的額度分開；**048 訂正一句被抄了三份的話**——阻擋級檢查不是全部結構性的，`possible-secret` 比對的是檔案內容，所以它不重試；**049 引用回驗比對的是被存下來的值不是它的編碼；050 封測與 Sandbox 驗收並行；051 生成的預設模型改為 mini——實測便宜 21 倍且通過率更高；052 M5 與未完成的 MVP 並行，硬邊界是「開工不等於曝光」；053 M5 底下那個零證據的假設脫離 D 日單獨問 5 個人並成為投入上限；054 上限由授權解除、M5 全面開工——但那個假設的證據量仍然是零；055 封測不套用免費額度，而「不限制」是一個動作不是維持現狀；**056 生成額度是自己的第二個開關且同樣是關的，四個數待追認、追認前不得上畫面**） | [docs/adr/README.md](docs/adr/README.md)（含索引與架構總圖） |
...
| `docs/design/` | **前端的兩把尺**：[system.md](docs/design/system.md) 管**一頁之內**（義務、原則、字級／間距／表面／狀態語彙、強制對照表），[information-architecture.md](docs/design/information-architecture.md) 管**一頁與一頁之間**（規則層 R1～R6、路由清單、導覽、可達性、網址狀態、旗標入口）。**兩份都是活文件，且都有機器**：`design-system.test.ts`／`ia.test.ts`／`contrast.test.ts`／`rendered.spec.ts`。**注意 IA 的方向**：§0 的規則走在程式前面（不一致改程式），§1～§4 的盤點跟在程式後面 | [docs/design/system.md](docs/design/system.md)、[information-architecture.md](docs/design/information-architecture.md) |
...
- 新 ADR 從 **ADR-057** 起編；選型類決策採 ADR-016 格式（含「評估選項」比較），邊界類可用精簡格式。
...
- **M5 的程式面已收斂（2026-08-23）**：`03` §19 的 11 項中 **8 項已勾，3 項為 ◐**：`GEN-009` ③④（要 Sandbox ＋評估管線＋人）、`GEN-008`（缺「生成前顯示預估成本」，而它缺的是一個沒人核定過的數字不是一段程式，`04` 丙-53）、`GEN-003`（失敗紀錄寫下去了但**沒有讀取面**，`04` 丙-55）。已交出的是一條從任務描述到工作區裡一個不可變版本的完整路徑，**加上兩個預設關著的開關**：`GENERATE_QUOTA`（額度，[ADR-056](docs/adr/ADR-056-the-generation-allowance-is-its-own-switch-and-it-is-off.md)）與 `GENERATE_SKILL_EXPOSED`（曝光，ADR-052——**「關」的定義是那條路由不存在，不是它會拒絕**）。**「程式面收斂」不等於這條路成立**：所有「通過」都只到 `skillpkg.Validate`，那是語法門不是品質門，而 [ADR-046](docs/adr/ADR-046-generating-a-skill-from-a-task-description.md) 底下那個假設的證據量**仍然是零**（`ask-5` 一次都沒問過）。
