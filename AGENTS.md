...
- **殘項三類清單**（甲＝部署期驗收、乙＝待負責人決策、丙＝實作或資料工作）見 [docs/plans/04-backlog-and-handoffs.md](docs/plans/04-backlog-and-handoffs.md)（**活文件**，隨時更新）——**開工前先看那份，它記的是「已定值但沒有強制」與「已量到但沒查根因」那一類洞**。目前**甲 4 項、乙 7 項、丙 20 項**（丙類 20 項中 5 項已結案，2 項部分結案）（2026-08-18 M4 對帳批後為甲 4／乙 6／丙 17；**2026-08-22 兩批再加**：設計系統與 UI 對帳批新增丙-28～32 並結案其中三項，產品裁定批依 ADR-042 新增乙-19／乙-20 與丙-33～35）。**丙類沒有一項在等決策**——ADR-042 之後連丙-32 都有裁定了，逐項補法見 [m4/release-checklist.md §1.9](docs/plans/mvp/m4/release-checklist.md)。
...
| `docs/adr/` | 43 份架構決策紀錄（ADR-000～042；014 已由 018 取代，024 已由 031 取代，019 §1 現由 031 修訂；042 回答 041 的六個待決策） | [docs/adr/README.md](docs/adr/README.md)（含索引與架構總圖） |
...
- 新 ADR 從 **ADR-043** 起編；選型類決策採 ADR-016 格式（含「評估選項」比較），邊界類可用精簡格式。
