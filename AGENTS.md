...
| `docs/adr/` | 48 份架構決策紀錄（ADR-000～047；014 已由 018 取代，024 已由 031 取代，019 §1 現由 031 修訂；042 回答 041 的六個待決策，043 修訂 026 的 defence 3 判準，044 釘選 Agent Skills 規格並定義「符合規格」的範圍；045 讓使用者第一次下載得了自己的 Skill——自己帶進來的位元組交還給自己不是再散布；**046 開 M5：從任務描述生成 Skill，生成物只在個人工作區、不進目錄也不進搜尋索引；047 把 046 的五個待決策一次裁完——單次重試且平台不改模型的位元組、截斷不重試、額度按「一次生成」計且失敗不扣、與 Run 的額度分開**） | [docs/adr/README.md](docs/adr/README.md)（含索引與架構總圖） |
...
- 新 ADR 從 **ADR-048** 起編；選型類決策採 ADR-016 格式（含「評估選項」比較），邊界類可用精簡格式。
