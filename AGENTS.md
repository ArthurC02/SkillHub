...
- **M5 的程式面已收斂（2026-08-23）**：`03` §19 的 11 項中 **9 項已勾，2 項為 ◐**：`GEN-009` ③④（試跑後的評估判定分布、人的接受率，**都要 Sandbox ＋評估管線＋人**）與 `GEN-008`（畫面做完了，但 `02:GEN-001` 要求的「生成前顯示預估成本」沒做，而它缺的是一個沒人核定過的數字不是一段程式——`04` 丙-50）。已交出的是一條從任務描述到工作區裡一個不可變版本的完整路徑，**加上兩個預設關著的開關**：`GENERATE_QUOTA`（額度，[ADR-056](docs/adr/ADR-056-the-generation-allowance-is-its-own-switch-and-it-is-off.md)）與 `GENERATE_SKILL_EXPOSED`（曝光，ADR-052——**「關」的定義是那條路由不存在，不是它會拒絕**）。**「程式面收斂」不等於這條路成立**：所有「通過」都只到 `skillpkg.Validate`，那是語法門不是品質門，而 [ADR-046](docs/adr/ADR-046-generating-a-skill-from-a-task-description.md) 底下那個假設的證據量**仍然是零**（`ask-5` 一次都沒問過）。
