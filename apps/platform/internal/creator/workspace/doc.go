// Package identity is the current Go package for ADR-038's proposed product
// domain 「創作者帳戶與工作區」: it lets creators safely use and manage their own
// workspace. Its stable Boundary ID/package slug remains `identity`; ADR-038 is
// Proposed, so this does not rename the package, its owner facts or its path.
// It owns login, sessions and resolving the authenticated user (CORE-005,
// ADR-020; ADR-032 §1 Identity & Workspace, requirement prefixes WS and SEC).
//
// # What it owns
//
...
// it by inverting the direction. Six contexts own rows in a workspace being
