...
func TestNothingWasCollected(t *testing.T) {
	started := RunState{Status: "succeeded", Started: true}
	neverStarted := RunState{Status: "refused"}
	cases := []struct {
		name   string
		run    RunState
		health []StreamHealth
		...
		{"the workload ran and no stream exists at all", started, nil, true},
		{"the workload ran and only the platform's own events exist",
			started, []StreamHealth{{EmittedBy: SourceOrchestr}}, true},
		{"the recorder spoke", started, []StreamHealth{{EmittedBy: SourceSandbox}}, false},
		{"the workload never started, so there is nothing to have collected", neverStarted, nil, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := nothingWasCollected(tc.run, tc.health); got != tc.want {
