// Package trace is the current Go package for ADR-038's proposed product domain
// 「執行證據」: it lets creators inspect masked, traceable evidence of what a
// Skill trial did. Its stable Boundary ID/package slug remains `trace`; ADR-038
// is Proposed and does not rename this package, its owner facts or its path.
// It is the Run Trace context (ADR-032 §1 Supporting, requirement prefix TRACE; ADR-009's Run Trace plane): the masking, storage and reading of what a run actually did.
//
// # What it owns
//
...
//	trace -> run       inverted at the composition root. Trace receives three
//	                   narrow readers for workspace-scoped state, signed-grant
//	                   scope, and ordered transitions; it does not import run or
//	                   sqlc's generated Run rows.
...
// [Service.MaskingActivity] is the one read that is not about a single run: it is what
// the masker did across a window, which internal/run's supervisor reads as
// 02:SEC-010's TraceMaskingStopped criterion. The write takes caller `pgx.Tx` so
// it commits with the state change; the read is pool-backed on Service and returns
// three owner count facts. Neither exposes generated queries or rows.
//
