// Package eval is the current Go package for ADR-038's proposed product domain
// 「成果判定與改善」: it lets creators understand a trial against its criteria,
// compare results and choose whether to adopt an improvement. Its stable Boundary
// ID/package slug remains `eval`; ADR-038 is Proposed and does not rename it.
// It is the control-plane half of the evaluation pipeline (EVAL-001): the Evaluation aggregate, the deterministic checks, the judge leg, and the improvement suggestions that hang off a verdict (EVAL-002).
//
...
//
// Run state, output-manifest evidence and latest-attempt selection stay owned by
// internal/run. API and worker composition roots inject two narrow callbacks
// that translate those facts into this package's RunFacts and EvaluationInput;
// eval neither imports run nor receives generated Run or Artifact rows.
//
