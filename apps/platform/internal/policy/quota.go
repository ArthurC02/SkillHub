...
func Usage(
	ctx context.Context, q *gen.Queries, l QuotaLimits, workspaceID pgtype.UUID, now time.Time,
) (QuotaState, error) {
...
//
// q must be the create-run transaction's handle, and run's requireRunSlot must
// already have run on it — that call takes the per-workspace advisory lock, and
// this one runs inside it. Two things follow, and only one of them is a race fix:
//
...
func EnforceQuota(
	ctx context.Context, q *gen.Queries, l QuotaLimits, workspaceID pgtype.UUID,
) (string, error) {
	if !l.Enforced() {
		return "", nil
	}
	state, err := Usage(ctx, q, l, workspaceID, time.Now())
	if err != nil {
