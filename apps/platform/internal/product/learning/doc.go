// Package analytics is the current Go package for ADR-038's proposed product
// domain 「創作者旅程學習」: it lets the product improve from controlled journey
// signals and feedback, not from a second source of domain truth. Its stable
// Boundary ID/package slug remains `analytics`; ADR-038 is Proposed and does not rename it.
// It writes the product analytics events of the core funnel (02:O11Y-004,
// ADR-029) — the fifth data class, and the only one that is explicitly not a
// source of truth.
//
...
// this context's file, but the file is not the boundary. Run membership and
// quota usage are read through injected Run-owned APIs; identity facts likewise
// stay behind Identity-owned APIs. Measurement owns only what it writes.
//
