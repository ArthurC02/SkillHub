...
type dataRetentionResponse struct {
	...
	Feedback      feedbackDisclosure    `json:"feedback"`
	Events        []analyticsDisclosure `json:"events"`
	Note          string                `json:"note"`
}

type feedbackDisclosure struct {
...
type analyticsDisclosure struct {
...
func retentionDays(retention time.Duration) int {
	days := feedbackDays(retention)
	if days < 0 {
		return 0
	}
	return days
...
func (h *Handler) DataRetention(w http.ResponseWriter, _ *http.Request) {
	feedback := feedbackDisclosure{
		...
		OnAccountDeletion: "去識別而不是刪除：workspace_id 與 user_id 設為 NULL，文字保留（範圍複審建立在人們說了什麼之上，帳號刪除不能悄悄撤回已被計入的回報）",
	}
	if days := feedbackDays(h.FeedbackRetention); days >= 0 {
		feedback.RetentionDays = &days
	} else {
	...
	if h.Svc != nil {
		retention = h.Svc.Retention
	}
	httpx.WriteJSON(w, http.StatusOK, dataRetentionResponse{
		Collecting:    h.Svc.Enabled(),
		RetentionDays: retentionDays(retention),
		Feedback:      feedback,
		Events: []analyticsDisclosure{
			{
				Name:        EventSearchPerformed,
				When:        "送出一次搜尋時",
				Attributes:  []string{"query_length", "query_language", "result_count", "has_results", "filters_applied"},
				NotRecorded: "查詢內容一個字都沒有記；query_language 是書寫系統的分桶（han／latin／mixed／other），不是語系設定",
			},
			{
				Name:        EventSkillDetailViewed,
				When:        "開啟一個 Skill 的頁面時",
				Attributes:  []string{"skill_id"},
				NotRecorded: "你是怎麼到這一頁的——從搜尋結果來還是直接開連結，以及它在那份結果裡排第幾。這兩欄在 migration 0040 之前存在、之後刪除（04 丙-59），現在沒有任何地方記它們",
			},
			{
...

			{
				...
				NotRecorded: "檔案實際上有沒有送出去；那件事記在 download_records，那是領域事實，而兩者分開正是這個事件存在的理由",
			},
		},
