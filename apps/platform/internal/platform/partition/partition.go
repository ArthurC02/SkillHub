...
//
// This is DDL, not DML, so `devctl automation-check`'s raw-SQL tripwire is not
// in play: it flags string literals containing INSERT/UPDATE/DELETE handed to a
// pgx entry point (tools/devctl/query_owners.go, isWriteStatement), and CREATE
// TABLE / DROP TABLE match none of those. The introspection SELECT is a read,
// which ADR-033 does not enforce through sqlc either.
package partition
