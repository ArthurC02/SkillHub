...
func SetAccessRestriction(ctx context.Context, tx pgx.Tx, skillID pgtype.UUID, reason *string) (gen.LockSkillForRestrictionRow, error) {
	if reason != nil && strings.TrimSpace(*reason) == "" {
		return gen.LockSkillForRestrictionRow{}, ErrEmptyRestriction
	}
	q := gen.New(tx)
	before, err := q.LockSkillForRestriction(ctx, skillID)
	if errors.Is(err, pgx.ErrNoRows) {
		return gen.LockSkillForRestrictionRow{}, ErrNotFound
	}
	if err != nil {
		return gen.LockSkillForRestrictionRow{}, err
	}
