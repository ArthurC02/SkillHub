// Package catalog owns skill discovery (ADR-002/ADR-032 §1 Catalog & Discovery,
// requirement prefix DISC). Hybrid retrieval: pgvector similarity ranks,
...
// # Public face
//
// For other contexts, the entire surface is the three projection writes:
// [IndexSkill], [IndexSkillEnriched] and [RemoveSkillFromIndex]. Each takes the
// caller's *gen.Queries and never opens a transaction of its own — that is the
// point, not an implementation detail. [Service] and [Handler] are for the
// composition root.
//
...
//
// Not `skills` or `skill_versions`: catalog reads them constantly and writes
// exactly one of their columns, through their owner.
//
...
//
// Not `skill_runtime_compatibility`, which this package reads for facets and
// neither writes nor owns — no query in db/queries writes it at all.
package catalog
