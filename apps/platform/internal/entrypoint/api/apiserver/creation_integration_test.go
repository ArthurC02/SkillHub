...
func creationFixture(t *testing.T) (*api, *creation.Service, *atomic.Int32) {
	...
	set, err := worker.BuildWorkers(pool, worker.Deps{CreationLimits: creationLimits(), LLM: &llmclient.Client{BaseURL: model.URL, Token: "test-service"}})
	if err != nil {
		t.Fatal(err)
	}
	...
	app, err := apiserver.NewApp(apiserver.Config{Pool: pool, Store: packages, OAuth: &identity.GitHubOAuth{}, DevLogin: true, GenerateExposed: true, CreationExposed: true, CreationLimits: creationLimits(), CreationTransient: creation.TransientClient(transient.URL, "test-worker", 5*time.Second)})
	if err != nil {
		t.Fatal(err)
	}
