...
const (
	migrationPath = "../../../../../../db/migrations/0069_evaluation_suggestions_applied.sql"
...
func catalogueEventTypes(t *testing.T) []string {
	...
	var types []string
	for _, m := range regexp.MustCompile("`((?:run|evaluation|skill)\\.[a-z_]+)`").FindAllStringSubmatch(section, -1) {
		if !seen[m[1]] {
			seen[m[1]] = true
