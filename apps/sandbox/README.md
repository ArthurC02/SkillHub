...
- **沒有任何 Postgres 連線，也不會有**（鐵律 2）。`go.mod` 是獨立的，編譯期就 import 不到 `apps/platform` 的 `internal/`。
...
| `SKILLHUB_SANDBOX_IMAGE` | `skillhub/runtime-agent-sdk:2026.08-13` | Runtime Image。**生產 `runsc` 強制填 digest**（I-02），tag 是移動標的。`2026.08-2` 起含 python3 與目錄宣告的 Python 依賴（[infra/images/README.md](../../infra/images/README.md)）——**目錄的 Agent 相容欄是在 `2026.08-1` 上實測的**，換版後那 45 筆結論不再適用，需重跑 CONTENT-008 基準才會有新的一組 |
...
docker run --rm -v "$PWD:/src" -w /src/apps/sandbox golang:1.25 \
...
docker run --rm -v "$PWD:/src" -v /var/run/docker.sock:/var/run/docker.sock \
  -w /src/apps/sandbox golang:1.25 go test -count=1 ./...

# lint（與 apps/platform 同一版本與設定）
docker run --rm -v "$PWD:/src" -w /src/apps/sandbox \
  golangci/golangci-lint:v2.12.2 sh -c "golangci-lint fmt --diff && golangci-lint run ./..."

# Runtime Image
docker build -t skillhub/runtime-agent-sdk:2026.08-13 infra/images/runtime-agent-sdk
```
