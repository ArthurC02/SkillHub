  ...
  return (
    <section>
      ...
      {/* §2.13,D 類:兩句講的是同一件事的正反面,而反面（公開目錄的不在）才是讀者
          ...
          ── 2026-09-08 稍晚：清單是空的時候這一句不渲染 ────────────────────────
          §2.13 D 類的判準是「第二次來看的人會不會因為這一段而作出不同判斷」，而
          **這一句判斷的對象是清單裡的列**：它解釋為什麼你在目錄看過的某一個不在
          這裡。清單一列都沒有的時候，那個困惑不存在——空狀態的讀者要的是「怎麼
          開始」，而下一句與下面三張卡就是。外部審查連三輪把 h1 底下這一疊讀成
          「文字三明治」，這是那一疊裡唯一一句在空狀態下不做事的。 */}
      {hasSkills && (
      ...
      */}
      {/*
        ...
        **解釋自己為什麼要說這句話**，不是那個型別詞本身。留下的兩件事：這是空清單
        ...
        ── 2026-09-08 稍晚：那句「下一步」再走一次，因為它在同一屏裡是第二遍 ────
        同日早些時候這裡補過一句「從下面挑一種方式，建立第一個。」，當時它指的是下面
        的三張卡。同一批把三張卡從清單後面搬到了這一句正下方，於是那個指向的落點就
        **緊接著這一行**，而它自己帶著一個 `h2`「建立一個 Skill」——同一屏裡，同一件
        事講了兩遍（§3 checklist 第 14 條）。留下的是型別詞那一半，也就是這一段從
        2026-08-25 起唯一在做的事。
      */}
