...
function screenFiles(extension: RegExp): string[] {
  return readdirSync(src, { recursive: true })
    ...
    .filter((f) => extension.test(f) && !f.includes(".test.") && !ASSEMBLY.has(f))
    .filter((f) => !f.startsWith("guards/") && !f.startsWith("testing/"));
}
...
function inboundByRoute(): Map<string, number> {
  ...
  for (const file of screenFiles(/\.tsx$/)) {
...
function flagHooks(): Set<string> {
  ...
  for (const file of screenFiles(/\.service\.ts$/)) {
    const body = readFileSync(join(src, file), "utf8");
