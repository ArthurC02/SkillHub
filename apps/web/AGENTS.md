...
| 讀寫伺服器資料、快取鍵、把東西給兩個頁面共用          | [ADR-081](../../docs/adr/ADR-081-frontend-components-in-three-layers-and-server-state-lives-in-api.md)（三層只往下 import；寫入是自己宣告失效的 `api/` hook；鍵只由 `api/queryKeys.ts` 產生） | `src/architecture.test.ts`                                                                                                                                                                                                      |
| 新的互動或視覺效果（卡片、選單、轉場、游標光）        | system.md §4.8（外部範例庫：只取技法，不裝套件）                                                                                                                                              | `src/design-system.test.ts`（色彩字面值、`opacity`、字級與間距尺度、`<svg` 站點）、`src/contrast.test.ts`                                                                                                                       |
...
跨頁守衛測試（`src/design-system.test.ts`、`src/ia.test.ts`、`src/a11y.test.tsx`、`src/contrast.test.ts`、`src/architecture.test.ts`、`e2e/rendered.spec.ts`）、所有頁面共用的快取鍵表 `src/api/queryKeys.ts` 與 `package-lock.json` 屬於 coordinator（主 Agent）。子代理需要改它們就停下來回報，不自己動——它們是所有頁面共用的尺，兩個寫入者同時改一把尺就沒有尺了。
